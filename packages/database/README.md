...
- A validação do catálogo de `evento_auditoria.acao` e da whitelist fail-closed de `contexto` (homologados em `docs/09` §12) é **regra da camada de aplicação** (`docs/07` §22.3) — **não** existe, por desenho, teste dela nesta suíte. `T-AUD-CONTEXTO` foi **RECLASSIFICADO** para a frente de `apps/api` (`docs/08` §19 `P-BACK-01`) e lá **EXECUTADO/PASSED** na Etapa 2.3A (estado vivo em `docs/10`); não implementar validação, trigger, CHECK ou JSON Schema no banco para replicá-la.
