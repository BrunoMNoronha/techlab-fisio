// TechLab Fisio — normalização dos erros das rotas `/clinica` (fatia CFG-001A).
//
...
// fatia. Aqui se garante o contrato fechado `{ erro }` para tudo o que ocorre
// a partir das guards e do handler de `/clinica`:
//
...
import { ERRO_CLINICA } from "./clinica.dto.js";
...
@Catch()
export class FiltroErroClinica implements ExceptionFilter {
  readonly #logger = new Logger("ConfiguracaoClinica");
    ...
    this.#logger.error(
      `Falha inesperada na configuração da clínica. classe=${
