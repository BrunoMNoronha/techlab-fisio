// TechLab Fisio — normalização dos erros das rotas `/motivos-cancelamento` (CFG-005).
//
// Filtro de CONTROLLER (não global), mesmo desenho de `FiltroErroServicos`:
// contrato fechado `{ erro }` para tudo o que ocorre a partir das guards e dos
// handlers de `/motivos-cancelamento`:
//
...
// do roteamento e não alcançam filtros de controller — status correto, corpo
// padrão da plataforma (mesmo limite de `erro-servicos.filter.ts`).
...
@Catch()
export class FiltroErroMotivosCancelamento implements ExceptionFilter {
  readonly #logger = new Logger("MotivosCancelamento");
    ...
    this.#logger.error(
      `Falha inesperada nos motivos de cancelamento. classe=${
