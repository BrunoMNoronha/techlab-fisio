...
// 5. Verificação dos métodos auxiliares de clienteHttp (GET, POST, PUT, PATCH, DELETE, erros e envelopes)
console.log(`\n${ROTULO} --- 5. Métodos auxiliares de clienteHttp ---`);

const fetchOriginal = globalThis.fetch;
let ultimosParametrosFetch = null;

function mockFetchResponda(status, dadosRetorno, headersRetorno = {}) {
  globalThis.fetch = async (url, opcoes) => {
    ...
    return {
      ...
      status,
      headers: {
        get: (nome) => headersRetorno[nome.toLowerCase()] ?? null,
      },
      json: async () => {
        if (typeof dadosRetorno === "string") throw new Error("Não é JSON");
        return dadosRetorno;
      },
    };
  };
...
// Teste GET
try {
  mockFetchResponda(200, { nome: "Teste" });
  const resGet = await clienteHttp.get("/api/pacientes", {
