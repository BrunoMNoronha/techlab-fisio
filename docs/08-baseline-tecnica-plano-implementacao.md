...
> **Status:** **HOMOLOGADO (ato de homologação: REV. 2) — BASELINE TÉCNICA E PLANO DE IMPLEMENTAÇÃO FÍSICA APROVADOS — REVISÃO VIGENTE: REV. 30**
...
Permanecem **PENDENTES por escopo**, conforme já declarado: Playwright, provedor de armazenamento S3-compatível e versão concreta de engine Docker (§6, §6.1). O **Tailwind CSS deixou de ser `PENDENTE`** na **REV. 23**: foi fixado em **4.3.3** no scaffold do frontend (sprint `FRONT-F0`) e a matriz de §6 passa a registrar a versão homologada.
...
Quatro itens nasceram `PENDENTE` **por escopo, não por falta de pesquisa**; **três permanecem** — Playwright, provedor S3 e engine Docker. O **Tailwind CSS saiu da lista na REV. 23**, quando o consumidor (`apps/web`) passou a existir e a versão pôde ser fixada por medição, e não por antecipação. Fixar hoje a versão de Playwright ou do provedor S3 seria decidir sem o consumidor existir — precisamente o tipo de decisão prematura que TLF-BASE-V1 §4.5 proíbe. O bloqueio de cada um está declarado literalmente na coluna "Motivo" e nenhum deles impede a implementação física da persistência.
