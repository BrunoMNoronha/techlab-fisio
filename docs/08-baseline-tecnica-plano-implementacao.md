...
> **Status:** **HOMOLOGADO (ato de homologação: REV. 2) — BASELINE TÉCNICA E PLANO DE IMPLEMENTAÇÃO FÍSICA APROVADOS — REVISÃO VIGENTE: REV. 15**
...
- **Risco.** Baixo. **Rev.** Total. **Risco realizado: parcial — bloqueio normativo de `T-AUD-CONTEXTO` (`P-E14-01`), não risco técnico.**
...
| **`P-E14-01`** | **`T-AUD-CONTEXTO` bloqueado**: não existe catálogo autoritativo de `evento_auditoria.acao` nem lista branca de chaves de `contexto` por ação (`docs/07` §22.3 exige a política; o catálogo é entregável de `E-18`), nem camada de aplicação onde a validação viveria. **Menor decisão para desbloquear:** homologar o catálogo mínimo de ações + whitelist por ação e definir a camada de validação; o `it.todo` do spec marca o item em toda execução até lá | **Normativa/documental** — registrada na REV. 13 (§13 `E-14`, item 4) | **Bloqueia a conclusão integral de `E-14`** (15/16 itens aprovados); **não bloqueia `E-15`** |
...
**Fim do documento — HOMOLOGADO (ato: REV. 2) — revisão vigente REV. 15 — `docs/08-baseline-tecnica-plano-implementacao.md` — `E-01`..`E-13` EXECUTADAS; `E-14` EXECUTADA COM BLOQUEIO PARCIAL (`P-E14-01`); `E-15` EXECUTADA; `E-16` EXECUTADA (`V-04` APROVADA).**
