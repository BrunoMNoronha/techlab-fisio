...
> **Status:** **HOMOLOGADO (ato de homologação: REV. 2) — BASELINE TÉCNICA E PLANO DE IMPLEMENTAÇÃO FÍSICA APROVADOS — REVISÃO VIGENTE: REV. 31**
...
| **21** | **26/08/2026** | **HANDOFF EDITORIAL PARA A FRENTE DE BACKEND — revisão curta; nenhuma reabertura do plano físico `E-01`..`E-18`, da persistência encerrada (REV. 19/20), da baseline ou de decisão homologada.** Registra somente: (a) **`apps/api` passou a existir** pela **Etapa 2.3A — Fundação do Backend** (NestJS 11 em ESM real, Node 24, health check, integração pela API pública de `packages/database`, catálogo/validator de auditoria de `P-BACK-01`), concluída e homologada após revisão técnica independente (veredito A); (b) **`T-AUD-CONTEXTO` foi EXECUTADO na camada correta (aplicação/backend)** — estado vivo passa a **EXECUTADO/PASSED** (88 testes de auditoria: 83 unitários + 5 de integração via container NestJS real); (c) **`R-BL-02` ENCERRADO** após evidência executável (artefato compilado rodando diretamente em Node 24; `D-ESM-01` não reaberta — esta era exatamente a validação prática prevista); (d) **`P-BACK-01` permanece EM ANDAMENTO** e **`R2.2-04` permanece ABERTO** (sem fluxo emissor real ponta a ponta); (e) **o acompanhamento vivo dessas pendências passa a ocorrer em `docs/10-backend-implementacao.md`** — as entradas históricas de §19 não são reescritas e recebem apenas ponteiro editorial. Persistência física intacta (schema, 9 migrations e golden bit a bit idênticos a `382f366`); guardas de CI preservadas e estendidas com verificações de backend. |
| **22** | **05/09/2026** | **REGISTRO DE EXECUÇÃO DE `V-06.c` — SPRINT `FRONT-F0` (FUNDAÇÃO DO FRONTEND) — sem alteração de baseline, arquitetura, requisito ou decisão homologada.** (a) **`apps/web` passou a existir legitimamente** pela sprint `FRONT-F0`, autorizada expressamente por Bruno Menezes Noronha em 05/09/2026: workspace `@techlab-fisio/web` — Next.js **16.3.4** (App Router), React **19.2.8**, TypeScript **6.0.3** (o da raiz — **mantido**, não substituído pelo `^5` que o template do `create-next-app` sugere), Tailwind CSS **4.3.3** + `@tailwindcss/postcss` 4.3.3, `@types/react` 19.2.18, `@types/react-dom` 19.2.7, `@types/node` 24.13.3 (fixado no workspace, como em `apps/api`). **Fundação técnica sem funcionalidade de negócio e sem integração com `apps/api`.** (b) **`V-06.c` APROVADA** — todos os critérios de sucesso de §12.2 atendidos por execução: Next.js 16 instalado; TypeScript 6.0.x mantido; configuração oficial do Next gerada a partir do template `app-tw` do `create-next-app@16.3.4` e ajustada (`tsconfig.json` estendendo `tsconfig.base.json`, com `strict`, `noUncheckedIndexedAccess`, `noImplicitOverride` e **`skipLibCheck: false`** preservados); `next typegen && tsc --noEmit -p tsconfig.json` **exit 0**; `next build` **exit 0** (Turbopack; verificação de tipos do Next concluída; rotas `/` e `/_not-found` pré-renderizadas). Um único desvio foi encontrado e corrigido **sem tocar em versão alguma**: com `lib: es2023` herdado da base, as declarações do próprio `next` falham sob `skipLibCheck: false` (`TS2304: Cannot find name 'PromiseWithResolvers'` — tipo de ES2024); alinhar `lib` ao valor do template oficial (`["dom", "dom.iterable", "esnext"]`) resolveu — **erro de configuração, não incompatibilidade**, exatamente a distinção que §12.2 exige. Consequência: **`V-06` = INTEGRALMENTE APROVADA** (`V-06.a` + `V-06.b` + `V-06.c`); `R-BL-03` passa ao residual (teto do TypeScript 6.0 medido contra os três consumidores — Prisma 7, Jest 30 e Next.js 16); o **encerramento formal do risco é ato de Bruno**, não desta revisão. (c) **Tailwind CSS**: a coluna "Motivo" de §6 mandava fixar a versão no scaffold do frontend "junto com a versão que o `create-next-app` do Next 16 instalar" — o template declara `^4`, que resolve para **4.3.3** na data; a linha `PENDENTE` de §6 **não foi editada** por esta revisão (a atualização da matriz é ato de homologação). Playwright e S3 permanecem `PENDENTE`. (d) **Validações da sprint, todas verdes, em worktree Git isolado** (`agent/front-f0-fundacao`, a partir de `origin/main` = `c4c9ba2`): `npm run typecheck` (raiz + 3 workspaces); `npm run build` (raiz — `packages/database` → `apps/api` → `apps/web`); suíte da API 24 suites · 592 passed · 3 skipped; integração API × PostgreSQL 18 real 10 suites · 384 passed · 2 skipped; persistência 10 suites · 87 passed; Guarda 1 (10 migrations · 37 objetos protegidos); provas de runtime `verify:argon2-runtime`, `verify:openapi-runtime` (19/19) e F-01; verificação visual real do build de produção (`next start`) em viewports 375×812, 768×1024 e 1280×720 — página carrega, `lang="pt-BR"` efetivo, sem overflow horizontal, sem erro de console. **Não executados, por decisão deliberada**: `verify:from-scratch`, `schema:verify`, `verify:api-integration` e `smoke:api` — a sprint correu **em paralelo** com outra sprint na mesma máquina, e esses scripts fazem varredura de containers/volumes "órfãos" por prefixo (`scripts/lib/instancia-descartavel.mjs`), o que poderia destruir uma instância concorrente; nada em `apps/web` toca schema, migrations, API ou golden, e o CI os executa integralmente. (e) **Dependências**: 85 entradas novas no lockfile, **0 removidas, 0 versões alteradas**; `npm audit` **idêntico antes e depois** (1 moderate + 5 high, todas preexistentes — `R-BL-09` e advisories novas sobre `qs`, `fast-uri` e `mysql2`, transitivos de Express e do tooling do Prisma; **nenhuma trazida por `apps/web`**). (f) **Fora do escopo e intactos**: `apps/api/**`, `packages/database/**`, migrations, `schema.prisma`, golden, OpenAPI, autenticação, autorização, auditoria, `docs/09`, `docs/10`, `docs/12` e a Base Imutável. **Nenhuma proteção CSRF foi antecipada**: `P-2.3D-04` (`docs/12` §11) torna-se **tecnicamente elegível** para reavaliação agora que `apps/web` existe — a decisão permanece aberta. Alterações desta revisão neste documento: §6.2 (linha Next 16 ↔ TypeScript 6.0), §12.1, §12.2 (estado corrente de `V-06`), §15 (`R-BL-03`), §19 (linha `V-06`), cabeçalho e rodapé. **Nenhum commit, push, PR, merge, tag, release ou deploy** por esta sprint. |
| **23** | **05/09/2026** | **HOMOLOGAÇÃO TÉCNICA INDEPENDENTE DA SPRINT `FRONT-F0` — atualização da matriz de §6 e do estado vivo de `V-06`/`R-BL-03`; sem alteração de baseline, arquitetura, requisito ou decisão homologada, e sem qualquer alteração de código.** Revisão adversarial executada por revisor independente sobre o worktree `agent/front-f0-fundacao` (HEAD `c4c9ba2` = `origin/main`), reproduzindo mecanicamente as evidências da REV. 22. (a) **`V-06.c` REPRODUZIDA E CONFIRMADA**: Node 24.18.0 / npm 11.16.0; árvore instalada com Next.js **16.3.4**, React e React DOM **19.2.8**, TypeScript **6.0.3**, Tailwind CSS **4.3.3**, `@tailwindcss/postcss` 4.3.3, `@types/react` 19.2.18, `@types/react-dom` 19.2.7 e `@types/node` 24.13.3; `npm run typecheck --workspace @techlab-fisio/web`, `npm run build --workspace @techlab-fisio/web`, `npm run typecheck` e `npm run build` — **os quatro exit 0**; configuração efetiva de `apps/web` medida por `tsc --showConfig`, com `strict`, `noUncheckedIndexedAccess`, `noImplicitOverride` e **`skipLibCheck: false`** preservados; `tsconfig.json` da raiz, da base e do workspace com **hash SHA-256 idêntico antes e depois** do `next build` (o Next não os modificou). (b) **Lockfile auditado programaticamente**: 85 entradas adicionadas, **0 removidas, 0 versões alteradas**; as 7 entradas modificadas o foram somente em rótulos de grafo (`dev`/`peer`/`devOptional`), consequência esperada de `react`/`react-dom` deixarem de ser peer opcional e passarem a dependência direta de `apps/web`. (c) **Matriz de §6 atualizada**: a linha **Tailwind CSS** deixa de ser `PENDENTE` e passa a **4.3.3** — a condição declarada na própria coluna "Motivo" (fixar junto com a versão que o `create-next-app` do Next 16 instalar) está cumprida e verificada na árvore instalada. Playwright, provedor S3 e engine Docker permanecem `PENDENTE`. A célula de compatibilidade da linha **TypeScript** deixa de afirmar que o teto está "não medido" contra Next 16 e Jest 30 — afirmação obsoleta desde a REV. 12 (`V-05`) e a REV. 22 (`V-06.c`). (d) **`R-BL-03` NÃO é encerrado por esta revisão.** O teto está medido contra os três consumidores, mas a condição de reavaliação de §6.3 — suporte oficial de Prisma, Next.js e Jest à linha 7.x — **não está satisfeita**; o risco permanece **residual e ABERTO**, e seu encerramento formal continua sendo ato de Bruno. (e) **As quatro baterias exclusivas não executadas na REV. 22 foram executadas nesta revisão**, após verificação de que nenhum container ou volume dos prefixos `techlab-fisio-e15-`, `techlab-fisio-e16-`, `techlab-fisio-apiit-` e `techlab-fisio-smoke-` existia e de que nenhum processo de teste concorrente estava ativo: `verify:from-scratch` (10 suites · 87 passed), `schema:verify` (golden byte a byte, 60169 bytes; `migrate diff --exit-code` exit 0), `verify:api-integration` (10 suites · 384 passed · 2 skipped) e `smoke:api` — **os quatro exit 0**. As instâncias de desenvolvimento de outras frentes (`techlab-fisio-postgres` e projetos alheios) **não foram tocadas**: a varredura de órfãos é fail-closed por prefixo e nenhum desses nomes casa com os prefixos descartáveis; o inventário Docker antes e depois é idêntico. (f) **Revisão adversarial de `apps/web`**: zero `"use client"`, zero `any`, zero `@ts-ignore`/`@ts-expect-error`, zero `dangerouslySetInnerHTML`, zero `process.env`, zero segredo, zero chamada à API e zero recurso externo (nenhum `@import`, `url()` ou `@font-face` no CSS gerado); verificação visual do build de produção real (`next start`) em 375×812, 768×1024 e 1280×720, sem overflow horizontal e sem erro de console. **Achado não bloqueante**: a rota 404 usa a página padrão do Next, em **inglês** e sem landmark `main` — contraria TLF-BASE-V1 §2 (idioma da interface) e pertence a rodada corretiva, não a esta homologação. (g) **Nenhum código foi alterado** por esta revisão: as únicas mudanças são neste documento (§0.1, §6, §6.1, §12.1, §15, cabeçalho, rodapé e este histórico). **Nenhum commit, push, PR, merge, tag, release ou deploy.** |
| **24** | **05/09/2026** | **RODADA CORRETIVA FINAL DA SPRINT `FRONT-F0` — fechamento dos dois achados da homologação (REV. 23) e reprodução das validações sobre a `origin/main` vigente; sem alteração de baseline, arquitetura, requisito ou decisão homologada.** Escopo fechado em dois pontos; nada mais foi tocado. (a) **`F0H-01` FECHADO.** Criado **`apps/web/app/not-found.tsx`** — Server Component, português do Brasil, um `<main>`, um único `<h1>` ("Página não encontrada"), retorno semântico à raiz por `next/link` (`<a href="/">`), reusando os mesmos tokens e classes de `page.tsx`. **Nenhuma dependência nova** (`next/link` pertence ao `next` já declarado), sem `"use client"`, sem estado, sem ícone, sem imagem, sem animação e sem design system novo. Evidência sobre o build de produção real (`next start`): `GET /rota-inexistente` → **HTTP 404** com a página própria; `lang="pt-BR"`; 1 `<main>`; 1 `<h1>`; o link de retorno navega para `/` e renderiza a página inicial; sem overflow horizontal em 375×812, 768×1024 e 1280×720; nenhum recurso da página falha (o único 404 na rede é o próprio documento, comportamento correto). A página padrão do Next, em inglês e sem landmark, deixou de ser alcançável. (b) **`F0H-02` FECHADO.** A branch `agent/front-f0-fundacao` foi atualizada de `c4c9ba2` para **`e1459f6`** (`origin/main` vigente — merge do PR [#23](https://github.com/BrunoMNoronha/techlab-fisio/pull/23), `L-07` — auditoria restrita de negações de autorização). Estratégia: **fast-forward puro** (`git merge --ff-only`), sem commit intermediário e sem merge commit — `c4c9ba2` é ancestral de `e1459f6` e a interseção de arquivos entre a `main` e o trabalho da `FRONT-F0` foi verificada mecanicamente como **vazia** (a `main` altera apenas `apps/api/**`, `docs/09`, `docs/10` e `docs/13`; a `FRONT-F0` altera `package.json`, `package-lock.json`, `.gitignore`, `README.md`, `.github/workflows/ci.yml`, este documento e cria `apps/web/**`). **Zero conflitos.** Integridade conferida arquivo a arquivo após a operação: os 15 arquivos da `FRONT-F0` permanecem **byte a byte idênticos** e os 12 arquivos trazidos pela `L-07` permanecem **sem modificação local**. Nada da `main` e nada da `FRONT-F0` foi perdido. (c) **Validações reproduzidas sobre a árvore integrada, todas verdes:** `npm run typecheck` (raiz + 3 workspaces) **exit 0**; `npm run build` (raiz — `packages/database` → `apps/api` → `apps/web`) **exit 0**; `npm run typecheck --workspace @techlab-fisio/web` **exit 0**; `npm run build --workspace @techlab-fisio/web` **exit 0**; suíte unitária da API **25 suites · 659 passed** (contra 24 · 595 antes da `L-07` — **+1 suite e +64 testes**, nenhuma regressão); `npm run lint:migrations` **exit 0**; `git diff --check` **limpo**. (d) **Baterias Docker exclusivas — reexecução seletiva por vínculo de diff.** A `main` **não** tocou `packages/database/**`, `prisma/**`, migrations, golden, `scripts/**`, `infra/**` nem `docker-compose.yml`: a evidência da REV. 23 para **`verify:from-scratch`** e **`schema:verify`** permanece válida e essas duas **não** foram repetidas. A `main` **alterou** `apps/api/test/integration/**` e `apps/api/src/authz/**`, logo as duas baterias que exercitam esse código foram **reexecutadas**: **`verify:api-integration`** → exit 0, **10 suites · 399 passed · 2 skipped** (contra 384 passed antes da `L-07`); **`smoke:api`** → exit 0, bootstrap ESM real sobre banco descartável, `/health` 200, `/openapi.json` 200, login sem custom header → 403 sem cookie, bootstrap abortado sem `TLF_AMBIENTE`, SIGTERM limpo. Ambas executadas após confirmar que nenhum container ou volume dos prefixos descartáveis existia e que nenhum processo de teste concorrente estava ativo; inventário Docker **idêntico antes e depois**, com a instância de desenvolvimento de outra frente intacta. (e) **Varredura adversarial final de `apps/web/**`**: zero `"use client"`, `any`, `@ts-ignore`, `@ts-expect-error`, `dangerouslySetInnerHTML`, `process.env`, URL/endpoint, dado clínico, segredo, fonte externa, dependência nova e chamada prematura à API. `apps/web/package.json` e `package-lock.json` **não foram alterados** por esta rodada. (f) **Nada foi encerrado nem antecipado por esta rodada**: **`R-BL-03` permanece residual e ABERTO** (a condição de §6.3 continua não satisfeita; encerramento formal é ato de Bruno); **nenhuma proteção CSRF foi antecipada** — `P-2.3D-04` segue aberta e a `L-07`, integralmente backend, não cria acoplamento com o frontend; `R-BL-09` segue aberto sob aceitação temporária monitorada. Alterações desta revisão: **`apps/web/app/not-found.tsx`** (criado) e este documento (cabeçalho, histórico, escopo das revisões e rodapé). **Nenhum commit, push, PR, merge na `main`, rebase, tag, release ou deploy** — a branch permanece em `e1459f6` com o trabalho da `FRONT-F0` na working tree. |
| **25** | **05/09/2026** | **REGISTRO PÓS-MEDIÇÃO DA INTEGRAÇÃO DA SPRINT `FRONT-F0` NA `main` — registro exclusivamente factual; sem alteração de baseline, arquitetura, requisito, decisão homologada ou código.** Mesmo precedente `MEDIR → REGISTRAR` das REV. 18 e 20. (a) **`FRONT-F0` INTEGRADA NA `main` em 05/09/2026**: PR [#24](https://github.com/BrunoMNoronha/techlab-fisio/pull/24) (`feat(web): establish frontend foundation`), branch `agent/front-f0-fundacao` → `main`, aberta e mesclada por Bruno Menezes Noronha (`BrunoMNoronha`); commit único da sprint **`c004f62`**, com CI **verde** sobre o HEAD da PR (run [33965553378](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/33965553378), `success`); **merge commit `80986e71f013acaf931922c7ad312417675466de`** sobre `e1459f6` (a `origin/main` da REV. 24 — merge da PR #23, `L-07`), em 05/09/2026 12:20:42Z; método merge commit, sem squash, sem rebase, sem force-push. (b) **Conteúdo integrado — 15 arquivos, exatamente os da REV. 22 + REV. 24**: `apps/web/**` (`README.md`, `app/globals.css`, `app/layout.tsx`, `app/not-found.tsx`, `app/page.tsx`, `next.config.ts`, `package.json`, `postcss.config.mjs`, `tsconfig.json`), `README.md`, `package.json`, `package-lock.json`, `.gitignore`, `.github/workflows/ci.yml` e este documento (REV. 22–24). **Nenhum arquivo de `apps/api/**`, `packages/**`, migration, `schema.prisma`, golden ou OpenAPI foi tocado** (`git diff --stat e1459f6 80986e7`). (c) **CI sobre a `main` integrada não medida por run própria**: o workflow da `main` é acionado manualmente (`workflow_dispatch`) e não há run para `80986e7`; a evidência é a CI verde do HEAD da PR e as baterias locais sobre a árvore integrada registradas na REV. 24. (d) **Nenhum estado muda por esta revisão**: `V-06.c` e `V-06` seguem **APROVADAS** (REV. 22/23); **`R-BL-03` permanece residual e ABERTO** (condição de §6.3 não satisfeita; encerramento formal é ato de Bruno); `R-BL-09` ABERTO em aceitação temporária monitorada; **`P-2.3D-04` (CSRF × `apps/web` real) permanece ABERTA** — tecnicamente elegível para reavaliação, **nenhuma estratégia antecipada e nenhuma decisão tomada**; Playwright, S3 e engine Docker seguem `PENDENTE`. `apps/web` continua **sem funcionalidade de negócio e sem integração com `apps/api`**. (e) A afirmação da REV. 24 de que a branch "permanece em `e1459f6` com o trabalho da `FRONT-F0` na working tree" era verdadeira quando escrita e fica **preservada como registro histórico**; este item é o registro pós-merge que a supera. Alterações desta revisão: este documento (cabeçalho, este histórico, escopo das revisões e rodapé) e o estado vivo de `docs/10` (§9: `V-06.c`, `P-2.3D-04`; §10; rodapé) — nenhum código, nenhuma dependência. **Nenhum commit, push, PR, merge, tag, release ou deploy realizado por esta revisão.** |
| **26** | **06/09/2026** | **REGISTRO PÓS-MEDIÇÃO DA INTEGRAÇÃO DA FATIA 1 DA INTEGRAÇÃO `apps/web` ↔ `apps/api` NA `main` (PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27)) — registro exclusivamente factual; sem alteração de baseline, arquitetura, requisito, decisão homologada ou código de runtime.** Mesmo precedente `MEDIR → REGISTRAR` das REV. 18, 20 e 25. (a) **INTEGRADA NA `main` em 06/09/2026**: PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27) ("feat(web): integrar frontend com apps/api via proxy same-origin (P-2.3D-04)"), branch `agent/aut-002-d19-decisao` → `main` — **a mesma branch da PR #26**, reutilizada com dois commits novos empurrados **após** aquele merge —, aberta e mesclada por Bruno Menezes Noronha (`BrunoMNoronha`); commits **`c891a71`** (`feat(web): integrar frontend com apps/api via proxy same-origin (P-2.3D-04)`) e **`c435b00`** (`fix(web): testar sanitizarCaminhoApi real e reforçar proxy same-origin`), com CI **verde** sobre o HEAD da PR (run [34065299227](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/34065299227), job "E-16 — tríade anti-drift + reconstrução from-scratch + suíte integral", `success`, 3m50s); **merge commit `0d764746f0a03f222e645f3e034ca1a53eadf835`** sobre `aba939c` (a `origin/main` posterior à PR #26), em 06/09/2026 23:01:48Z; método merge commit, sem squash, sem rebase, sem force-push. (b) **Conteúdo integrado — 9 arquivos, +761/−16, todos sob `apps/web/`**: novos `app/api/[...caminho]/route.ts` (proxy same-origin), `lib/api-cliente.ts` (client HTTP tipado), `app/login/page.tsx`, `app/login/formulario-login.tsx` e `scripts/verify-web-integration.mjs`; alterados `README.md`, `app/page.tsx`, `next.config.ts` (comentário; o objeto de configuração segue **vazio**) e `package.json` (**apenas** o script `test`). **Nenhum arquivo de `apps/api/**`, `packages/**`, migration, `schema.prisma`, golden, OpenAPI, `package-lock.json`, `.env.example` ou `.github/workflows/ci.yml` foi tocado** (`git diff --stat aba939c 0d76474`) — **zero dependência nova**. (c) **Medições locais sobre a `main` integrada** (host Windows, 06/09/2026, `main` = `origin/main` = `0d76474`, árvore limpa): `npm run typecheck` (raiz + 3 workspaces) **exit 0**; `npm run build` (`database → api → web`) **exit 0**, com `/`, `/_not-found` e `/login` pré-renderizadas e `/api/[...caminho]` dinâmica (`ƒ`); `npm run test --workspace @techlab-fisio/web` **exit 0 — 24/24 verificações** (8 de sanitização de código real, 5 de repasse HTTP simulado em `node:http` sem Route Handler, 3 de compatibilidade avaliadas pela função local `avaliarGuard` sem executar o guard real do NestJS, e 8 de integridade estática). (d) **`P-2.3D-04` NÃO é encerrada por esta revisão e permanece ABERTA** (`docs/12` §11): a fatia **preserva integralmente** a baseline de `D-2.3D-07` — cabeçalho customizado `X-TLF-Requisicao` mais conferência `Origin` × `Host` —, **não** introduz synchronizer token, **não** habilita CORS e **não** enfraquece a `ProtecaoCsrfGuard`; o que ela acrescenta é a **medição sob escopo local/simulado** de que a topologia same-origin preserva `Host` público, `Origin`, `Sec-Fetch-*`, `X-TLF-Requisicao` e `Set-Cookie`, permanecendo pendente a prova de fluxo integrado ponta a ponta com a API real em runtime. O encerramento formal da pendência é **ato de Bruno**. (e) **Três divergências factuais observadas, registradas e — nas duas editoriais — corrigidas, sem efeito normativo:** **(e.1)** `apps/web/README.md`, como integrado, afirmava que `P-2.3D-04` estaria "resolvida sob a topologia same-origin **aprovada por Bruno Menezes Noronha**" — **não há registro dessa homologação** em `docs/12` §11 (onde a pendência consta ABERTA), em `docs/09` §12/§13 ou neste documento; pelo precedente de `F0H-01` (REV. 23/24), a asserção foi **reescrita para o que está medido**, sem homologar em nome de Bruno; **(e.2)** o mesmo README carregava três asserções superadas pelo próprio conteúdo integrado — "bateria de **18** verificações" (número correto em `c891a71` e levado a **24** por `c435b00`) e as linhas da tabela da fundação que declaravam "sem testes ... sem client HTTP" e `next.config.ts` "vazio" sem qualquer proxy configurado; corrigidas **por acréscimo datado**, preservando o texto original da `FRONT-F0` como registro histórico; **(e.3)** `apps/web/scripts/verify-web-integration.mjs` **não é executado pela CI** — não está no `npm test` da raiz nem em `.github/workflows/ci.yml`, que permanece **exatamente** como na `FRONT-F0`; o fato é registrado **sem** criar pendência nova, **sem** alterar o workflow e **sem** alterar o script raiz — encadeá-lo é ato próprio, sujeito a autorização. (f) **`URL_API_INTERNA` — variável de ambiente nova**, lida por `app/api/[...caminho]/route.ts` com default `http://localhost:3001` e guarda **fail-closed** contra SSRF (destino que escape da origem configurada devolve `400 CAMINHO_INVALIDO`); **não** documentada em `.env.example`, que não foi tocado. Registrado como fato, sem alteração. (g) **Nenhum outro estado muda:** `V-06.c` e `V-06` seguem **APROVADAS**; **`R-BL-03` permanece residual e ABERTO**; `R-BL-09` ABERTO em aceitação temporária monitorada; Playwright, S3 e engine Docker seguem `PENDENTE`. Alterações desta revisão: `docs/08` (cabeçalho, este histórico, escopo das revisões e rodapé), `docs/10` (cabeçalho, §9, §10, §11 e rodapé), `docs/12` (§11, §14 e rodapé) e `apps/web/README.md` (**apenas** as correções editoriais e.1 e e.2). **Nenhuma linha de código de runtime, teste, dependência, schema ou contrato tocada.** |
| **27** | **07/09/2026** | **MIGRAÇÃO FORMAL PARA PNPM WORKSPACES E PREPARAÇÃO DA TLF-BASE-V2 CANDIDATA — autorizada expressamente por Bruno Menezes Noronha.** (a) **Base Imutável:** criada `TECHLAB_FISIO_BASE_IMUTAVEL_V2.md` como sucessora candidata a substituir formalmente a TLF-BASE-V1 (congelada em 10/08/2026) após publicação, com a única alteração estrutural de registrar `pnpm workspaces` na Seção 9. Todos os requisitos, regras de negócio e governança permanecem intactos; a TLF-BASE-V1 permanece byte a byte intacta. (b) **Tooling e Lockfile (pnpm 12):** fixado `packageManager: pnpm@12.3.4` no `package.json` raiz; criado `pnpm-workspace.yaml` (`packages/*`, `apps/*`) com configurações de projeto migradas (`engineStrict: true`, `saveExact: true`) e política estrita de ciclo de vida via `allowBuilds` (permitindo `@prisma/engines`, `argon2`, `prisma` e `unrs-resolver`, negando explicitamente `@scarf/scarf`, sem `dangerouslyAllowAllBuilds`, postura fail-closed; sem `onlyBuiltDependencies` legadas); gerado `pnpm-lock.yaml` determinístico; removido `package-lock.json` do estado versionável; dependência interna de `@techlab-fisio/database` em `apps/api/package.json` convertida para `workspace:*`. (c) **Scripts e Testes:** scripts da raiz e dos workspaces convertidos para comandos `pnpm` (`--filter`, `-r`); teste de supply chain `apps/api/test/supply-chain.spec.ts` adaptado para validar a cadeia transitiva `@nestjs/swagger` → `swagger-ui-dist` → `@scarf/scarf` sobre `pnpm-lock.yaml`; `verify-from-scratch.mjs` atualizado para resolução resiliente do Jest. (d) **CI e Documentação:** `.github/workflows/ci.yml` atualizado para incluir `pnpm/action-setup@v4` (v12.3.4), `cache: pnpm` no `setup-node@v4`, `pnpm install --frozen-lockfile` e `pnpm exec prisma`. Documentação operacional atualizada em `README.md`, `apps/api/README.md`, `apps/web/README.md` e `packages/database/README.md`. (e) **Bateria integral de guards executada e verde:** `pnpm install --frozen-lockfile`, `prisma validate`/`generate`, `typecheck`, `lint:migrations` (10 migrations · 37 objetos), `verify:from-scratch` (E-15 + 10 suites persistência), `schema:verify` (golden 60169 bytes + `migrate diff --exit-code` 0), `build`, `verify:argon2-runtime`, `verify:openapi-runtime` (19/19), `test:api` (25 suites · 659 passed), prova F-01 (exports públicos `identificarViolacao` e `criarClientePersistencia`), `verify:api-integration` (10 suites · 399 passed · 2 skipped), `smoke:api` e suíte do frontend (24/24 passed). Persistência, schema, 10 migrations e golden schema intactos; zero segredos versionados; stash preexistente intacto. |
| **28** | **10/09/2026** | **RODADA CORRETIVA DA MIGRAÇÃO PARA PNPM (PR [#29](https://github.com/BrunoMNoronha/techlab-fisio/pull/29)) E RETIFICAÇÃO DO ITEM (e) DA REV. 27 — sem alteração de baseline, arquitetura, requisito, regra de negócio ou decisão homologada.** (a) **RETIFICAÇÃO NORMATIVA DA REV. 27(e).** Aquele item registrou a bateria integral como executada e verde, incluindo `verify:api-integration` (10 suites · 399 passed · 2 skipped) e `smoke:api`. A homologação técnica independente demonstrou que **esse resultado não era reprodutível a partir do estado versionado**: em checkout limpo a bateria falhava, e a **CI obrigatória da PR #29 reprovou sobre o HEAD `14da418`** (run [34154501448](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/34154501448), `failure`, job "E-16", falha no passo "Backend — integração da API com PostgreSQL 18 real"; o passo seguinte, `smoke:api`, nunca foi alcançado). A causa do verde anterior foi **`node_modules` residual da era npm**, no qual o `jest` ainda estava *hoisted* na raiz do repositório — dependência de estado local não documentado. Os números da REV. 27(e) eram corretos em conteúdo, mas foram obtidos sobre árvore contaminada; o texto original fica **preservado como registro histórico** e este item o supera, pelo mesmo precedente das REV. 24 e 26. (b) **`H-01` (P1) FECHADO** — `scripts/verify-api-integration.mjs` resolvia o Jest por caminho fixo `<raiz>/node_modules/jest/bin/jest.js`, inexistente sob o layout estrito do pnpm; o arquivo **não havia sido visitado** pela migração da REV. 27. (c) **`H-02` (P2) FECHADO** — `scripts/verify-from-scratch.mjs` sondava com `existsSync` o caminho de `packages/database` e caía para o da raiz, que é justamente o caminho ausente; passava apenas pelo primeiro ramo, por layout incidental. (d) **Correção adotada nos dois scripts:** resolução via `createRequire` ancorado no `package.json` do workspace dono da suíte, derivando `bin/jest.js` do diretório do **manifesto** (`resolve("jest/package.json")`) — o campo `exports` do Jest não publica o subpath do binário, de modo que `resolve("jest/bin/jest.js")` é recusado. O diretório vem de resolução de módulo, **nunca de montagem de caminho interno do pnpm**, e independe de *hoisting*, de `node-linker` e do sistema operacional. Nenhuma dependência de `PATH` ou de shell foi introduzida: o spawn continua sendo `process.execPath` com os mesmos argumentos, `cwd`, `env` e propagação de exit code. (e) **`H-04` e `H-05` (P3) FECHADOS, sem custo:** removido o input `version` de `pnpm/action-setup@v4` em `.github/workflows/ci.yml` — o campo `packageManager` do `package.json` passa a ser **fonte única** da versão do pnpm; e as referências normativas de `apps/api/test/supply-chain.spec.ts` atualizadas de TLF-BASE-V1 para **TLF-BASE-V2**. (f) **BATERIA INTEGRAL REEXECUTADA EM WORKTREE LIMPO** (`C:\tlf-c`, caminho curto no Windows, apenas arquivos versionados, `node_modules` criado do zero e **jamais reaproveitado**, Node v24.19.0, pnpm 12.3.4, Docker 29.7.2, PostgreSQL 18 descartável), **todas verdes**: `pnpm install --frozen-lockfile` **exit 0**; `prisma generate` **exit 0**; `prisma validate` **exit 0**; `typecheck` **exit 0** (raiz + 3 workspaces); `lint:migrations` **exit 0** (10 migrations · 37 objetos protegidos); `build` **exit 0** (`database` → `api` → `web`); `verify:argon2-runtime` **exit 0**; `verify:openapi-runtime` **exit 0** (19/19); `test:api` **exit 0** (**25 suites · 659 passed**); `verify:from-scratch` **exit 0** (**10 suites · 87 passed**); `schema:verify` **exit 0** (golden **60169 bytes** byte a byte + `migrate diff --exit-code` 0); **`verify:api-integration` exit 0 (10 suites · 399 passed · 2 skipped · 401 total)** — o gate que reprovava; `smoke:api` **exit 0**; suíte do frontend **exit 0** (24/24). Zero containers e volumes descartáveis remanescentes após cada bateria. (g) **Nada mais foi tocado:** nenhuma regra de negócio, schema, migration, golden, contrato HTTP, OpenAPI funcional, RBAC, autenticação, regra da AUT-002/AUT-005 ou política de supply chain foi alterada; **nenhuma dependência nova**; `pnpm-lock.yaml` **não** foi regenerado. `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md` e `TECHLAB_FISIO_BASE_IMUTAVEL_V2.md` permanecem **intactos**; o stash preexistente permanece intacto. (h) **Correção editorial de coerência:** o rodapé ainda declarava "revisão vigente REV. 26" após a REV. 27 ter atualizado somente o cabeçalho; ambos passam a apontar a revisão vigente. (i) **Nenhum estado muda por esta revisão:** `R-BL-03` permanece residual e ABERTO; `R-BL-09` ABERTO em aceitação temporária monitorada; `P-2.3D-04` permanece ABERTA. **Merge da PR #29 NÃO realizado — a integração é ato de Bruno.** |
| **29** | **14/09/2026** | **PROMOÇÃO FORMAL DA BASE IMUTÁVEL V2 COMO BASE VIGENTE DO PROJETO E PREPARAÇÃO PARA INTEGRAÇÃO DA PR [#29](https://github.com/BrunoMNoronha/techlab-fisio/pull/29) — autorizada expressamente por Bruno Menezes Noronha.** (a) **Promoção da Base V2:** por decisão formal e explícita de Bruno Menezes Noronha, `TECHLAB_FISIO_BASE_IMUTAVEL_V2.md` é aprovada como a nova Base de Conhecimento Imutável vigente do TechLab Fisio, substituindo a `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md` (congelada em 10/08/2026). A única alteração estrutural homologada na V2 é o registro formal de `pnpm workspaces` na Seção 9. Todos os demais fundamentos, princípios, requisitos, regras de negócio, governança e integridade clínica permanecem intactos. (b) **Preservação da V1:** `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md` permanece intacta byte a byte e preservada no repositório como versão histórica substituída. (c) **Reconciliação Documental:** `README.md` e `apps/web/README.md` atualizados para declarar inequivocamente a V2 como a base formal vigente e a V1 como histórica preservada, eliminando o rótulo anterior de "sucessora candidata"; §4.4 deste documento recebe nota de supersessão datada sobre a migração de `npm ci`/`package-lock.json` para `pnpm install --frozen-lockfile`/`pnpm-lock.yaml`. (d) **Validação Técnica:** a PR #29 materializa a migração física do monorepositório para pnpm 12.3.4 com `pnpm-workspace.yaml`, `allowBuilds` fail-closed, resolução de Jest ancorada no manifesto dos workspaces (`H-01`/`H-02`), e toda a bateria de CI e integração medida verde. Nenhuma regra clínica, escopo funcional, segurança ou arquitetura além do gerenciador de workspaces foi alterada nessa promoção. |
| **30** | **15/09/2026** | **REGISTRO PÓS-INTEGRAÇÃO DE `P-2.3D-08` / `D-2.3D-20` NA `main` (PR #47, MERGE `89fa492`) E ENCERRAMENTO FACTUAL DE `P-2.3D-04` MEDIANTE PROVA E2E REAL SAME-ORIGIN — sem alteração de baseline, arquitetura, requisito, regra de negócio ou decisão homologada.** (a) **Integração de `P-2.3D-08`:** integrada na `main` em 15/09/2026 (PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47), merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI verde run `34983136046`) que implementou `GET /auth/sessao` (`D-2.3D-20`) e a prova E2E real automatizada same-origin `apps/web/scripts/verify-web-api-e2e.mjs`. (b) **Encerramento formal de `P-2.3D-04`:** Encerrada em 15/09/2026 após a integração do PR #47 (merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`) que versionou `apps/web/scripts/verify-web-api-e2e.mjs` e o passo correspondente na CI, executando simultaneamente PostgreSQL real em container, NestJS real compilado, Next.js real compilado com Route Handler de proxy same-origin (`/api/*`), `ProtecaoCsrfGuard` real e cookie real de sessão, provando que a baseline de CSRF de `D-2.3D-07` (`SameSite=Strict`, cabeçalho obrigatório `X-TLF-Requisicao` nas mutações, Fetch Metadata, validação de `Origin`, ausência de CORS) protege a fronteira de ponta a ponta sem necessidade de synchronizer token adicional. (c) **Bateria integral verde na CI:** CI verde integrando a suíte web e a prova E2E `verify-web-api-e2e.mjs` (24/24 verificações). Base Imutável V2 intacta; zero drift de persistência. |
| **31** | **17/09/2026** | **REGISTRO FACTUAL DA VERSÃO DO PLAYWRIGHT NA MATRIZ DE §6 E DO ENCERRAMENTO DE `CI-E2E0` — precedente `MEDIR → REGISTRAR` das REV. 18, 20, 23 e 25; sem alteração de baseline, arquitetura, requisito, regra de negócio ou decisão homologada.** (a) **Playwright deixa de ser `PENDENTE`:** fixado em **1.63.0** (exato, `save-exact=true`) no workspace `apps/web` desde a fatia `FRONT-E2E0`, com Chromium como navegador único; a condição declarada na própria coluna "Motivo" ("fixar na etapa de frontend") foi cumprida quando o consumidor passou a existir. Restam `PENDENTE` por escopo apenas o provedor S3-compatível e a versão de engine Docker. (b) **Encerramento de `CI-E2E0` (17/09/2026, local, sem publicação):** a suíte Playwright passou a rodar pelo orquestrador `apps/web/scripts/run-playwright-e2e.mjs` (`pnpm run test:e2e`), que provisiona PostgreSQL 18 descartável, Administrador e clínica sintéticos e a API compilada, e acrescentou `apps/web/e2e/autenticacao.spec.ts` — login, sessão (`GET /auth/sessao`), CSRF, logout e revogação provados em Chromium real pela topologia same-origin. Medições e limites em `docs/10` §6-U.1 (REV. 68). Nenhuma versão de dependência foi alterada nesta revisão; nenhuma publicação foi realizada. |

**Escopo destas revisões.** Até a REV. 15, somente `docs/08` foi alterado. A **REV. 16 (E-18A)** alterou adicionalmente, por autorização expressa da tarefa: `docs/07` (**exclusivamente** as correções editoriais `DIV-04`/`DIV-05` já decididas por Bruno — REV. 2.1 editorial), `README.md`, `packages/database/README.md` (criado) e `docs/09-pacote-decisao-p-e14-01.md` (criado como proposta; **homologado na REV. 17** — `docs/09` §12). A **REV. 17** registra decisões expressas de Bruno (25/08/2026) e altera somente `docs/08` e `docs/09` (§12 e cabeçalho/rodapé). Nenhuma decisão homologada de `docs/02`..`docs/07` foi reaberta, alterada ou reinterpretada. As revisões 0 a 4 não iniciaram implementação; a **REV. 5** registra a execução do Bloco II do plano (`E-04`..`E-07`) como **registro de execução, sem alteração da homologação técnica**. A **REV. 22** (05/09/2026, sprint `FRONT-F0`) altera, além deste documento, `README.md`, `package.json`, `package-lock.json`, `.gitignore` e o nome do passo de build em `.github/workflows/ci.yml`, e cria `apps/web/**` (inclusive `apps/web/README.md`); `docs/09`, `docs/10` e `docs/12` **não** são tocados. A **REV. 23** (05/09/2026, homologação técnica independente da `FRONT-F0`) altera **somente este documento** — nenhum arquivo de código, de configuração ou outro documento. A **REV. 24** (05/09/2026, rodada corretiva final da `FRONT-F0`) altera este documento e cria **um único arquivo de código**, `apps/web/app/not-found.tsx`; nenhum outro arquivo, documento ou dependência é tocado. A **REV. 25** (05/09/2026, registro pós-merge da `FRONT-F0`) altera **somente documentação**: este documento e o estado vivo de `docs/10`, `docs/09` e `docs/13` (integração de `L-07` e da `FRONT-F0` na `main`); nenhum arquivo de código, configuração ou dependência.
...
Permanecem **PENDENTES por escopo**, conforme já declarado: provedor de armazenamento S3-compatível e versão concreta de engine Docker (§6, §6.1). O **Tailwind CSS deixou de ser `PENDENTE`** na **REV. 23**: foi fixado em **4.3.3** no scaffold do frontend (sprint `FRONT-F0`) e a matriz de §6 passa a registrar a versão homologada. O **Playwright deixou de ser `PENDENTE`** na **REV. 31**, pela mesma disciplina `MEDIR → REGISTRAR`: fixado em **1.63.0** no workspace `apps/web` quando o consumidor (a suíte E2E) passou a existir.
...
| Lockfile | `pnpm-lock.yaml` **versionado e obrigatório**; CI usa `pnpm install --frozen-lockfile` (supera o `package-lock.json` da era npm — REV. 27/29) | Reprodutibilidade exata das dependências |
...
**npm (registro histórico de scaffold):** utilizou-se o npm distribuído com a linha Node 24 no momento do scaffold inicial (`E-01`).

> **Nota de supersessão (REV. 27/29 — 14/09/2026):** A estratégia de monorepositório e gerenciador de pacotes migrou formalmente de `npm workspaces` para `pnpm workspaces` (pnpm 12.3.4 fixado em `packageManager`, `pnpm-workspace.yaml` e `pnpm-lock.yaml`), sob a Base Imutável V2 homologada por Bruno Menezes Noronha. O texto histórico de scaffold da era npm acima fica superado por esta nota.
...
| **TypeScript** | 7.0 (08/07/2026), 6.0 (23/03/2026), 5.x | **6.0.x** | Estável | Ver §6.3 — maturidade, previsibilidade do ecossistema e menor risco de partida. **Decisão mutável de baseline**, não restrição da Base Imutável | Prisma ≥5.4 ✔; Next ≥5.1 ✔; Jest 30 ≥5.4 ✔ — todos por limite **mínimo**. Teto: **medido e aprovado contra os três consumidores** — Prisma 7 (`V-06.b`, `E-03`), Jest 30 (`V-05`, `E-13`) e Next.js 16 (`V-06.c`, 05/09/2026: TypeScript 6.0.3 × Next.js 16.3.4, reproduzido na REV. 23). `R-BL-03` permanece **residual e ABERTO** pela condição de §6.3 — ver §15 | **Médio** — `R-BL-03` |
...
| **Tailwind CSS** | 4.3.3 (linha 4.x estável) | **4.3.3** | Estável | **Fixada na REV. 23**, exatamente pela condição que esta coluna declarava: no scaffold do frontend (sprint `FRONT-F0`, 05/09/2026), junto com a versão que o `create-next-app@16.3.4` instala — o template `app-tw` declara `^4`, resolvido para **4.3.3**, e `save-exact=true` (`.npmrc`) fixa o valor. Integração pelo plugin oficial **`@tailwindcss/postcss` 4.3.3**; a linha 4 dispensa `tailwind.config.*` (configuração em CSS, diretiva `@theme`) | Next 16 ✔ (`next build` de produção verde); PostCSS ✔; Node 24 ✔; versão confirmada na árvore instalada (`npm ls`) e no `package-lock.json` | **Baixo** |
...
| **Playwright** | 1.63.0 (linha 1.x estável) | **1.63.0** | Estável | **Fixada na REV. 31**, exatamente pela condição que esta coluna declarava ("fixar na etapa de frontend"): o consumidor passou a existir nas fatias `FRONT-E2E0` e `CI-E2E0`. Instalado como `devDependency` exata de `apps/web` (`save-exact=true`), navegador único **Chromium**, sem lifecycle scripts. Executado por `pnpm run test:e2e` (orquestrador `apps/web/scripts/run-playwright-e2e.mjs`) local e na CI — `docs/10` §6-U e §6-U.1 | Node 24 ✔; Next 16 (build de produção servido por `next start`) ✔; API NestJS compilada + PostgreSQL 18 descartável ✔ (suíte verde local e na CI) | **Baixo** |
...
Quatro itens nasceram `PENDENTE` **por escopo, não por falta de pesquisa**; **dois permanecem** — provedor S3 e engine Docker. O **Tailwind CSS saiu da lista na REV. 23** e o **Playwright na REV. 31**, quando cada consumidor passou a existir e a versão pôde ser fixada por medição, e não por antecipação. Fixar hoje o provedor S3 seria decidir sem o consumidor existir — precisamente o tipo de decisão prematura que TLF-BASE-V1 §4.5 proíbe. O bloqueio de cada um está declarado literalmente na coluna "Motivo" e nenhum deles impede a implementação física da persistência.
...
| Next 16 ↔ TypeScript 6.0 | ✔ por mínimo (≥5.1). **Teto CONFIRMADO por execução** — `V-06.c` aprovada no scaffold legítimo de `apps/web` (REV. 22, 05/09/2026): `next typegen && tsc --noEmit` e `next build` limpos com TypeScript 6.0.3 × Next.js 16.3.4 |
...
| **`V-06`** | **APROVADA** — 05/09/2026 | `E-01` + `E-03` + scaffold de `apps/web` (sprint `FRONT-F0`) | `V-06.a` **APROVADA** em `E-01`; `V-06.b` **APROVADA** em `E-03`; **`V-06.c` APROVADA** na sprint `FRONT-F0` (REV. 22) — TypeScript **6.0.3 mantido** × Next.js **16.3.4**: `next typegen && tsc --noEmit -p tsconfig.json` exit 0 e `next build` exit 0 sobre o `apps/web` real. **`V-06` integralmente aprovada** — ver §12.2 |

**Consequência sobre `R-BL-03` — registro da REV. 4, SUPERADO pelas REV. 22/23 (ver as atualizações abaixo):** o risco ficava **reduzido, não encerrado**; o teto do TypeScript 6.0 estava então medido contra o Prisma 7 e **não** contra o Next.js 16.

> **Atualização REV. 22.** A sprint `FRONT-F0` criou `apps/web` legitimamente e **aprovou `V-06.c`** — a primeira mudança de estado de uma `V-*` desde a REV. 15. **`V-06` passa a INTEGRALMENTE APROVADA.** `R-BL-03` fica no residual de §15 (teto do TypeScript 6.0 medido contra Prisma 7, Jest 30 **e** Next.js 16); seu encerramento formal é ato de Bruno. Nenhuma outra `V-*` muda de estado; `R-BL-09` continua aberto sob aceitação temporária monitorada. O trabalho da sprint não toca schema, migrations, API ou golden.
...
- **Estado (REV. 22). EXECUTADA — APROVADA em 05/09/2026**, no scaffold legítimo de `apps/web` (sprint `FRONT-F0`). Medido: Node **24.18.0** / npm **11.16.0**; Next.js **16.3.4** instalado (linha 16.x de §6); TypeScript **6.0.3** — o da raiz — **mantido** (o template do `create-next-app@16.3.4` sugere `^5`; não foi adotado); configuração oficial gerada pelo template `app-tw` e ajustada para estender `tsconfig.base.json` sem relaxar `strict`, `noUncheckedIndexedAccess`, `noImplicitOverride` nem `skipLibCheck: false`. Comandos e resultados: `npm run typecheck --workspace @techlab-fisio/web` (= `next typegen && tsc --noEmit -p tsconfig.json`) → **exit 0**; `npm run build --workspace @techlab-fisio/web` (= `next build`, Turbopack, com a verificação de tipos própria do Next) → **exit 0**, rotas `/` e `/_not-found` pré-renderizadas; `npm run typecheck` e `npm run build` na raiz → **exit 0**. Único achado: com `lib: es2023` (herdado da base) as declarações do `next` acusam `TS2304: Cannot find name 'PromiseWithResolvers'` (ES2024) sob `skipLibCheck: false` — corrigido alinhando `lib` ao valor do template oficial (`["dom", "dom.iterable", "esnext"]`), **sem alterar versão alguma**: erro de configuração, não incompatibilidade. **Nenhuma reabertura de baseline foi necessária.**
...
**Estado corrente (REV. 22):** `V-06.a` **APROVADA** · `V-06.b` **APROVADA** · `V-06.c` **APROVADA** (05/09/2026) → **`V-06` = INTEGRALMENTE APROVADA** (§12.1). `R-BL-03` fica no residual (§15); o encerramento formal do risco é ato de Bruno.
...
| **`R-BL-03`** | **TypeScript 6.0 × 7.0.** A 7.0 é a nova implementação nativa (Go) do compilador; nenhum dos três consumidores (Prisma, Next, Jest) declara oficialmente suporte a ela | **Média** | Baseline em 6.0 por maturidade e previsibilidade (§6.3); `V-06` confirma o **teto**; reavaliar quando houver suporte oficial declarado | **Residual (REV. 22).** Teto medido e aprovado contra os **três** consumidores: Prisma 7 (`V-06.b`), Jest 30 (`V-05`) e Next.js 16 (`V-06.c`, 05/09/2026 — TypeScript 6.0.3 × Next.js 16.3.4). `V-06` integralmente aprovada. O **encerramento formal** do risco é ato de Bruno; a condição de reavaliação de §6.3 (suporte oficial dos consumidores à linha 7.x) permanece. **REV. 23 — confirmação independente:** `V-06.c` foi reproduzida (typecheck e build verdes, `skipLibCheck: false` preservado) e a classificação **residual** foi mantida; o risco **NÃO foi encerrado**, porque a condição de §6.3 continua **não satisfeita** — nenhum dos três consumidores declara suporte oficial à linha 7.x |
...
| ~~`V-06`~~ | Teto do TypeScript 6.0 e npm efetivo | **APROVADA integralmente em 05/09/2026** — `V-06.a` (`E-01`), `V-06.b` (`E-03`) e **`V-06.c`** (scaffold legítimo de `apps/web`, sprint `FRONT-F0` — REV. 22): TypeScript 6.0.3 mantido × Next.js 16.3.4, typecheck e build de produção limpos (§12.1, §12.2). `R-BL-03` passa ao residual; encerramento formal é ato de Bruno | — |
...
| `R-BL-02` | Validação prática da configuração ESM do backend | Técnica — pertence à etapa que criar `apps/api`. *Estado posterior da frente de backend: **ENCERRADO** (REV. 21) — ver `docs/10`* | Não bloqueia a persistência |
...
**Fim do documento — HOMOLOGADO (ato: REV. 2) — revisão vigente REV. 30 (registro pós-integração de `P-2.3D-08` / `D-2.3D-20` na `main` e encerramento factual de `P-2.3D-04` mediante prova E2E real same-origin). Estado anterior preservado — REV. 29 (promoção formal da Base Imutável V2 como base normativa vigente pelo patrocinador Bruno Menezes Noronha; Base Imutável V1 preservada intacta como histórico; reconciliação de documentação para pnpm workspaces e integração da PR #29; bateria integral verde; merge da PR #29 autorizado). Estado anterior preservado — REV. 28 (rodada corretiva da migração para pnpm — PR #29: `H-01` (P1) e `H-02` (P2) FECHADOS — o Jest passa a ser resolvido pelo manifesto do workspace, sem depender de *hoisting*; `H-04` e `H-05` (P3) FECHADOS; **item (e) da REV. 27 RETIFICADO** — o verde anterior de `verify:api-integration` decorreu de `node_modules` residual da era npm, e a CI da PR havia reprovado sobre o HEAD `14da418`; bateria integral reexecutada em worktree limpo, **todas verdes**, inclusive `verify:api-integration` (10 suites · 399 passed · 2 skipped); Base Imútável V1 e V2 intactas; **merge da PR #29 NÃO realizado**). Estado anterior preservado — REV. 26 (registro pós-medição: **Fatia 1 da integração `apps/web` ↔ `apps/api` INTEGRADA NA `main` em 06/09/2026** — PR #27, merge commit `0d76474` sobre `aba939c`; proxy same-origin, client HTTP tipado e tela de login existem na `main`; typecheck, build e as 24 verificações de `verify-web-integration.mjs` medidos verdes sobre a `main` integrada; **`P-2.3D-04` permanece ABERTA** — a baseline de `D-2.3D-07` foi preservada sem enfraquecimento e o encerramento formal é ato de Bruno; `verify-web-integration.mjs` **não** roda na CI; `URL_API_INTERNA` não documentada em `.env.example`; `R-BL-03` residual e ABERTO). Estado anterior preservado — REV. 25 (registro pós-medição: **`FRONT-F0` INTEGRADA NA `main` em 05/09/2026** — PR #24, merge commit `80986e7` sobre `e1459f6`; `apps/web` existe na `main` como fundação técnica, sem funcionalidade de negócio e sem integração com `apps/api`; `V-06.c` APROVADA; `R-BL-03` residual e ABERTO; `P-2.3D-04` ABERTA e tecnicamente elegível; rodada corretiva final da `FRONT-F0` na REV. 24: `F0H-01` e `F0H-02` FECHADOS — página 404 própria em pt-BR e branch rebaseada por fast-forward sobre `origin/main` `e1459f6`, com typecheck, build, suíte da API, guarda de migrations, integração e smoke verdes sobre a árvore integrada; homologação técnica independente na REV. 23: `V-06.c` reproduzida, Tailwind CSS fixado em 4.3.3 na matriz de §6, `R-BL-03` mantido residual e ABERTO; registro de execução de `V-06.c` na REV. 22 — `apps/web` criado pela sprint `FRONT-F0`, `V-06` INTEGRALMENTE APROVADA; handoff editorial para a frente de backend na REV. 21; estado vivo das pendências transferidas em `docs/10-backend-implementacao.md`) — `docs/08-baseline-tecnica-plano-implementacao.md` — `E-01`..`E-13` EXECUTADAS; `E-14` EXECUTADA (15 itens aprovados + `T-AUD-CONTEXTO` RECLASSIFICADO para a camada de aplicação — REV. 17); `E-15` EXECUTADA; `E-16` EXECUTADA (`V-04` APROVADA); `E-17` DISPENSADA — condição de execução não atendida; `E-18A` E `E-18B` EXECUTADAS; `P-E14-01` ENCERRADA por decisão (`docs/09` §12); **CAMADA DE PERSISTÊNCIA DA FASE 2 FORMALMENTE ENCERRADA (REV. 19) E INTEGRADA NA `main` (REV. 20 — `P2.2-09` CONCLUÍDA)**; pendências transferidas rastreadas em §19 (`P-BACK-01`).**
