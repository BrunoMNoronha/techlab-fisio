...
| Item | Estado |
| --- | --- |
| Implementação de PRO-003 (rotas, regra de versões, migration de `D-PRO3-06`) | **IMPLEMENTADA E PUBLICADA NA `main`** em 17/09/2026 (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§5.3; `docs/10` §6-AG). *(Correção factual: até a REV. 5 esta linha dizia "NÃO PUBLICADA".)* |
| Consumo da regra pela agenda (`D-PRO3-04`) | **CONSUMIDA** — `VerificadorDisponibilidadeProfissional` é provider do `AgendaModule` e é chamado no passo 9 de `D-AGD-04`, na criação e na remarcação de agendamento (`docs/10` §6-AI). *(Correção factual: até a REV. 5 esta linha registrava "SEM CONSUMIDOR", o que era verdade enquanto AGD-A não existia. A regra NÃO foi alterada — só passou a ser chamada.)* |
...
| `D-PRO3-04` | Função pura `avaliarDisponibilidadeProfissional` + adaptador transacional `VerificadorDisponibilidadeProfissional`, sem lock, delegando a contenção à função de CFG-002. *(Estado na REV. 5: "não integrada a fluxo algum — a agenda não existe". **Superado na REV. 6**: AGD-A a integra no passo 9 de `D-AGD-04`, sem alterar uma linha da regra.)* |
...
A matriz `TD-01`..`TD-13` de §6 foi coberta integralmente; o detalhamento das provas, das baterias medidas, dos mutation challenges e das limitações está em `docs/10` §6-AG. **Limite declarado:** `TD-12` é provado de forma proporcional ao estado real, porque AGD-A ainda não existe (`docs/10` §6-AG.5).
...
| REV. | Data | Descrição |
| --- | --- | --- |
| **6** | 17/09/2026 | **Atualização exclusivamente factual** após a implementação de AGD-A (`docs/15` §8, `docs/10` §6-AI): a regra de `D-PRO3-04` **deixou de estar sem consumidor** — é chamada no passo 9 de `D-AGD-04` — e PRO-003 deixou de estar "não publicada" (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`). Atualizadas §5.2 e a linha de `D-PRO3-04` em §5.3, com registro do que valia antes. **Nenhuma decisão `D-PRO3-*` criada, alterada ou reaberta; nenhuma linha da regra ou do verificador alterada**; §4 e §6 preservados. |
