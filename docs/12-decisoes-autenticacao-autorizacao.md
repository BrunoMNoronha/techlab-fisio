...
| **F7** | Consolidação, revisão independente e rito de integração | todas | **EM CURSO** — revisão independente executada em 05/09/2026 (`docs/10` §6-Q); **encerramento da Etapa BLOQUEADO** pelo conflito de fontes `Q-01` (§11) |
...
| `Q-01` | **Conflito entre fontes sobre a auditoria e a autoria da revogação em massa da T-07** — `docs/09` §5 atribui `usuario.sessao.revogacao` à revogação “em massa **via T-07**”, com ator **Administrador**, enquanto a linha seguinte audita a mesma transação como `usuario.senha.recuperacao_concluida`, com ator **Usuário**; e `docs/06` §12.7 define T-07 **exclusivamente** como “Recuperação de senha / revogação”. Na conclusão **não há Administrador autenticado**. Decorrem daí duas questões abertas: se a T-07 deve emitir também `usuario.sessao.revogacao` (`L-F6-07`) e qual ator alimenta `revogada_por_usuario_id` (`L-F6-06`) | **ABERTA — BLOQUEANTE do encerramento da Etapa 2.3D.** Levantada pela revisão independente da F7 em 05/09/2026 (`docs/10` §6-Q.2). Exige **decisão expressa de Bruno**: nenhuma das leituras pode ser adotada por interpretação de agente (TLF-BASE-V1 §15). Comportamento vigente **preservado** |
...
| **AUT-002** — Encerrar, expirar e revogar sessões | `D-2.3D-01` (janela física), `D-2.3D-04` (expiração absoluta e ociosa), `D-2.3D-05` (estados terminais) | **PARCIAL.** Estados e transições preservados **sem alteração**: `ATIVA -> EXPIRADA`, `ATIVA -> REVOGADA`. Cobertos: logout, expiração e revogação em massa da T-07. **NÃO implementados:** revogação **administrativa** de terceiro (`sessoes.revogar_terceiro`) e inativação de usuário (AUT-005) |
...
| REV. | Data | Alterações |
| --- | --- | --- |
| **9** | 05/09/2026 | **Atualização FACTUAL da revisão independente da F7 — NENHUMA decisão normativa criada, alterada, renumerada ou reaberta.** `D-2.3D-01`..`D-2.3D-17` permanecem exatamente como homologadas. **Nenhuma `D-2.3D-18` foi criada:** a promoção de `L-F6-06` chegou a ser redigida nesta fatia e foi **retirada** depois que a revisão da PR [#22](https://github.com/BrunoMNoronha/techlab-fisio/pull/22) demonstrou que seu fundamento não se sustenta — o critério adotado (“o campo registra o ator que `docs/09` §5 fixa para o evento correspondente”) depende de qual linha de `docs/09` §5 é a correspondente, e as duas candidatas divergem no ator. **(a) Nova pendência `Q-01` em §11, BLOQUEANTE do encerramento da Etapa:** `docs/09` §5 atribui `usuario.sessao.revogacao` à revogação “em massa via T-07” com ator **Administrador**, enquanto a linha seguinte audita a mesma transação como `usuario.senha.recuperacao_concluida` com ator **Usuário**, e `docs/06` §12.7 define T-07 **exclusivamente** como recuperação de senha — sendo que na conclusão não existe Administrador autenticado. `L-F6-06` e `L-F6-07` **permanecem abertas** e dependem dessa decisão. **(b) `P-2.3D-03` ENCERRADA** em §11 — medida e verde desde a F3 e provada a cada CI por `verify:openapi-runtime` (19/19); **`D-2.3D-11` não foi alterada**. **(c) Correções factuais:** a linha da fatia **F7** em §9 passa a **EM CURSO**, com o encerramento bloqueado por `Q-01`; a linha de **AUT-002** em §12 passa a **PARCIAL**, nomeando o que está coberto (logout, expiração, revogação em massa da T-07) e o que não está (revogação administrativa de terceiro e inativação). **A Etapa 2.3D NÃO é declarada encerrada.** AUT-005 permanece NÃO MATERIALIZADA; `P2.2-05` permanece NÃO INICIADA. |
...
**Fim — `docs/12-decisoes-autenticacao-autorizacao.md` — `D-2.3D-01`..`D-2.3D-17` registradas; F0 da Etapa 2.3D-B executada em 27/08/2026; `D-2.3D-13` homologada em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` homologadas em 31/08/2026; `D-2.3D-16` e `D-2.3D-17` homologadas em 05/09/2026; F1..F6 CONCLUÍDAS, F7 EM CURSO — Etapa 2.3D NÃO ENCERRADA, aguardando decisão de Bruno sobre `Q-01` (§11).**
