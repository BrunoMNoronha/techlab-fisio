...
> **Data:** 27 de agosto de 2026 (REV. 3 a REV. 5 em 28/08/2026; REV. 6 em 31/08/2026; REV. 7 em 02/09/2026)
...
| Campo | Valor |
| --- | --- |
| Identificadores | `D-2.3D-01` .. `D-2.3D-18` |
| Autoridade | Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1) |
| Data da homologação | 27/08/2026 (`D-2.3D-01`..`D-2.3D-12`) · 28/08/2026 (`D-2.3D-13`) · 31/08/2026 (`D-2.3D-14` e `D-2.3D-15`) · 05/09/2026 (`D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18`) |
...
| Efeito físico na F0 | **exclusivamente `D-2.3D-01`**; as demais são normativas e produzem efeito nas fatias F1..F7 (`D-2.3D-13` foi acrescentada em 28/08/2026; `D-2.3D-14` e `D-2.3D-15`, em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18`, em 05/09/2026) |
...
**Origem.** `D-2.3D-05` fixa `revogada_por_usuario_id` = o próprio usuário **apenas para o logout**; nenhuma fonte fixava o valor na revogação em massa disparada pela conclusão da recuperação de senha (T-07). A F6 implementou o próprio usuário e registrou a escolha como decisão local `L-F6-06`, apresentando-a para decisão expressa. Uma primeira tentativa de encerrá-la por analogia com o logout foi **recusada** pela revisão da PR #21, corretamente: analogia não é fundamento normativo. A F7 reabriu a questão a partir da semântica real do campo e Bruno homologou o comportamento vigente em 05/09/2026.

**Semântica do campo, apurada na F7.** `sessao_autenticacao.revogada_por_usuario_id` é FK anulável para `usuario`, sem CHECK, **nunca lida** por código de aplicação — é trilha forense, não insumo de decisão de runtime. É escrita em exatamente três pontos: `NULL` na emissão; o próprio usuário no logout (`D-2.3D-05`); o próprio usuário na revogação em massa da T-07. Permanece `NULL` em toda sessão `ATIVA` e em toda sessão `EXPIRADA` — a transição de expiração não toca a coluna. Disso decorre a invariante vigente: **`estado = 'REVOGADA'` ⟺ a coluna está preenchida**.

**Regra vinculante.** Na revogação em massa de sessões executada pela conclusão da recuperação de senha (T-07), `revogada_por_usuario_id` recebe o **`usuario_id` da própria sessão revogada** — isto é, o titular da conta cuja senha foi redefinida. O valor é derivado da própria linha (`SET revogada_por_usuario_id = usuario_id`), nunca de um parâmetro, de modo que é impossível atribuí-lo a outro usuário.

**Fundamento — o ator do ato que causa a revogação.** O campo registra **quem praticou o ato do qual a revogação decorreu**, e esse ator é o que `docs/09` §5 já fixa para o evento de auditoria correspondente:

| Ato que revoga | Evento de auditoria | Ator homologado | `revogada_por_usuario_id` |
| --- | --- | --- | --- |
| Logout | `usuario.sessao.logout` | Usuário | o próprio usuário (`D-2.3D-05`) |
| Conclusão da recuperação (T-07) | `usuario.senha.recuperacao_concluida` | Usuário | o próprio usuário (**esta decisão**) |
| Revogação administrativa de terceiro | `usuario.sessao.revogacao` | Administrador | o Administrador — quando a operação existir |

A regra é, portanto, **específica da T-07** e não generaliza: ela não determina o valor para a revogação administrativa de terceiro (`sessoes.revogar_terceiro`, `docs/04` §5.1/§8), ainda **não implementada**, cujo ator homologado é o Administrador.

**Alternativas rejeitadas, e por quê:**

- **`NULL`** — colide com a semântica vigente da coluna, em que `NULL` marca sessão **não revogada** (`ATIVA` ou `EXPIRADA`). Destruiria a invariante acima e tornaria a coluna incapaz de distinguir "revogada sem autor" de "nunca revogada". Também afirmaria ação de sistema, e o modelo **não possui** representação de identidade de sistema.
- **O Administrador que iniciou a recuperação** — factualmente falso. Ele iniciou a emissão do segredo; a revogação ocorre **somente se e quando** alguém concluir o fluxo, possivelmente meia hora depois, e pode nunca ocorrer. Atribuir-lhe o ato confundiria este caso com a futura revogação administrativa, corrompendo o significado desse registro antes mesmo de ele existir.
...
**Limite declarado.** A rota de conclusão não é autenticada: o sistema prova **posse do segredo**, não a identidade de quem o apresenta. Atribuir a autoria ao titular é, portanto, a mesma presunção que `docs/09` §5 já adota ao fixar o Usuário como ator de `usuario.senha.recuperacao_concluida` — esta decisão a torna consistente entre a trilha de auditoria e a coluna, e **não** cria presunção nova.
...
| **AUT-002** — Encerrar, expirar e revogar sessões | `D-2.3D-01` (janela física), `D-2.3D-04` (expiração absoluta e ociosa), `D-2.3D-05` (estados terminais), `D-2.3D-18` (autoria da revogação em massa da T-07) | Estados e transições preservados **sem alteração**: `ATIVA -> EXPIRADA`, `ATIVA -> REVOGADA`. A revogação **administrativa** de terceiro (`sessoes.revogar_terceiro`) permanece **NÃO IMPLEMENTADA** |
...
| REV. | Data | Alterações |
| --- | --- | --- |
| **9** | 05/09/2026 | **ACRÉSCIMO DE UMA DECISÃO NOVA — `D-2.3D-18` (§5.18), homologada por Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1) — E ENCERRAMENTO DA ETAPA 2.3D PELA F7.** Nenhuma decisão anterior foi modificada, renumerada, reduzida ou reaberta: `D-2.3D-01`..`D-2.3D-17` permanecem exatamente como homologadas, e **`D-2.3D-05` em particular está intacta**. `D-2.3D-18` fecha `L-F6-06`, a última ambiguidade comportamental aberta da Etapa: na revogação em massa da T-07, `revogada_por_usuario_id` recebe o `usuario_id` da própria linha. O fundamento **não** é analogia com o logout — essa justificativa foi recusada, com razão, pela revisão da PR [#21](https://github.com/BrunoMNoronha/techlab-fisio/pull/21) — e sim a semântica apurada do campo pela F7: ele registra **quem praticou o ato do qual a revogação decorreu**, e esse ator é o que `docs/09` §5 já fixa para o evento de auditoria correspondente (`Usuário` para `usuario.senha.recuperacao_concluida`). A regra é **restrita à T-07** e não determina o valor para a revogação administrativa de terceiro, cujo ator homologado é o Administrador e que segue **não implementada**. Registradas as alternativas rejeitadas (`NULL`, que colidiria com o significado vigente da coluna em sessões `ATIVA`/`EXPIRADA`; o Administrador iniciador, factualmente falso; coluna nova, desnecessária) e o limite de que a conclusão prova **posse do segredo**, não identidade. **`L-F6-06` vira alias histórico.** **Encerramentos factuais:** a linha da fatia **F7** em §9 passa a **CONCLUÍDA**; a pendência **`P-2.3D-03`** em §11 passa a **ENCERRADA**, por estar medida e verde desde a F3 e provada permanentemente a cada CI por `verify:openapi-runtime` (19/19) — **`D-2.3D-11` não é alterada**; e a linha de **AUT-002** em §12 passa a citar `D-2.3D-18`. Registro em `docs/10` §6-Q e §11, REV. 34. **Permanecem abertas** `P-2.3D-01`, `P-2.3D-04`, `P-2.3D-05` e `P-2.3D-06` (adiada) — nenhuma delas pertence à Etapa 2.3D. **AUT-005 permanece NÃO MATERIALIZADA / NÃO ENCERRADA**; `P2.2-05` permanece **NÃO INICIADA**; a revogação administrativa de sessão de terceiro permanece **NÃO IMPLEMENTADA**. |
...
**Fim — `docs/12-decisoes-autenticacao-autorizacao.md` — `D-2.3D-01`..`D-2.3D-18` registradas; F0 da Etapa 2.3D-B executada em 27/08/2026; `D-2.3D-13` homologada em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` homologadas em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18` homologadas em 05/09/2026; F1..F7 CONCLUÍDAS e Etapa 2.3D ENCERRADA em 05/09/2026.**
