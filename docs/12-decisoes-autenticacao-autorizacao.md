...
> **Data:** 27 de agosto de 2026 (REV. 3 a REV. 5 em 28/08/2026; REV. 6 em 31/08/2026; REV. 7 em 02/09/2026; REV. 8 e REV. 9 em 05/09/2026; REV. 10 e REV. 11 em 06/09/2026; REV. 12 em 14/09/2026; REV. 13 em 15/09/2026)
...
| `P-2.3D-04` | Reavaliação da proteção CSRF contra a arquitetura real do `apps/web` | **ABERTA** — o frontend passou a existir (`FRONT-F0`, 05/09/2026) e a **integrar-se** com `apps/api` em 06/09/2026 pela Fatia 1 (PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27), merge `0d76474`; `docs/08` REV. 26, `docs/10` REV. 38), sob topologia **same-origin** por proxy transparente que preserva `Host` público, `Origin`, `Sec-Fetch-*` e `X-TLF-Requisicao`, CORS desabilitado e compatibilidade de cabeçalhos avaliada. A prova real ponta a ponta em runtime executando simultaneamente PostgreSQL, NestJS, Next.js e CSRF guard foi materializada por `P-2.3D-08`. O encerramento formal da pendência permanece reservado a rito próprio após a integração desta fatia na `main` |
...
| `P-2.3D-08` | **Materializar D-2.3D-20 (`GET /auth/sessao`) e construir prova E2E real same-origin** | **EM ANDAMENTO / IMPLEMENTADA EM BRANCH PRÓPRIA em 15/09/2026 (`agent/p-2.3d-08-sessao-atual-e2e`)** — Endpoint `GET /auth/sessao` entregue sob `SessaoAutenticadaGuard`, `Cache-Control: no-store`, envelope estrito `ConsultarSessaoRespostaDto` `{ usuarioId, sessaoId }`, sem CSRF guard, sem emissão de auditoria. Prova E2E real automatizada `verify-web-api-e2e.mjs` com PostgreSQL 18 descartável, NestJS compilado, Next.js com proxy Route Handler, `ProtecaoCsrfGuard` real e cookies reais. Critérios `A-01`..`A-22` atendidos, 4 mutation challenges executados e integrados à CI |
