...
| `P-2.3D-04` | Reavaliação da proteção CSRF contra a arquitetura real do `apps/web` | **ABERTA** — o frontend passou a existir (`FRONT-F0`, 05/09/2026) e a **integrar-se** com `apps/api` em 06/09/2026 pela Fatia 1 (PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27), merge `0d76474`; `docs/08` REV. 26, `docs/10` REV. 38), o que torna a reavaliação de `D-2.3D-07` **materialmente possível e já medida**: a topologia é **same-origin** por um proxy transparente que preserva `Host` público, `Origin`, `Sec-Fetch-*` e `X-TLF-Requisicao`, CORS permanece **desabilitado** e a compatibilidade de cabeçalhos é avaliada por 3 provas contra uma função local de reprodução (`avaliarGuard` em `scripts/verify-web-integration.mjs`, que aprova o repasse simulado, recusa `CABECALHO_REQUISICAO_AUSENTE` e recusa `ORIGIN_INVALIDA` — **sem executar o `ProtecaoCsrfGuard` real do NestJS** e sem executar o Route Handler real do Next.js). **A baseline de `D-2.3D-07` foi preservada sem enfraquecimento** — nenhum synchronizer token, nenhuma exceção no guard, nenhuma decisão normativa criada ou alterada. **A pendência NÃO é encerrada por esse fato:** a fatia **mede** sob escopo local/simulado, a bateria de 24 verificações não roda na CI, a prova de fluxo integrado ponta a ponta em runtime ainda não foi realizada, e o encerramento formal é ato expresso de Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1). Registro da divergência: o `README.md` de `apps/web`, como integrado, declarava a pendência "resolvida ... aprovada por Bruno Menezes Noronha" **sem lastro documental** nesta seção; a asserção foi corrigida editorialmente na REV. 11, sem alterar código |
