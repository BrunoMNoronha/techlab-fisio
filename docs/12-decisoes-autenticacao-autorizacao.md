# Decisões de Autenticação e Autorização — `D-2.3D-01`..`D-2.3D-19`
...
> **Status:** **DECIDIDO — `D-2.3D-01`..`D-2.3D-19` homologadas por Bruno Menezes Noronha (`D-2.3D-01`..`D-2.3D-12` em 27/08/2026; `D-2.3D-13` em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18` em 05/09/2026; `D-2.3D-19` em 06/09/2026 — TLF-BASE-V1 §15, item 1)**
> **Data:** 27 de agosto de 2026 (REV. 3 a REV. 5 em 28/08/2026; REV. 6 em 31/08/2026; REV. 7 em 02/09/2026; REV. 8 e REV. 9 em 05/09/2026; REV. 10 e REV. 11 em 06/09/2026)
...
> **Revisão vigente:** REV. 10 (06/09/2026) — **ACRÉSCIMO DE UMA DECISÃO NOVA: `D-2.3D-19` (§5.19) — revogação administrativa de sessão de terceiro**, homologada por Bruno Menezes Noronha. Rodada **exclusivamente normativa e documental**: nenhuma linha de runtime, schema, migration, dependência, contrato ou teste foi alterada. `D-2.3D-01`..`D-2.3D-18` permanecem exatamente como homologadas — **`D-2.3D-05` e `D-2.3D-18` em particular estão intactas**. A AUT-002 continua **NÃO IMPLEMENTADA**: esta revisão fecha a lacuna **normativa** de autoria e abre `P-2.3D-07` (§11) para a implementação. A faixa de decisões do cabeçalho foi corrigida de `..D-2.3D-15` para `..D-2.3D-19`: a atualização havia sido omitida nas REV. 8 e REV. 9, que acrescentaram `D-2.3D-16`..`D-2.3D-18` sem tocar o cabeçalho. A correção é **editorial e factual** e não altera decisão alguma.
...
| Campo | Valor |
| --- | --- |
| Identificadores | `D-2.3D-01` .. `D-2.3D-19` |
| Autoridade | Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1) |
| Data da homologação | 27/08/2026 (`D-2.3D-01`..`D-2.3D-12`) · 28/08/2026 (`D-2.3D-13`) · 31/08/2026 (`D-2.3D-14` e `D-2.3D-15`) · 05/09/2026 (`D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18`) · 06/09/2026 (`D-2.3D-19`) |
...
**Dentro do escopo deste registro:** persistência de sessão; hash de senha; normalização do login; política de sessão; estados terminais; anti-abuso; cookie e CSRF; recuperação de senha; RBAC inicial; primeiro Administrador; OpenAPI; auditoria de autenticação; **revogação administrativa de sessão de terceiro** (`D-2.3D-19`, acrescentada em 06/09/2026 — decisão **normativa**; a implementação é fatia própria e **não foi executada**).
...
| --- | --- | --- | --- |
| `A-01` | Administrador com `sessoes.revogar_terceiro` revoga uma sessão **ativa** de outro usuário | natureza da operação | **PENDENTE** |
| `A-02` | `sessao_autenticacao.revogada_por_usuario_id` recebe o **Administrador autenticado** executor | ator | **PENDENTE** |
| `A-03` | `evento_auditoria.ator_usuario_id` recebe **exatamente o mesmo** Administrador de `A-02` | ator | **PENDENTE** |
| `A-04` | **Não existe** campo HTTP — corpo, query, path, cabeçalho ou DTO — capaz de substituir ou injetar o ator; o contrato **não possui** campo equivalente, ainda que fosse ignorado | ator | **PENDENTE** |
| `A-05` | Usuário autenticado **sem** `sessoes.revogar_terceiro` **não** executa a operação | RBAC (`D-2.3D-09`) | **PENDENTE** |
| `A-06` | Autorrevogação pela capacidade administrativa **não altera** a sessão | proibição de autorrevogação | **PENDENTE** |
| `A-07` | Sessão **inexistente** produz **o mesmo envelope** da operação efetiva | envelope uniforme | **PENDENTE** |
| `A-08` | Sessão **já revogada** produz o mesmo envelope | envelope uniforme | **PENDENTE** |
| `A-09` | Sessão **já expirada** produz o mesmo envelope | envelope uniforme | **PENDENTE** |
| `A-10` | Sessão efetivamente vencida pela política temporal (`D-2.3D-04`) **não** é reclassificada como `REVOGADA` pelo Administrador; preserva a semântica de **expiração** | estado temporal | **PENDENTE** |
| `A-11` | Revogação efetiva e evento de auditoria são **atômicos** — mesma transação | atomicidade | **PENDENTE** |
| `A-12` | Falha da escrita de auditoria **impede** a revogação efetiva (rollback), sem falso `204`, sem `401`/`403` e sem vazamento interno | atomicidade | **PENDENTE** |
| `A-13` | Token, hash de token, cookie, senha, segredo, payload e cabeçalhos sensíveis **nunca** entram na auditoria; `contexto` permanece **vazio** | auditoria | **PENDENTE** |
| `A-14` | Após revogação efetiva, o token antigo **não autoriza** nenhuma operação | AUT-002; `D-2.3D-05` | **PENDENTE** |
| `A-15` | Duas revogações administrativas **concorrentes** sobre a mesma sessão convergem para estado terminal único e coerente, com autoria coerente e **sem** evento duplicado de sucesso | concorrência; no-op | **PENDENTE** |
| `A-16` | Revogação administrativa **×** logout do próprio usuário converge corretamente, com autoria compatível com a operação que efetivamente venceu | concorrência | **PENDENTE** |
| `A-17` | Revogação administrativa **×** validação/registro de atividade converge corretamente; **não existe janela** em que o `204` seja reportado e a sessão siga utilizável | concorrência | **PENDENTE** |
| `A-18` | Revogação administrativa **×** expiração converge corretamente, sem estado contraditório e sem ressurreição | concorrência; estado temporal | **PENDENTE** |
| `A-19` | **Logout mantém `D-2.3D-05`** — `revogada_por_usuario_id` continua sendo o proprietário da própria sessão, sem regressão | exceção delimitada | **PENDENTE** |
| `A-20` | **T-07 mantém `D-2.3D-18`** — a revogação em massa continua derivando a autoria da própria linha, sem regressão | exceção delimitada | **PENDENTE** |
| `A-21` | **Nenhum artefato de AUT-005** é introduzido — sem ativação/inativação de usuário, sem módulo `usuario-administrativo`, sem enum, erro ou contrato exclusivos de AUT-005 | escopo | **PENDENTE** |
| `A-22` | **Nenhum drift de persistência** — `schema.prisma`, migrations, `schema.golden.sql`, `protected-objects.json` e lockfiles inalterados | zero drift | **PENDENTE** |
...
| `P-2.3D-04` | Reavaliação da proteção CSRF contra a arquitetura real do `apps/web` | **ABERTA** — o frontend passou a existir (`FRONT-F0`, 05/09/2026) e a **integrar-se** com `apps/api` em 06/09/2026 pela Fatia 1 (PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27), merge `0d76474`; `docs/08` REV. 26, `docs/10` REV. 38), o que torna a reavaliação de `D-2.3D-07` **materialmente possível e já medida**: a topologia é **same-origin** por um proxy transparente que preserva `Host` público, `Origin`, `Sec-Fetch-*` e `X-TLF-Requisicao`, CORS permanece **desabilitado** e a `ProtecaoCsrfGuard` é exercitada por 3 provas (aprova o repasse legítimo; recusa `CABECALHO_REQUISICAO_AUSENTE`; recusa `ORIGIN_INVALIDA`). **A baseline de `D-2.3D-07` foi preservada sem enfraquecimento** — nenhum synchronizer token, nenhuma exceção no guard, nenhuma decisão normativa criada ou alterada. **A pendência NÃO é encerrada por esse fato:** a fatia **mede**, não homologa, e o encerramento formal é ato expresso de Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1). Registro da divergência: o `README.md` de `apps/web`, como integrado, declarava a pendência "resolvida ... aprovada por Bruno Menezes Noronha" **sem lastro documental** nesta seção; a asserção foi corrigida editorialmente na REV. 11, sem alterar código |
...
| `P-2.3D-07` | **Implementação da revogação administrativa de sessão de terceiro (AUT-002 / `sessoes.revogar_terceiro`), conforme `D-2.3D-19`** | **ABERTA — decisão normativa FECHADA em 06/09/2026, implementação NÃO INICIADA.** `D-2.3D-19` (§5.19) fecha a lacuna de autoria que impedia a fatia; os critérios de aceite estão em §10.4 (`A-01`..`A-22`). Não existe rota, service, DTO nem emissor de auditoria — `sessoes.revogar_terceiro` segue **catalogada e órfã**. Fatia **própria**, fora da Etapa 2.3D (encerrada), sujeita a **autorização própria** e sem migration esperada. Encerra também a dependência registrada do risco de volume V0 de `PBACK-AUD-09` (`docs/10` §9), que conta com esta operação como resposta administrativa |

| ~~`Q-01`~~ | Suposto conflito entre `docs/09` §5 e `docs/06` §12.7 sobre a auditoria e a autoria da revogação em massa da T-07 | **RESOLVIDO em 05/09/2026 — não havia conflito normativo.** `docs/09` declara §§1..11 "insumo decisório histórico, **sem valor normativo próprio**", com §12 prevalecendo; `D-AUD-01` (§12.2) homologa **apenas os 24 nomes de ações**, não as colunas de ator ou descrição. A linha invocada pertence a §5 e não vincula. **`docs/09` NÃO foi alterado** — o documento manda preservar §§1..11 intactos, e sua própria regra de precedência já resolve a leitura. Decidiu `L-F6-06` (→ `D-2.3D-18`) |
...
| **AUT-002** — Encerrar, expirar e revogar sessões | `D-2.3D-01` (janela física), `D-2.3D-04` (expiração absoluta e ociosa), `D-2.3D-05` (estados terminais), `D-2.3D-18` (autoria da revogação em massa da T-07), **`D-2.3D-19` (revogação administrativa de terceiro — normativa, NÃO implementada)** | **PARCIAL.** Estados e transições preservados **sem alteração**: `ATIVA -> EXPIRADA`, `ATIVA -> REVOGADA`. **Implementados:** logout, expiração e revogação em massa da T-07. **DECIDIDA mas NÃO IMPLEMENTADA:** revogação **administrativa** de terceiro — `D-2.3D-19` (§5.19) fecha a lacuna normativa de autoria em 06/09/2026, com critérios de aceite em §10.4 e a fatia rastreada como `P-2.3D-07`; não existe rota, service nem emissor. **NÃO implementada e sem decisão nesta revisão:** inativação de usuário (AUT-005) |
...
| --- | --- |
| `D-AUD-01` — catálogo de 24 ações | **INALTERADO.** `usuario.autenticacao`, `usuario.sessao.logout`, `usuario.sessao.revogacao`, `usuario.senha.recuperacao_iniciada`, `usuario.senha.recuperacao_concluida` e `usuario.papeis.alterados` já pertencem ao catálogo. **Nenhuma ação acrescentada** — inclusive por `D-2.3D-19`, que **reutiliza** `usuario.sessao.revogacao` e **não** cria ação de "tentativa de revogação" |
...
**Fim — `docs/12-decisoes-autenticacao-autorizacao.md` — `D-2.3D-01`..`D-2.3D-19` registradas; F0 da Etapa 2.3D-B executada em 27/08/2026; `D-2.3D-13` homologada em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` homologadas em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18` homologadas em 05/09/2026; F0..F7 CONCLUÍDAS e Etapa 2.3D ENCERRADA em 05/09/2026; `D-2.3D-19` homologada em 06/09/2026 — decisão normativa da revogação administrativa de sessão de terceiro, cuja implementação permanece PENDENTE como `P-2.3D-07`, fora da Etapa 2.3D; REV. 11 (06/09/2026) registra, de forma exclusivamente factual, a integração da REV. 10 na `main` (PR #26, merge `aba939c`) e a da Fatia 1 da integração `apps/web` ↔ `apps/api` (PR #27, merge `0d76474`), com **`P-2.3D-04` mantida ABERTA** e a baseline de `D-2.3D-07` preservada sem enfraquecimento.**
