# Decisões de Autenticação e Autorização — `D-2.3D-01`..`D-2.3D-22`
...
> **Status:** **DECIDIDO — `D-2.3D-01`..`D-2.3D-22` homologadas por Bruno Menezes Noronha (`D-2.3D-01`..`D-2.3D-12` em 27/08/2026; `D-2.3D-13` em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18` em 05/09/2026; `D-2.3D-19` em 06/09/2026; `D-2.3D-20` em 07/09/2026; `D-2.3D-21` em 16/09/2026; `D-2.3D-22` em 17/09/2026 — TLF-BASE-V1 §15, item 1)**
> **Data:** 27 de agosto de 2026 (REV. 3 a REV. 5 em 28/08/2026; REV. 6 em 31/08/2026; REV. 7 em 02/09/2026; REV. 8 e REV. 9 em 05/09/2026; REV. 10 e REV. 11 em 06/09/2026; REV. 12 em 14/09/2026; REV. 13 e REV. 14 em 15/09/2026; REV. 15 em 16/09/2026; REV. 16 a REV. 19 em 17/09/2026)
...
> **Revisão vigente:** REV. 19 (17/09/2026) — **SINCRONIZAÇÃO PÓS-INTEGRAÇÃO DE `P-2.3D-10` / `D-2.3D-22` NA `main` (PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), MERGE `bc6a763`) E CORREÇÕES DA REVISÃO AUTOMATIZADA** (§5.22, §10.7). Nenhuma decisão criada, modificada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-22` vigentes. Fatos: PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), commits da fatia `254a677` (implementação e testes), `58c4741` (`docs/12` REV. 18) e `400377e` (`docs/10` REV. 49), merge commit **`bc6a763f3525b16384022ad5b43b95a542be5b2f`** (merge commit, sem squash/rebase, por auto-merge autorizado por Bruno Menezes Noronha), 17/09/2026 05:48:03Z, CI verde nas runs `35186845908` (push) e `35186860133` (pull_request). Correções: revisão automatizada do Codex sobre `400377e`, recebida após o merge, com dois achados P2: **(1)** falha técnica da listagem saía no corpo padrão do Nest (`{"statusCode":500,"message":...}`) porque `FiltroErroAutenticacao` não reconhecia a rota — **defeito real, corrigido**: a rota exata `GET /auth/usuarios/:usuarioId/sessoes` entra no escopo fechado do filtro, com `L-15` (integração) e testes unitários de escopo; mutação que remove a rota do filtro detectada por `L-15`; **(2)** "a consulta não escreve nada" era afirmado sem ressalva, mas a guarda de sessão do **operador** registra atividade (e detecta expiração) como em toda rota autenticada, inclusive `GET /auth/sessao` (`D-2.3D-04`/`D-2.3D-20`) — **esclarecimento de escopo, sem alteração de comportamento nem de decisão**: a ausência de escrita refere-se às sessões do **alvo**; provado por `L-16`.
> **Estado anterior preservado:** REV. 18 (17/09/2026) — **ACRÉSCIMO DE UMA DECISÃO NOVA: `D-2.3D-22` (§5.22) — listagem administrativa das sessões ativas de um usuário (AUT-002)**, homologada por Bruno Menezes Noronha em 17/09/2026 (opção conservadora do pacote de decisão da frente de Administração de Sessões), **e materialização da fatia `P-2.3D-10` em branch própria** (`agent/sessoes-admin-next`), com critérios `L-01`..`L-14` (§10.7) atendidos. Nenhuma decisão anterior foi modificada, renumerada, reduzida ou reaberta: `D-2.3D-01`..`D-2.3D-21` vigentes. Zero drift de persistência; nenhuma permissão, ação de auditoria ou dependência nova. **Não integrada na `main`**: sem commit, push, PR ou merge nesta execução.
> **Estado anterior preservado:** REV. 17 (17/09/2026) — **REGISTRO PÓS-INTEGRAÇÃO DO REFORÇO DE PROVAS DE `P-2.3D-09` / `D-2.3D-21` NA `main` (PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), MERGE `bce22e8`)** (§10.6). Registro **exclusivamente factual**: nenhuma decisão criada, modificada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-21` vigentes; nenhuma linha de código de produção, schema, migration, dependência ou CI alterada. Fatos: PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), commit `4d95c386d15d08060c319300a09a1f4eef5f3590` (somente `apps/api/test/integration/usuarios-situacao.integration.spec.ts`), merge commit **`bce22e8265317be16bf78fcb1861aafdc4b05b0c`** (merge commit, sem squash/rebase), 17/09/2026 03:54:24Z, CI verde na run `35179564801`.
> **Estado anterior preservado:** REV. 16 (17/09/2026) — **SINCRONIZAÇÃO PÓS-INTEGRAÇÃO DE `P-2.3D-09` / `D-2.3D-21` NA `main` (PR [#52](https://github.com/BrunoMNoronha/techlab-fisio/pull/52), MERGE `817da42`)**. Registro **exclusivamente factual**: nenhuma decisão criada, modificada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-21` vigentes. Integrada em 17/09/2026 por Bruno Menezes Noronha — commits da fatia `7d16ded` (implementação e testes) e `dbef1fb9c4483bd2d9c261a77fb11270b7ffef70` (documentação), merge commit **`817da427de84fe452230df233cb4ae15c8a43593`** (merge commit, sem squash/rebase), 17/09/2026 03:08:37Z, CI verde na run `35176158028`. `P-2.3D-09` passa a **CONCLUÍDA / INTEGRADA NA `main`**; AUT-005 passa a **MATERIALIZADA**.
> **Estado anterior preservado:** REV. 15 (16/09/2026) — **ACRÉSCIMO DE UMA DECISÃO NOVA: `D-2.3D-21` (§5.21) — ativação e inativação administrativa de usuário (AUT-005)**, homologada por Bruno Menezes Noronha em 16/09/2026, **e materialização da fatia `P-2.3D-09` em branch própria** (`agent/aut-005-situacao-usuario`), com critérios `A-01`..`A-16` (§10.6) atendidos. Nenhuma decisão anterior foi modificada, renumerada, reduzida ou reaberta: `D-2.3D-01`..`D-2.3D-20` permanecem exatamente como homologadas. Zero drift de persistência; zero dependências adicionadas; nenhuma ação de auditoria criada (`usuario.situacao.alterada` já pertence a `D-AUD-01`, com whitelist de `contexto` vazia). **Não integrada na `main`** — o registro da integração pertence a rito pós-merge.
> **Estado anterior preservado:** REV. 14 (15/09/2026) — **SINCRONIZAÇÃO PÓS-INTEGRAÇÃO DE `P-2.3D-08` / `D-2.3D-20` NA `main` (PR #47, MERGE `89fa492`) E ENCERRAMENTO FACTUAL DE `P-2.3D-04` MEDIANTE PROVA E2E REAL SAME-ORIGIN**. Nenhuma decisão anterior foi modificada, renumerada, reduzida ou reaberta: `D-2.3D-01`..`D-2.3D-20` vigentes; Base Imutável V2 intacta; zero drift de persistência (`packages/database`); zero dependências adicionadas. **(a) Sincronização pós-integração:** `D-2.3D-20` (`GET /auth/sessao`) e a prova E2E real same-origin integradas na `main` em 15/09/2026 via PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47) (commit `4f78c4ed2de5b2e9f358f55accfa021b027fc124`, merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI run `34983136046`). `P-2.3D-08` passa a **CONCLUÍDA / INTEGRADA NA `main`**. **(b) Encerramento formal de `P-2.3D-04`:** Encerrada em 15/09/2026 após a integração do PR #47 (merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`) que versionou `apps/web/scripts/verify-web-api-e2e.mjs` e o passo correspondente na CI, executando simultaneamente PostgreSQL real em container, NestJS real compilado, Next.js real compilado com Route Handler de proxy same-origin (`/api/*`), `ProtecaoCsrfGuard` real e cookie real de sessão, provando que a baseline de CSRF de `D-2.3D-07` (`SameSite=Strict`, cabeçalho obrigatório `X-TLF-Requisicao` nas mutações, Fetch Metadata, validação de `Origin`, ausência de CORS) protege a fronteira de ponta a ponta sem necessidade de synchronizer token adicional.
...
| Campo | Valor |
| --- | --- |
| Identificadores | `D-2.3D-01` .. `D-2.3D-21` |
| Autoridade | Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1) |
| Data da homologação | 27/08/2026 (`D-2.3D-01`..`D-2.3D-12`) · 28/08/2026 (`D-2.3D-13`) · 31/08/2026 (`D-2.3D-14` e `D-2.3D-15`) · 05/09/2026 (`D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18`) · 06/09/2026 (`D-2.3D-19`) · 07/09/2026 (`D-2.3D-20`) · 16/09/2026 (`D-2.3D-21`) |
...
### 5.21 `D-2.3D-21` — Ativação e inativação administrativa de usuário (AUT-005) *(normativa; homologada em 16/09/2026 por Bruno Menezes Noronha; complementa `D-2.3D-05` e `D-2.3D-10`; **não** altera `D-2.3D-18` nem `D-2.3D-19`)*

**Origem.** AUT-005 permanecia **NÃO MATERIALIZADA**: §12 associava o requisito a `D-2.3D-05` e `D-2.3D-10`, mas nenhuma decisão fixava o contrato HTTP, a autoria da revogação em massa decorrente da inativação, o tratamento da auto-inativação, a semântica da reativação ou o comportamento de pedidos que não mudam o estado. A permissão `usuarios.gerenciar` (`D-2.3D-09`) e a ação `usuario.situacao.alterada` (`D-AUD-01`, whitelist de `contexto` vazia) já existiam, **órfãs**. Bruno Menezes Noronha homologou em 16/09/2026 as regras abaixo — incluindo os três pontos `D21-01`..`D21-03` — antes da integração da fatia.

**Contrato mínimo vinculante:**
- **Método e caminho:** `PATCH /auth/usuarios/:usuarioId/situacao`, em módulo próprio (`UsuariosModule`), sem ampliar a fronteira do `AuthModule`.
- **Autorização:** `@RequerPermissao("usuarios.gerenciar")` (sessão válida + permissão); sem sessão → `401 SESSAO_INVALIDA`; sem permissão → `403 ACESSO_NEGADO`.
- **CSRF:** mutação sob `ProtecaoCsrfGuard` (`D-2.3D-07`); cabeçalho `X-TLF-Requisicao` obrigatório.
- **Ator:** derivado **exclusivamente** da sessão autenticada (`ContextoAutenticado`). É **proibido** receber ator do cliente por qualquer mecanismo.
- **Entrada:** `usuarioId` em formato UUID; corpo **estrito** `{ "ativo": boolean }` — sem coerção de tipo e sem chaves excedentes. Violação → `400 REQUISICAO_INVALIDA`.
- **Respostas:**
  - `200 OK` com `{ usuarioId, ativo, inativadoEm }` (`inativadoEm` em ISO-8601 ou `null`) — para mutação efetiva **e** para no-op (`D21-03`);
  - `404 USUARIO_INEXISTENTE` para alvo inexistente (operação administrativa autenticada e autorizada; não há exigência anti-enumeração equivalente à de `D-2.3D-19`);
  - `422 AUTO_INATIVACAO_PROIBIDA` para auto-inativação (`D21-01`);
  - `413` para corpo acima do limite do parser; `500 FALHA_INTERNA` para falha técnica.

**Regras homologadas:**
- **`D21-01` — Auto-inativação proibida.** Se `ativo = false` e `usuarioId` = ator, a operação é recusada com `422 AUTO_INATIVACAO_PROIBIDA`, sem mutação, sem revogação de sessões e sem auditoria. Protege contra a perda acidental do próprio acesso administrativo. A auto-**reativação** não é caso alcançável (usuário inativo não autentica) e não recebe regra própria.
- **`D21-02` — Reativação limpa os metadados de inativação.** `ativo = true` sobre usuário inativo grava `ativo = true`, `inativado_em = NULL` e `inativado_por_usuario_id = NULL`. A reativação **não** cria, restaura nem reabre sessões: o usuário precisa autenticar novamente.
- **`D21-03` — No-op idempotente.** Pedido cujo `ativo` já é o estado vigente devolve `200` com o estado corrente, **sem** mutação, **sem** alterar timestamps e **sem** evento de auditoria (no-op não emite auditoria falsa de sucesso, no mesmo espírito de `D-2.3D-19`).
- **Inativação.** Grava `ativo = false`, `inativado_em = agora` e `inativado_por_usuario_id = ator`, e **revoga em massa** as sessões `ATIVA` do alvo na **mesma transação**:
  - sessões já vencidas (absoluta ou ociosamente, `D-2.3D-04`) são fechadas como `EXPIRADA`, e **não** reclassificadas como revogadas pelo Administrador;
  - as demais passam a `REVOGADA`, com `encerrada_em = agora` e `revogada_por_usuario_id` = **Administrador executor** — autoria distinta de `D-2.3D-18` (T-07, titular) e de `D-2.3D-05` (logout próprio); a regra vale **somente** para esta operação e não é generalizável.
- **Login posterior.** Usuário inativo não autentica (AUT-001), com o mesmo desfecho uniforme de credenciais inválidas (`D-2.3D-02`/`D-2.3D-12`).
- **Concorrência.** O alvo é bloqueado (`SELECT ... FOR UPDATE`) antes da decisão. Inativações concorrentes convergem para **uma** mutação e **um** evento. Um login em voo concorrente com a inativação nunca deixa sessão `ATIVA` após o commit da inativação.
- **Auditoria obrigatória e atômica.** Toda mutação efetiva emite `usuario.situacao.alterada` na **mesma transação**, com `ator_usuario_id` = ator, `alvo_tipo = "usuario"`, `alvo_id` = usuário-alvo, `resultado = SUCESSO`, `justificativa = NULL` e `contexto` **vazio** (`D-AUD-07`). Se o evento não puder ser persistido, a mutação do usuário **e** a revogação das sessões sofrem rollback conjunto.

**Não-objetivos:** não cria permissão, ação de auditoria, valor de `resultado` nem chave de `contexto`; não cria migration nem altera schema (usa `usuario.ativo`, `usuario.inativado_em`, `usuario.inativado_por_usuario_id` e `sessao_autenticacao` existentes); não implementa listagem, criação ou edição de usuários, nem alteração de papéis; não altera `D-2.3D-05`, `D-2.3D-18`, `D-2.3D-19` ou `D-2.3D-20`.

### 5.22 `D-2.3D-22` — Listagem administrativa das sessões ativas de um usuário *(normativa; homologada em 17/09/2026 por Bruno Menezes Noronha; complementa `D-2.3D-19`; **não** altera `D-2.3D-04`, `D-2.3D-05`, `D-2.3D-19` nem `D-2.3D-20`)*

**Origem.** `D-2.3D-19` materializou a revogação administrativa de sessão de terceiro por `sessao_id` (`DELETE /auth/sessoes/:sessaoId`, `P-2.3D-07`), mas nenhuma rota, permissão ou decisão permitia ao Administrador **descobrir** esse identificador: a capacidade existia e não era alcançável pela aplicação. A reconciliação de 17/09/2026 classificou a lacuna como **decisão necessária** (rota, permissão, dados expostos, paginação e auditoria da consulta) e apresentou opção conservadora e opção flexível. Bruno homologou a **opção conservadora** como `D-2.3D-22`.

**Contrato vinculante:**

- **Método e caminho:** `GET /auth/usuarios/:usuarioId/sessoes`. `usuarioId` é o **único** parâmetro; sem query string e sem corpo.
- **Autorização:** sessão autenticada **e** `sessoes.revogar_terceiro`, pela cadeia vigente de `@RequerPermissao` (`D-2.3D-09`). **Nenhuma permissão nova** é criada: quem pode listar é exatamente quem pode revogar, e a consulta existe apenas para servir a essa capacidade. A matriz de `docs/04` e o seed RBAC **não mudam**.
- **Método seguro:** **sem** `ProtecaoCsrfGuard` e sem `X-TLF-Requisicao`, pelo mesmo racional de `D-2.3D-20`. A consulta **não escreve nas sessões do alvo** — nem a detecção persistente de expiração que `validar` executa. *Esclarecimento (REV. 19):* a guarda de sessão do **operador** continua registrando atividade e detectando expiração da **própria** sessão, exatamente como em toda rota autenticada, inclusive `GET /auth/sessao` (`D-2.3D-04`, `D-2.3D-20`); isso não é efeito da consulta e não altera esta decisão.
- **Conteúdo:** somente sessões em `estado = ATIVA` **e ainda válidas** pela política temporal de `D-2.3D-04` (`expira_em > agora` e `ultima_atividade_em > agora - timeout ocioso`), pelo **mesmo relógio** da validação. Sessões `REVOGADA`, `EXPIRADA` ou `ATIVA` já vencidas e ainda não detectadas **não** são listadas.
- **Campos expostos — conjunto fechado:** `sessaoId` (uuid), `criadaEm`, `ultimaAtividadeEm` e `expiraEm` (ISO-8601 UTC). **Nunca**: token, segredo, `token_hash`, cookie, estado, autoria de revogação, e-mail, nome, papéis ou qualquer metadado de rede.
- **Envelope:** `200 OK` com `{ "sessoes": [ ... ] }`, ordenado por `criadaEm` crescente (desempate por `sessaoId`), com `Cache-Control: no-store` e sem `Set-Cookie`. **Sem paginação**: o volume de sessões ativas por usuário é limitado pela política temporal.
- **Anti-oráculo:** usuário **inexistente** produz o **mesmo** `200 { "sessoes": [] }` de um usuário sem sessões ativas.
- **Sessões do próprio ator:** listadas como as de qualquer usuário; a revogação delas pela rota administrativa continua sendo no-op (`D-2.3D-19`).
- **Erros:** `400 REQUISICAO_INVALIDA` para `usuarioId` estruturalmente inválido; `401`, `403` e `500` pelos contratos gerais vigentes (`D-2.3D-07`, `D-2.3D-09`, `D-2.3D-11`), todos no envelope fechado `{ "erro": "<CODIGO>" }` do `FiltroErroAutenticacao` — inclusive `500 FALHA_INTERNA` (corrigido na REV. 19).
- **Auditoria:** **nenhum** evento é emitido. Nenhuma ação é acrescentada ao catálogo fechado de `D-AUD-01`, e `sessoes.revogar_terceiro` **não** entra na lista de `autorizacao.negada` (`PBACK-AUD-09`).
- **Zero drift de persistência:** a consulta usa o índice existente `(usuario_id, estado)` (IDX-S1); nenhuma migration, coluna, enum ou índice.

**O que esta decisão NÃO faz.** Não lista sessões de todos os usuários, não expõe histórico de sessões encerradas, não cria filtros nem paginação, não cria revogação em massa administrativa (que continua exigindo decisão própria, `D-2.3D-19`), não cria tela administrativa e não altera nenhuma rota existente.
...
### 10.6 Critérios de aceite normativos de `P-2.3D-09` / `D-2.3D-21` (ativação e inativação de usuário)

Provas em `apps/api/test/integration/usuarios-situacao.integration.spec.ts` (PostgreSQL 18 real), `apps/api/test/usuarios.service.spec.ts` e `apps/api/test/openapi.spec.ts`:

| ID | Critério de aceite | Origem em §5.21 | Estado |
| --- | --- | --- | --- |
| `A-01` | Sem cookie de sessão → `401 SESSAO_INVALIDA` | autorização | **CONCLUÍDO** |
| `A-02` | Operador sem `usuarios.gerenciar` → `403 ACESSO_NEGADO` | autorização | **CONCLUÍDO** |
| `A-03` | Sem `X-TLF-Requisicao` → `403 REQUISICAO_NAO_AUTORIZADA` | CSRF | **CONCLUÍDO** |
| `A-04` | Auto-inativação → `422 AUTO_INATIVACAO_PROIBIDA` | `D21-01` | **CONCLUÍDO** |
| `A-05` | Auto-inativação recusada não muta o usuário nem revoga a sessão do ator | `D21-01` | **CONCLUÍDO** |
| `A-06` | Inativação grava `ativo = false`, `inativado_em` e `inativado_por_usuario_id` = ator | inativação | **CONCLUÍDO** |
| `A-07` | Sessões ativas do alvo passam a `REVOGADA` com `revogada_por_usuario_id` = ator; sessões vencidas passam a `EXPIRADA` sem autoria | inativação | **CONCLUÍDO** |
| `A-08` | Tokens das sessões revogadas deixam de validar e login posterior do alvo é recusado | inativação / AUT-001 | **CONCLUÍDO** |
| `A-09` | Evento `usuario.situacao.alterada` com ator, `alvo_tipo = "usuario"`, `SUCESSO` e `contexto` vazio | auditoria | **CONCLUÍDO** |
| `A-10` | Reativação limpa `inativado_em` e `inativado_por_usuario_id` | `D21-02` | **CONCLUÍDO** |
| `A-11` | Reativação não cria nem restaura sessões | `D21-02` | **CONCLUÍDO** |
| `A-12` | Falha na escrita da auditoria desfaz a mutação do usuário | atomicidade | **CONCLUÍDO** |
| `A-13` | Falha na escrita da auditoria preserva as sessões ativas | atomicidade | **CONCLUÍDO** |
| `A-14` | Alvo inexistente → `404 USUARIO_INEXISTENTE`; UUID ou corpo inválido → `400 REQUISICAO_INVALIDA` | contrato | **CONCLUÍDO** |
| `A-15` | No-op (ativar ativo / inativar inativo) → `200` sem mutação, sem alteração de timestamp e sem auditoria | `D21-03` | **CONCLUÍDO** |
| `A-16` | Concorrência: inativações simultâneas convergem para uma mutação e um evento; login em voo não deixa sessão `ATIVA` | concorrência | **CONCLUÍDO** |

Contrato OpenAPI provado: rota publicada com `200, 400, 401, 403, 404, 413, 422, 500`, `usuarioId` com `format: uuid`, cabeçalho CSRF obrigatório, `ErroSituacaoUsuarioDto` com conjunto fechado e resposta limitada a `usuarioId`, `ativo` e `inativadoEm` (também em `verify:openapi-runtime`). Zero drift de persistência e zero dependências novas.

#### 10.6.1 Reforço de provas pós-integração (REV. 17)

Integrado na `main` em 17/09/2026 — PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), commit `4d95c386d15d08060c319300a09a1f4eef5f3590` (somente `apps/api/test/integration/usuarios-situacao.integration.spec.ts`), merge commit **`bce22e8265317be16bf78fcb1861aafdc4b05b0c`** (merge commit, sem squash/rebase), 17/09/2026 03:54:24Z, CI verde na run `35179564801`. **Sem mudança normativa:** `D-2.3D-21` e os critérios `A-01`..`A-16` acima permanecem exatamente como homologados; as provas abaixo **reforçam** `A-12`/`A-13` (atomicidade) e `A-16` (concorrência), sem criar critério novo.

| Prova acrescentada | O que demonstra |
| --- | --- |
| Falha na revogação em massa | `500`; usuário permanece ativo, sessão permanece `ATIVA`, nenhum evento `usuario.situacao.alterada` (rollback conjunto) |
| C2 — ativação × inativação (5 rodadas) | Estado final coerente; um evento por transição real; sessão prévia `REVOGADA` pelo Administrador que inativou |
| C5 — revogação individual (`D-2.3D-19`) × inativação (5 rodadas) | Sessão revogada uma única vez; `usuario.sessao.revogacao` só existe se a revogação individual foi a autora |
| C6 — conclusão de recuperação (T-07) × inativação (5 rodadas) | Sem deadlock; nenhuma sessão `ATIVA`; se a inativação vence, senha não é trocada nem auditada |
| C7 — 10 `GET /auth/sessao` em voo | Nenhuma leitura em voo ressuscita a sessão; `401` após o commit |
| C3 determinístico | Inativação comitada entre a leitura do login e a emissão impede a sessão (ordem forçada, independente de agendamento) |
| C6 determinístico | Inativação comitada durante o Argon2 da recuperação produz `RECUPERACAO_INVALIDA` e preserva a senha |

**Mutation challenges** (aplicados e revertidos localmente; todos detectados pela suíte de integração):

| Mutação | Testes que falharam |
| --- | --- |
| M1 — ator derivado do path em vez da sessão | 13 |
| M2 — permissão exigida trocada (`usuarios.gerenciar` → `sessoes.revogar_terceiro`) | múltiplos |
| M3 — inativação sem revogação das sessões | 6 |
| M4 — auditoria fora da transação | 2 |
| M5 — lock do login sem `ativo = true` | 2 |
| M6 — `contexto` não homologado (`ativo_anterior`/`ativo_novo`) | 11 |
| M7a — `inativado_por_usuario_id = NULL` | 7 |
| M7b — sessões apagadas em vez de revogadas | 5 |

Verificação local no commit da fatia: `typecheck` verde; unitários 847/847; integração 443 aprovados e 2 ignorados preexistentes.

### 10.7 Critérios de aceite normativos de `P-2.3D-10` / `D-2.3D-22` (listagem de sessões ativas)

Provados contra PostgreSQL 18 real em `apps/api/test/integration/sessoes-listagem.integration.spec.ts`, com provas unitárias e de contrato complementares.

| ID | Critério de aceite | Estado |
| --- | --- | --- |
| `L-01` | Administrador com `sessoes.revogar_terceiro` lista as sessões ativas do alvo com `200` | **CONCLUÍDO** |
| `L-02` | Cada item expõe **exatamente** `sessaoId`, `criadaEm`, `ultimaAtividadeEm` e `expiraEm`; envelope só `sessoes` | **CONCLUÍDO** |
| `L-03` | Ordem por `criadaEm` crescente; `Cache-Control: no-store`; sem `Set-Cookie` | **CONCLUÍDO** |
| `L-04` | Token, segredo e `token_hash` nunca aparecem na resposta | **CONCLUÍDO** |
| `L-05` | Sessões `REVOGADA` e `EXPIRADA` persistidas não são listadas | **CONCLUÍDO** |
| `L-06` | Sessão `ATIVA` vencida por ociosidade não é listada **e não é escrita** pela consulta | **CONCLUÍDO** |
| `L-07` | Sessão no teto de expiração absoluta não é listada | **CONCLUÍDO** |
| `L-08` | Usuário inexistente e usuário sem sessões produzem o **mesmo** `200 {"sessoes":[]}` | **CONCLUÍDO** |
| `L-09` | Usuário autenticado sem a permissão recebe `403 ACESSO_NEGADO`, sem dados | **CONCLUÍDO** |
| `L-10` | Sem sessão, ou com sessão do operador revogada, `401 SESSAO_INVALIDA` | **CONCLUÍDO** |
| `L-11` | `usuarioId` estruturalmente inválido: `400 REQUISICAO_INVALIDA` | **CONCLUÍDO** |
| `L-12` | A consulta não emite evento de auditoria | **CONCLUÍDO** |
| `L-13` | Fluxo vertical: o `sessaoId` listado é revogado por `D-2.3D-19` e deixa de ser listado | **CONCLUÍDO** |
| `L-14` | Regressão: o `DELETE` de `D-2.3D-19` mantém a CSRF avaliada antes da sessão (`403 REQUISICAO_NAO_AUTORIZADA` sem cabeçalho e sem cookie) | **CONCLUÍDO** |
| `L-15` | Falha técnica na consulta devolve `500 {"erro":"FALHA_INTERNA"}`, sem detalhe interno *(REV. 19)* | **CONCLUÍDO** |
| `L-16` | A consulta não altera as sessões do alvo; a sessão do operador segue o registro de atividade de `D-2.3D-04` *(REV. 19)* | **CONCLUÍDO** |

**Mutation challenges** (aplicados, detectados e revertidos com conferência por SHA-256):

| Mutação | Testes que falharam |
| --- | --- |
| M1 — `@RequerPermissao` removido da listagem | `L-09`, `L-10` |
| M2 — predicados temporais removidos da consulta | `L-06`, `L-07` |
| M3 — `tokenHash` acrescentado ao item da resposta | `L-01`/`L-02`, `L-04` |
| M4 — `ProtecaoCsrfGuard` reordenado após as guards de sessão/permissão no `DELETE` | `L-14` e a prova unitária de ordem de guards |
| M5 — rota da listagem removida do escopo do `FiltroErroAutenticacao` *(REV. 19)* | `L-15` (corpo padrão do Nest em vez de `FALHA_INTERNA`) |
...
| ~~`P-2.3D-09`~~ | **Implementação de AUT-005 (ativação e inativação administrativa de usuário), conforme `D-2.3D-21`** | **CONCLUÍDA / INTEGRADA NA main em 17/09/2026 (PR [#52](https://github.com/BrunoMNoronha/techlab-fisio/pull/52))** — commits da fatia `7d16ded` (implementação e testes) e `dbef1fb9c4483bd2d9c261a77fb11270b7ffef70` (documentação), merge commit **`817da427de84fe452230df233cb4ae15c8a43593`** (merge commit, sem squash/rebase), 17/09/2026 03:08:37Z, CI verde na run `35176158028`. Rota `PATCH /auth/usuarios/:usuarioId/situacao` com RBAC, CSRF, revogação em massa atômica e auditoria `usuario.situacao.alterada`; critérios `A-01`..`A-16` (§10.6) atendidos; zero drift. **Reforço de provas integrado em 17/09/2026** (PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), merge `bce22e8`; §10.6.1) |
| ~~`P-2.3D-10`~~ | **Implementação da listagem administrativa das sessões ativas de um usuário (AUT-002 / `sessoes.revogar_terceiro`), conforme `D-2.3D-22`** | **CONCLUÍDA / INTEGRADA NA main em 17/09/2026 (PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64))** — PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), commits da fatia `254a677` (implementação e testes), `58c4741` (`docs/12` REV. 18) e `400377e` (`docs/10` REV. 49), merge commit **`bc6a763f3525b16384022ad5b43b95a542be5b2f`** (merge commit, sem squash/rebase, por auto-merge autorizado por Bruno Menezes Noronha), 17/09/2026 05:48:03Z, CI verde nas runs `35186845908` (push) e `35186860133` (pull_request). Rota `GET /auth/usuarios/:usuarioId/sessoes`; critérios `L-01`..`L-16` (§10.7); mutation challenges M1..M5; zero drift. Correções da revisão automatizada pós-merge registradas na REV. 19 |
...
| **AUT-002** — Encerrar, expirar e revogar sessões | `D-2.3D-01` (janela física), `D-2.3D-04` (expiração absoluta e ociosa), `D-2.3D-05` (estados terminais), `D-2.3D-18` (autoria da revogação em massa da T-07), **`D-2.3D-19` (revogação administrativa de terceiro — INTEGRADA na main em 14/09/2026, PR #46)**, **`D-2.3D-20` (consulta de sessão)**, **`D-2.3D-22` (listagem administrativa das sessões ativas — `P-2.3D-10`, INTEGRADA na main em 17/09/2026, PR #64)** | **PARCIAL.** Estados e transições preservados **sem alteração**: `ATIVA -> EXPIRADA`, `ATIVA -> REVOGADA`. **Implementados:** logout, expiração, revogação em massa da T-07, revogação administrativa de terceiro (`D-2.3D-19` / `P-2.3D-07`, PR #46) e consulta da sessão atual (`D-2.3D-20` / `P-2.3D-08`). **Inativação de usuário:** a revogação em massa decorrente é regida por `D-2.3D-21` (ver AUT-005) |
...
| **AUT-005** — Ativar e inativar usuários | `D-2.3D-05` (revogação), `D-2.3D-10` (papel do Administrador), **`D-2.3D-21` (contrato, auto-inativação, reativação, no-op, autoria e atomicidade)** | **MATERIALIZADA** — integrada na `main` em 17/09/2026 (`P-2.3D-09`, PR #52, merge `817da42`; §10.6). A revogação por inativação usa os mesmos estados terminais; sessões já vencidas fecham como `EXPIRADA` |
...
| REV. | Data | Conteúdo |
| --- | --- | --- |
| **19** | 17/09/2026 | **SINCRONIZAÇÃO PÓS-INTEGRAÇÃO DE `P-2.3D-10` / `D-2.3D-22` NA `main` (PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), MERGE `bc6a763`) E CORREÇÕES DA REVISÃO AUTOMATIZADA.** Nenhuma decisão criada, modificada, renumerada ou reaberta. **(a) Fatos:** PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), commits da fatia `254a677` (implementação e testes), `58c4741` (`docs/12` REV. 18) e `400377e` (`docs/10` REV. 49), merge commit **`bc6a763f3525b16384022ad5b43b95a542be5b2f`** (merge commit, sem squash/rebase, por auto-merge autorizado por Bruno Menezes Noronha), 17/09/2026 05:48:03Z, CI verde nas runs `35186845908` (push) e `35186860133` (pull_request). **(b) Correções:** revisão automatizada do Codex sobre `400377e`, recebida após o merge, com dois achados P2: **(1)** falha técnica da listagem saía no corpo padrão do Nest (`{"statusCode":500,"message":...}`) porque `FiltroErroAutenticacao` não reconhecia a rota — **defeito real, corrigido**: a rota exata `GET /auth/usuarios/:usuarioId/sessoes` entra no escopo fechado do filtro, com `L-15` (integração) e testes unitários de escopo; mutação que remove a rota do filtro detectada por `L-15`; **(2)** "a consulta não escreve nada" era afirmado sem ressalva, mas a guarda de sessão do **operador** registra atividade (e detecta expiração) como em toda rota autenticada, inclusive `GET /auth/sessao` (`D-2.3D-04`/`D-2.3D-20`) — **esclarecimento de escopo, sem alteração de comportamento nem de decisão**: a ausência de escrita refere-se às sessões do **alvo**; provado por `L-16`. **(c) Critérios:** §10.7 passa a `L-01`..`L-16` e M1..M5. Zero drift. |
| **18** | 17/09/2026 | **ACRÉSCIMO DE UMA DECISÃO NOVA — `D-2.3D-22` (§5.22): listagem administrativa das sessões ativas de um usuário (AUT-002), homologada por Bruno Menezes Noronha em 17/09/2026 (TLF-BASE-V1 §15, item 1) — E MATERIALIZAÇÃO DE `P-2.3D-10` EM BRANCH PRÓPRIA (`agent/sessoes-admin-next`, a partir de `origin/main` `63bb058`).** Nenhuma decisão anterior modificada, renumerada ou reaberta. **(a) Lacuna:** a revogação de `D-2.3D-19` exigia um `sessao_id` que nenhuma rota permitia descobrir. **(b) Decisão:** opção conservadora — `GET /auth/usuarios/:usuarioId/sessoes` sob `sessoes.revogar_terceiro`, método seguro sem CSRF, somente sessões ativas e temporalmente válidas, quatro campos, sem paginação, lista vazia para usuário inexistente, sem auditoria. **(c) Implementação:** `SessoesController` (prefixo `auth`, CSRF por handler no `DELETE`, ordem de guards preservada), `SessoesService.listarSessoesAtivasDoUsuario`, `SessaoAtivaUsuarioDto`/`ListarSessoesUsuarioRespostaDto`. **(d) Provas:** §10.7 `L-01`..`L-14`, M1..M4. **(e) Zero drift:** schema, migrations, golden, protected-objects, dependências, CI e Playwright intocados. **Não integrada.** |
| **17** | 17/09/2026 | **REGISTRO PÓS-INTEGRAÇÃO DO REFORÇO DE PROVAS DE `P-2.3D-09` / `D-2.3D-21` NA `main` (PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), MERGE `bce22e8`).** Atualização **exclusivamente factual**; nenhuma decisão criada, modificada, renumerada ou reaberta — `D-2.3D-01`..`D-2.3D-21` permanecem exatamente como homologadas. **(a) Fatos:** PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), commit `4d95c386d15d08060c319300a09a1f4eef5f3590` (somente `apps/api/test/integration/usuarios-situacao.integration.spec.ts`), merge commit **`bce22e8265317be16bf78fcb1861aafdc4b05b0c`** (merge commit, sem squash/rebase), 17/09/2026 03:54:24Z, CI verde na run `35179564801`. **(b) §10.6.1:** provas acrescentadas (rollback por falha na revogação; C2, C5, C6, C7; C3 e C6 determinísticos) e mutation challenges M1..M7 (M7a/M7b) detectados. **(c) §11:** `P-2.3D-09` permanece **CONCLUÍDA / INTEGRADA NA `main`**, com nota do reforço. **(d) Demais pendências:** `P-2.3D-01`, `P-2.3D-05`, `P-2.3D-06` e `Q-02` inalteradas; `P2.2-05` permanece **NÃO INICIADA**. |
| **16** | 17/09/2026 | **SINCRONIZAÇÃO PÓS-INTEGRAÇÃO DE `P-2.3D-09` / `D-2.3D-21` NA `main` (PR [#52](https://github.com/BrunoMNoronha/techlab-fisio/pull/52), MERGE `817da42`).** Atualização **exclusivamente factual**; nenhuma decisão criada, modificada, renumerada ou reaberta — `D-2.3D-01`..`D-2.3D-21` permanecem exatamente como homologadas. **(a) Fatos:** commits da fatia `7d16ded` (implementação e testes) e `dbef1fb9c4483bd2d9c261a77fb11270b7ffef70` (documentação), merge commit **`817da427de84fe452230df233cb4ae15c8a43593`** (merge commit, sem squash/rebase), 17/09/2026 03:08:37Z, CI verde na run `35176158028`. **(b) Status:** em §11, `P-2.3D-09` passa a **CONCLUÍDA / INTEGRADA NA `main`**; em §12, AUT-005 passa a **MATERIALIZADA**. **(c) Demais pendências:** `P-2.3D-01`, `P-2.3D-05`, `P-2.3D-06` e `Q-02` inalteradas; `P2.2-05` permanece **NÃO INICIADA**. |
| **15** | 16/09/2026 | **ACRÉSCIMO DE UMA DECISÃO NOVA — `D-2.3D-21` (§5.21): ativação e inativação administrativa de usuário (AUT-005), homologada por Bruno Menezes Noronha em 16/09/2026 (TLF-BASE-V1 §15, item 1) — E MATERIALIZAÇÃO DE `P-2.3D-09` EM BRANCH PRÓPRIA.** Nenhuma decisão anterior foi modificada, renumerada, reduzida ou reaberta: `D-2.3D-01`..`D-2.3D-20` permanecem exatamente como homologadas. **(a) `D-2.3D-21`:** `PATCH /auth/usuarios/:usuarioId/situacao` sob `usuarios.gerenciar` e `ProtecaoCsrfGuard`; ator só da sessão; corpo estrito `{ ativo }`; `D21-01` auto-inativação → `422`; `D21-02` reativação limpa metadados e não cria sessões; `D21-03` no-op → `200` sem mutação nem auditoria; inativação revoga em massa com o Administrador executor como autor (vencidas → `EXPIRADA`); `usuario.situacao.alterada` atômica com `contexto` vazio e rollback conjunto. **(b) `P-2.3D-09`:** implementação na branch `agent/aut-005-situacao-usuario`; critérios `A-01`..`A-16` (§10.6) atendidos. **(c) §12:** AUT-005 passa a **MATERIALIZADA EM BRANCH PRÓPRIA**. **(d) Não ampliação:** nenhuma permissão, ação de auditoria, chave de `contexto`, migration ou dependência nova. `P-2.3D-01`, `P-2.3D-05`, `P-2.3D-06` e `Q-02` inalteradas; `P2.2-05` permanece **NÃO INICIADA**. |
...
**Fim — `docs/12-decisoes-autenticacao-autorizacao.md` — `D-2.3D-01`..`D-2.3D-21` registradas; F0 da Etapa 2.3D-B executada em 27/08/2026; `D-2.3D-13` homologada em 28/08/2026; `D-2.3D-14` e `D-2.3D-15` homologadas em 31/08/2026; `D-2.3D-16`, `D-2.3D-17` e `D-2.3D-18` homologadas em 05/09/2026; F0..F7 CONCLUÍDAS e Etapa 2.3D ENCERRADA em 05/09/2026; `D-2.3D-19` homologada em 06/09/2026 e integrada na `main` em 14/09/2026 (PR #46); `D-2.3D-20` homologada em 07/09/2026 e integrada na `main` em 15/09/2026 (PR #47); REV. 14 (15/09/2026) registra a integração de `D-2.3D-20` e da prova E2E real same-origin (`P-2.3D-08`, PR #47, merge `89fa492`) e o encerramento factual de `P-2.3D-04`; `D-2.3D-21` homologada em 16/09/2026 e integrada na `main` em 17/09/2026 (`P-2.3D-09`, PR #52, merge `817da42`; REV. 16); reforço de provas integrado em 17/09/2026 (PR #55, merge `bce22e8`; REV. 17).**
