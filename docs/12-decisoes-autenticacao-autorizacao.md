...
| `R-2.3D-03` | Atualização de `ultima_atividade_em` implementada como last-writer-wins, permitindo regressão temporal e estendendo sessão indevidamente | Média | `D-2.3D-04` exige monotonicidade atômica; a CHECK física impede apenas sair da janela, **não** a regressão dentro dela — a prova é da F2 |
