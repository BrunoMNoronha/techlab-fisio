...
| `disponibilidade_profissional` | `vigencia_fim IS NULL OR vigencia_fim >= vigencia_inicio` (`ck_disponibilidade_profissional_vigencia`) | **`D-PRO3-06`** (`docs/16` §4.6) — coerência das datas da versão; acréscimo posterior à Etapa 2.2. *Acrescentada na REV. 2.4* |
...
| IDX-D1 | `disponibilidade_profissional` | `btree (profissional_id, vigencia_inicio)` — `ix_disponibilidade_profissional_vigencia` | Leitura da **versão aplicável** a uma data (passo 9 de T-01) e de todas as versões do profissional em ordem de vigência | **`D-PRO3-04`**, `D-PRO3-06`; PRO-003. *Acrescentado na REV. 2.4* |
...
| **Índice único sobre expressão** | U-14 (`ux_servico_clinica_nome`, `(clinica_id, lower(btrim(nome)))`) — *acrescentado na REV. 2.3* | O schema declarativo não expressa índice sobre `lower(btrim(...))`; vive na migration `20260917120000_servico_catalogo_invariantes`, protegido pelo golden e por teste de integração |
| **Invariante entre colunas de data** | `ck_disponibilidade_profissional_vigencia` — *acrescentada na REV. 2.4* | Caso particular de "CHECK constraints" (linha acima), registrado à parte porque acompanha a mudança de nulidade de `vigencia_inicio`: a coluna **é** representável no schema declarativo, a CHECK **não**. Vive na migration `20260917210000_disponibilidade_profissional_vigencia`, protegida pelo golden e por teste de efeito na Guarda 2 |
