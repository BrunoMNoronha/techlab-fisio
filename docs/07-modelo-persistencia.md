...
> **Status:** **HOMOLOGADO — REV. 2 — ETAPA 2.2 CONCLUÍDA** *(correções editoriais `DIV-04`/`DIV-05` aplicadas na REV. 2.1, 25/08/2026; absorção editorial de `D-2.3D-01` aplicada na REV. 2.2, 27/08/2026; absorção editorial de `D-CFG-22`/`D-CFG-23` aplicada na REV. 2.3, 17/09/2026; absorção editorial de `D-CFG-34`/`D-CFG-35` e `D-CFG-46`/`D-CFG-47` aplicada na REV. 2.4, 17/09/2026; absorção editorial de `D-PRO3-01`/`D-PRO3-06` aplicada na REV. 2.5, 17/09/2026; absorção editorial de `D-AGD-07` aplicada na REV. 2.6, 17/09/2026 — ver §1.3; nenhuma decisão homologada da Etapa 2.2 alterada)*
...
- **REV. 2.3 (editorial) — 17/09/2026 — `CFG-003`.** Absorção exclusivamente **editorial** das decisões **`D-CFG-22`** e **`D-CFG-23`**, homologadas por Bruno Menezes Noronha em 17/09/2026 e registradas em **`docs/14-decisoes-configuracao-clinica.md` §3.11.1–§3.11.2** — cuja implementação física **já está integrada na `main`** (migration `20260917120000_servico_catalogo_invariantes`, PR #80, merge commit `fd3c205`). Atualizados: **§7.2** (`servico` passa a citar as restrições físicas), **§10.1** (unicidade `U-14`, `ux_servico_clinica_nome`), **§10.2** (`ck_servico_duracao_positiva`, `ck_servico_preco_nao_negativo`, `ck_servico_situacao`), **§28.1** e **§28.2** (U-14 na Categoria B — correção pós-integração da revisão do PR #83; as CHECKs já estavam cobertas por "todas de §10.2") (pendência `P-CFG-D07-01`). **Nada da Etapa 2.2 foi reaberto, revogado ou reinterpretado**; a descrição de `servico` até a REV. 2.2 continua correta — apenas deixou de ser completa. Esta revisão **não cria norma**: a fonte normativa é `docs/14`; `docs/07` passa a refletir o estado físico vigente (TLF-BASE-V1 §4.8), no mesmo precedente da REV. 2.2. §10-A não muda: os índices das unicidades de §10.1 não são repetidos ali.
- **REV. 2.4 (editorial) — 17/09/2026 — `CFG-004` e `CFG-005`.** Absorção exclusivamente **editorial** das decisões **`D-CFG-34`**/**`D-CFG-35`** e **`D-CFG-46`**/**`D-CFG-47`**, homologadas por Bruno Menezes Noronha em 17/09/2026 e registradas em **`docs/14-decisoes-configuracao-clinica.md` §3.12.1–§3.12.2 e §3.13.1–§3.13.2** — cuja implementação física **já está integrada na `main`** (migrations `20260917170000_forma_pagamento_invariantes` e `20260917180000_motivo_cancelamento_invariantes`, PR [#86](https://github.com/BrunoMNoronha/techlab-fisio/pull/86), merge commit `e17024c`). Ambas as decisões condicionavam este alinhamento à integração da migration. Atualizados: **§7.2** (`forma_pagamento` e `motivo_cancelamento` passam a citar as restrições físicas), **§10.1** (unicidades `U-15` e `U-16`), **§10.2** (`ck_forma_pagamento_situacao` e `ck_motivo_cancelamento_situacao`), **§28.1** (pendência `P-CFG-D07-02`) e **§28.2** (Categoria B — índice único sobre expressão). **Nada da Etapa 2.2 foi reaberto, revogado ou reinterpretado**; a descrição das duas tabelas até a REV. 2.3 continua correta — apenas deixou de ser completa. **Nenhuma restrição foi criada em `agendamento` ou `historico_agendamento`** (`D-CFG-51`). Esta revisão **não cria norma**: a fonte normativa é `docs/14`; `docs/07` passa a refletir o estado físico vigente (TLF-BASE-V1 §4.8), no mesmo precedente das REV. 2.2 e 2.3. §10-A não muda: os índices das unicidades de §10.1 não são repetidos ali.
- **REV. 2.5 (editorial) — 17/09/2026 — `PRO-003`.** Absorção exclusivamente **editorial** das decisões **`D-PRO3-01`** e **`D-PRO3-06`**, homologadas por Bruno Menezes Noronha em 17/09/2026 e registradas em **`docs/16-pacote-decisao-disponibilidade-profissional.md` §4.1 e §4.6** — cuja implementação física está na migration `20260917210000_disponibilidade_profissional_vigencia`, **implementada e comprovada** (reconstrução limpa, golden byte a byte e efeito provado na Guarda 2), em branch local **ainda não publicada**. Atualizados: **§7.3** (`disponibilidade_profissional` passa a declarar `vigencia_inicio` obrigatória e a semântica de versão), **§10.2** (`ck_disponibilidade_profissional_vigencia`), **§10-A** (índice `IDX-D1`) e **§28.2** (Categoria B). **Nada da Etapa 2.2 foi reaberto, revogado ou reinterpretado**; a descrição de `disponibilidade_profissional` até a REV. 2.3 continua correta — apenas deixou de ser completa, e `vigencia_inicio` deixou de ser anulável. Esta revisão **não cria norma**: a fonte normativa é `docs/16`; `docs/07` passa a refletir o estado físico vigente (TLF-BASE-V1 §4.8), no mesmo precedente das REV. 2.2 e 2.3.
- **REV. 2.6 (editorial) — 17/09/2026 — `AGD-A`.** Absorção exclusivamente **editorial** da decisão **`D-AGD-07`**, homologada por Bruno Menezes Noronha em 17/09/2026 e registrada em **`docs/15-pacote-decisao-agenda.md` §4.7** — cuja implementação física está na migration `20260917230000_agendamento_cancelamento_coerente`, **implementada e comprovada** (reconstrução limpa, golden byte a byte e efeito provado na Guarda 2), em branch local **ainda não publicada**. Atualizados: **§7.5** (`agendamento` passa a citar a restrição física), **§10.2** (`ck_agendamento_cancelamento_coerente`) e **§28.2** (Categoria B). **Esta revisão SUPERA, no ponto específico, a afirmação da REV. 2.4 de que "nenhuma restrição foi criada em `agendamento`"** — aquela afirmação registrava `D-CFG-51`, que a decisão posterior `D-AGD-07` resolve expressamente; a nota da REV. 2.4 é **preservada como registro histórico** e recebe remissão a esta. **Nada da Etapa 2.2 foi reaberto, revogado ou reinterpretado**; nenhuma restrição foi criada em `historico_agendamento`. Esta revisão **não cria norma**: a fonte normativa é `docs/15`; `docs/07` passa a refletir o estado físico vigente (TLF-BASE-V1 §4.8).
...
**`servico`** — `id PK`; `clinica_id FK NN`; `nome text NN`; `duracao_min integer NN`; `preco_referencia numeric(12,2) NN`; `ativo boolean NN`; `inativado_em timestamptz ∅`. **O preço de referência nunca reescreve cobrança histórica** (CFG-003) — a cobrança copia o valor na criação. **Restrições físicas (REV. 2.3, `D-CFG-22`/`D-CFG-23`):** nome único por clínica, ativos e inativos, sem distinção de caixa e sem espaços de borda (`U-14`); `duracao_min > 0`; `preco_referencia >= 0`; `ativo = (inativado_em IS NULL)` (§10.2). *(CFG-003, RN-008)*

**`forma_pagamento`**, **`motivo_cancelamento`** — `id PK`; `clinica_id FK NN`; descrição; `ativo boolean NN`; `inativado_em timestamptz ∅`. **Restrições físicas (REV. 2.4):** descrição única por clínica, ativas e inativas, sem distinção de caixa e sem espaços de borda — `U-15` (`D-CFG-34`/`D-CFG-35`) e `U-16` (`D-CFG-46`/`D-CFG-47`); `ativo = (inativado_em IS NULL)` em ambas (§10.2). Nenhuma restrição nova em `agendamento` ou `historico_agendamento` (`D-CFG-51`). *(CFG-004, CFG-005, RN-007)*
...
**`disponibilidade_profissional`** — `id PK`; `profissional_id FK NN`; `dia_semana smallint NN`; `hora_inicio time NN`; `hora_fim time NN`; **`vigencia_inicio date NN`**; `vigencia_fim date ∅`. Mudanças criam novas vigências; **não reescrevem atendimentos passados** (PRO-003). Uma **versão** é o conjunto de linhas do mesmo profissional com o mesmo par (`vigencia_inicio`, `vigencia_fim`) — **não há tabela de versão**; `vigencia_fim NULL` é a versão **aberta** (`D-PRO3-01`). *`vigencia_inicio` passou a obrigatória na REV. 2.5 (`D-PRO3-06`, migration `20260917210000_disponibilidade_profissional_vigencia`); a não sobreposição entre versões e entre janelas é garantida no backend sob `SELECT ... FOR UPDATE` do profissional (`D-PRO3-07`), não fisicamente.*
...
`CHECK ((estado = 'CANCELADO') = (motivo_cancelamento_id IS NOT NULL AND cancelado_em IS NOT NULL AND cancelado_por_usuario_id IS NOT NULL))` — `ck_agendamento_cancelamento_coerente`, **`D-AGD-07`** (`docs/15` §4.7), migration `20260917230000_agendamento_cancelamento_coerente`. *Acrescentada na REV. 2.6.*
...
| **U-14** | **`servico (clinica_id, lower(btrim(nome)))`** — `ux_servico_clinica_nome`, índice único sobre expressão | **`D-CFG-22`** (`docs/14` §3.11.1) — abrange serviços **ativos e inativos**; o nome de serviço inativo não é liberado para reuso (mesmo racional de `H2.2-13`). *Acrescentada na REV. 2.3* |
| **U-15** | **`forma_pagamento (clinica_id, lower(btrim(descricao)))`** — `ux_forma_pagamento_clinica_descricao`, índice único sobre expressão | **`D-CFG-34`**/**`D-CFG-35`** (`docs/14` §3.12.1–§3.12.2) — abrange formas **ativas e inativas**; a descrição de forma inativa não é liberada para reuso (o caminho é a reativação). *Acrescentada na REV. 2.4* |
| **U-16** | **`motivo_cancelamento (clinica_id, lower(btrim(descricao)))`** — `ux_motivo_cancelamento_clinica_descricao`, índice único sobre expressão | **`D-CFG-46`**/**`D-CFG-47`** (`docs/14` §3.13.1–§3.13.2) — abrange motivos **ativos e inativos**; a descrição de motivo inativo não é liberada para reuso. *Acrescentada na REV. 2.4* |
...
| `disponibilidade_profissional` | `vigencia_fim IS NULL OR vigencia_fim >= vigencia_inicio` (`ck_disponibilidade_profissional_vigencia`) | **`D-PRO3-06`** (`docs/16` §4.6) — coerência das datas da versão; acréscimo posterior à Etapa 2.2. *Acrescentada na REV. 2.5* |
...
| `servico` | `duracao_min > 0` (`ck_servico_duracao_positiva`); `preco_referencia >= 0` (`ck_servico_preco_nao_negativo`) | **`D-CFG-23`** (`docs/14` §3.11.2) — acréscimo posterior à Etapa 2.2 (ver nota abaixo) |
| `servico` | `ativo = (inativado_em IS NULL)` (`ck_servico_situacao`) | **`D-CFG-23`** — coerência da situação (RN-007/RN-008); acréscimo posterior à Etapa 2.2 |
| `forma_pagamento` | `ativo = (inativado_em IS NULL)` (`ck_forma_pagamento_situacao`) | **`D-CFG-35`** (`docs/14` §3.12.2) — coerência da situação; acréscimo posterior à Etapa 2.2 (ver nota abaixo) |
| `motivo_cancelamento` | `ativo = (inativado_em IS NULL)` (`ck_motivo_cancelamento_situacao`) | **`D-CFG-47`** (`docs/14` §3.13.2) — coerência da situação; acréscimo posterior à Etapa 2.2 (ver nota abaixo) |
| `agendamento` | `(estado = 'CANCELADO') = (motivo_cancelamento_id IS NOT NULL AND cancelado_em IS NOT NULL AND cancelado_por_usuario_id IS NOT NULL)` (`ck_agendamento_cancelamento_coerente`) | **`D-AGD-07`** (`docs/15` §4.7) — coerência do cancelamento; acréscimo posterior à Etapa 2.2 (ver nota abaixo). *Acrescentada na REV. 2.6* |
...
> **Nota editorial (REV. 2.3).** As três CHECKs de `servico` e a unicidade `U-14` foram acrescentadas pelas decisões **`D-CFG-22`/`D-CFG-23`** (`docs/14` §3.11) e implementadas na migration `20260917120000_servico_catalogo_invariantes`. `preco_referencia >= 0` admite `0.00` por decisão expressa (`D-CFG-27`) e é coerente com `cobranca.valor_bruto >= 0`, que copia o preço. Nenhuma das CHECKs anteriores foi alterada, renomeada ou removida.

> **Nota editorial (REV. 2.4).** As unicidades `U-15`/`U-16` e as CHECKs `ck_forma_pagamento_situacao`/`ck_motivo_cancelamento_situacao` foram acrescentadas pelas decisões **`D-CFG-34`/`D-CFG-35`** (`docs/14` §3.12) e **`D-CFG-46`/`D-CFG-47`** (`docs/14` §3.13) e implementadas nas migrations `20260917170000_forma_pagamento_invariantes` e `20260917180000_motivo_cancelamento_invariantes`. As duas unicidades seguem o mesmo racional de `U-14`: comparação sem distinção de caixa e sem espaços de borda, abrangendo linhas ativas e inativas, de modo que a descrição de uma linha inativa **não** seja liberada para reuso. As duas CHECKs materializam a mesma coerência de situação já registrada para `servico`. **Nenhuma restrição foi criada em `agendamento` ou `historico_agendamento`** (`D-CFG-51`): as FKs anuláveis `agendamento.motivo_cancelamento_id` e `historico_agendamento.motivo_cancelamento_id` permanecem exatamente como estavam. Nenhuma CHECK, unicidade ou índice anterior foi alterado, renomeado ou removido.

> **Nota editorial (REV. 2.5).** `ck_disponibilidade_profissional_vigencia` foi acrescentada pela decisão **`D-PRO3-06`** (`docs/16` §4.6) e implementada na migration `20260917210000_disponibilidade_profissional_vigencia`, junto com `vigencia_inicio SET NOT NULL` e o índice `IDX-D1` de §10-A. A igualdade é admitida (`>=`): uma versão de **um único dia** é legítima. **Deliberadamente NÃO criadas**: exclusion constraint de sobreposição entre versões ou entre janelas, unicidade artificial das janelas, tabela de versão, trigger, função, coluna de versão otimista e coluna de autoria — a não sobreposição é regra de backend sob lock (`D-PRO3-07`), mesmo racional de `D-CFG-16`, e a autoria é limitação declarada de `D-PRO3-09`. As duas CHECKs anteriores da tabela não foram alteradas, renomeadas ou removidas.

> **Nota editorial (REV. 2.6).** `ck_agendamento_cancelamento_coerente` foi acrescentada pela decisão **`D-AGD-07`** (`docs/15` §4.7) e implementada na migration `20260917230000_agendamento_cancelamento_coerente`. Ela materializa AGD-003 ("motivo padronizado") no BANCO: o estado `CANCELADO` passa a exigir `motivo_cancelamento_id`, `cancelado_em` e `cancelado_por_usuario_id` preenchidos **juntos**, e nenhum outro estado pode carregar os três. **Ela SUPERA, no ponto específico, a afirmação da nota da REV. 2.4** — "nenhuma restrição foi criada em `agendamento`" (`D-CFG-51`) —, que fica como registro do estado então vigente: `D-AGD-07` resolve `D-CFG-51` expressamente. **`historico_agendamento` não foi tocada**: sua FK anulável `motivo_cancelamento_id` permanece exatamente como estava. **Limite declarado da expressão homologada:** a comparação é com a CONJUNÇÃO dos três campos, de modo que um resíduo PARCIAL em agendamento não cancelado (apenas `cancelado_em`, por exemplo) mantém a conjunção falsa e **não** é rejeitado; nenhum caminho da aplicação o produz — o cancelamento grava os quatro campos numa única instrução — e o limite é registrado em teste de efeito na Guarda 2. **Deliberadamente NÃO criadas**: coluna de justificativa, coluna de `confirmado_em`/`checkin_em`, enum de operação do histórico, trigger e qualquer restrição em `historico_agendamento`. Nenhuma CHECK, unicidade, índice ou exclusion constraint anterior foi alterado, renomeado ou removido.
...
| IDX-D1 | `disponibilidade_profissional` | `btree (profissional_id, vigencia_inicio)` — `ix_disponibilidade_profissional_vigencia` | Leitura da **versão aplicável** a uma data (passo 9 de T-01) e de todas as versões do profissional em ordem de vigência | **`D-PRO3-04`**, `D-PRO3-06`; PRO-003. *Acrescentado na REV. 2.5* |
...
| ~~**`P-CFG-D07-01`**~~ | Absorção editorial de `D-CFG-22`/`D-CFG-23` (`U-14` `ux_servico_clinica_nome`, `ck_servico_duracao_positiva`, `ck_servico_preco_nao_negativo`, `ck_servico_situacao`) neste documento | **ENCERRADA em 17/09/2026** — absorvida na REV. 2.3 (§1.3) |
| ~~**`P-CFG-D07-02`**~~ | Absorção editorial de `D-CFG-35`/`D-CFG-47` (`U-15` `ux_forma_pagamento_clinica_descricao`, `U-16` `ux_motivo_cancelamento_clinica_descricao`, `ck_forma_pagamento_situacao`, `ck_motivo_cancelamento_situacao`) neste documento | **ENCERRADA em 17/09/2026** — absorvida na REV. 2.4 (§1.3), após a integração das migrations na `main` pela PR [#86](https://github.com/BrunoMNoronha/techlab-fisio/pull/86) |
...
| **Índice único sobre expressão** | U-14 (`ux_servico_clinica_nome`, `(clinica_id, lower(btrim(nome)))`) — *acrescentado na REV. 2.3*; U-15 (`ux_forma_pagamento_clinica_descricao`) e U-16 (`ux_motivo_cancelamento_clinica_descricao`), ambos `(clinica_id, lower(btrim(descricao)))` — *acrescentados na REV. 2.4* | O schema declarativo não expressa índice sobre `lower(btrim(...))`; vive na migration `20260917120000_servico_catalogo_invariantes`, protegido pelo golden e por teste de integração |
| **Invariante entre colunas de data** | `ck_disponibilidade_profissional_vigencia` — *acrescentada na REV. 2.5* | Caso particular de "CHECK constraints" (linha acima), registrado à parte porque acompanha a mudança de nulidade de `vigencia_inicio`: a coluna **é** representável no schema declarativo, a CHECK **não**. Vive na migration `20260917210000_disponibilidade_profissional_vigencia`, protegida pelo golden e por teste de efeito na Guarda 2 |
| **Equivalência entre estado e colunas de encerramento** | `ck_agendamento_cancelamento_coerente` — *acrescentada na REV. 2.6* | Caso particular de "CHECK constraints" (linha acima), registrado à parte por ser a primeira restrição física acrescentada a `agendamento` depois da Etapa 2.2. Vive na migration `20260917230000_agendamento_cancelamento_coerente`, protegida pelo golden e por teste de efeito na Guarda 2 |
