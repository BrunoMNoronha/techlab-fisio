...
> **Revisão vigente:** REV. 34 (05/09/2026) — **ETAPA 2.3D-B / F7 (consolidação e revisão independente): EXECUTADA — ETAPA 2.3D NÃO ENCERRADA** (§6-Q). **Nenhuma alteração de código, teste, schema, migration, OpenAPI, dependência ou configuração; nenhuma decisão normativa criada — `D-2.3D-01`..`D-2.3D-17` intactas.** A revisão do conjunto integrado F1..F6 e a passagem adversarial sobre 17 hipóteses **não encontraram defeito de código novo**. Encontraram, porém, **`Q-01` — achado BLOQUEANTE de natureza normativa**: `docs/06` §12.7 define T-07 **exclusivamente** como “Recuperação de senha / revogação”, e `docs/09` §5 atribui `usuario.sessao.revogacao` à revogação “em massa **via T-07**” com ator **Administrador**, enquanto a linha seguinte audita a **mesma** transação como `usuario.senha.recuperacao_concluida` com ator **Usuário** — e na conclusão **não existe Administrador autenticado**. O conflito decide simultaneamente **`L-F6-06`** (autoria de `revogada_por_usuario_id`) e **`L-F6-07`** (emitir ou não o evento adicional), e foi **devolvido a Bruno** (TLF-BASE-V1 §15), registrado em `docs/12` §11. **Uma promoção foi redigida e RETIRADA nesta fatia:** `L-F6-06` chegou a ser promovida a `D-2.3D-18`, e a revisão da PR #22 mostrou que o fundamento não decide — **nenhuma `D-2.3D-18` existe**. Bateria integral verde no host: `typecheck` 0; `build` 0; `test:api` **24 · 595**; `verify:api-integration` **10 · 384 + 2 pulados**; `test:integration`/`verify:from-scratch` **10 · 87**; OpenAPI **19/19**; Argon2, `smoke:api`, `lint:migrations` (10 · 37) e `schema:verify` (golden 60 169 bytes; `migrate diff` 0) verdes. **`P-2.3D-03` ENCERRADA**; baseline de §3 corrigida. Matriz normativa sem item `SEM PROVA` ou `INCONSISTENTE`; **três `PARCIAL` declarados** — AUT-002, AUT-003 e a auditoria da T-07. **Bloqueador único do encerramento: `Q-01`.** AUT-005 permanece NÃO MATERIALIZADA; `P2.2-05`, NÃO INICIADA.
...
## 6-Q. Etapa 2.3D-B / F7 — consolidação e revisão independente da Etapa 2.3D
...
**Resultado: a Etapa 2.3D NÃO é encerrada nesta fatia.** A revisão independente não encontrou defeito de código novo, mas encontrou um **conflito entre fontes normativas** (`Q-01`) que decide simultaneamente `L-F6-06` e `L-F6-07` e que **nenhum agente pode resolver por interpretação** (TLF-BASE-V1 §15). O comportamento vigente foi integralmente preservado.
...
### 6-Q.2 `L-F6-06` — apuração, tentativa de fechamento e o conflito `Q-01`
...
**O critério que a apuração sugeriu** foi: o campo registra **quem praticou o ato do qual a revogação decorreu**, e esse ator é o que `docs/09` §5 já fixa para o evento de auditoria correspondente. O critério é sólido — mas **não decide**, porque a fonte não diz qual é o evento correspondente.

#### `Q-01` — o conflito entre fontes

| Fonte | O que diz |
| --- | --- |
| `docs/06` §12.7 | **T-07 = "Recuperação de senha / revogação (M1) — Invalidar segredo + nova senha + revogar sessões"**. É a **única** T-07 do projeto |
...
| Implementação (F6) | emite **apenas** `usuario.senha.recuperacao_concluida`; grava `revogada_por_usuario_id = usuario_id` |

Como T-07 é, por definição, a recuperação de senha, a expressão "em massa via T-07" na primeira linha **refere-se a este fluxo** — e atribui-lhe ator **Administrador**. A segunda linha audita a **mesma transação** com ator **Usuário**. Na conclusão, porém, **não existe Administrador autenticado**: a rota é sem sessão, e o Administrador agiu até 30 minutos antes.

Uma leitura anterior desta fatia supôs que "via T-07" descrevia apenas a mecânica transacional de uma futura revogação administrativa de terceiro. A revisão da PR #22 demonstrou que essa leitura **não se sustenta** diante de `docs/06` §12.7. O registro fica como está: a suposição foi retirada, não corrigida silenciosamente.

**Duas questões abertas, ambas decorrentes de `Q-01`:**

1. a T-07 deve emitir **também** `usuario.sessao.revogacao`? (`L-F6-07`)
2. qual ator alimenta `revogada_por_usuario_id` na revogação em massa? (`L-F6-06`)

**Alternativas para `revogada_por_usuario_id`, avaliadas:**

| Alternativa | Fidelidade semântica | Auditoria | Compatibilidade | Custo | Situação |
| --- | --- | --- | --- | --- | --- |
| **A — próprio usuário** (vigente) | Coerente **se** o evento correspondente for `usuario.senha.recuperacao_concluida`; derivada da própria linha, impossível forjar | Consistente com esse evento | Preserva a invariante `REVOGADA ⟺ preenchido` | zero | **vigente; depende de `Q-01`** |
| B — Administrador iniciador | Coerente **se** o evento correspondente for `usuario.sessao.revogacao` (ator Administrador). Contra: ele iniciou a emissão do segredo; a revogação só ocorre se e quando alguém concluir, e pode nunca ocorrer | Alinharia com a linha 68 de `docs/09` §5 | Preserva a invariante; exigiria propagar `criado_por_usuario_id` até a revogação | baixo (código) | **depende de `Q-01`** |
| C — `NULL` | Colide com o significado vigente (`ATIVA`/`EXPIRADA`); afirmaria "sistema", que o modelo não representa | Perde o autor | **Quebra** a invariante | zero, mas perda semântica | rejeitada |
| D — coluna/enum de origem | Marginalmente mais expressiva | — | Exigiria migration e drift de persistência | migration + golden | rejeitada — o contrato vigente representa o requisito |

**Classificação: decisão dependente do PO.** Nenhuma decisão normativa foi criada. **`L-F6-06` e `L-F6-07` permanecem abertas**, e `Q-01` é registrada em `docs/12` §11 como pendência **bloqueante do encerramento da Etapa 2.3D**. O comportamento vigente é preservado sem alteração.

A prova regressiva do comportamento vigente existe e continua verde: `recuperacao-senha.integration.spec.ts:678` (`revogada_por_usuario_id = alvo.id` após a T-07) e `:701` (`NULL` na sessão fechada como `EXPIRADA`); `sessao.integration.spec.ts:477` (logout), `:176`/`:424` (`NULL` em `ATIVA`) e `:511`/`:525` (`NULL` em `EXPIRADA`).
...
| --- | --- | --- | --- |
| `Q-01` | **Bloqueante (normativo)** | Conflito entre `docs/09` §5 e `docs/06` §12.7 sobre a auditoria e a autoria da revogação em massa da T-07 | **DEVOLVIDO A BRUNO** — `docs/12` §11 |
| `Q-02` | P3 (documental) | `docs/10` §3 descrevia a baseline normativa como `D-2.3D-01`..`D-2.3D-13`, desatualizada desde a F5 | **CORRIGIDO** |
| `Q-03` | P3 (documental) | `P-2.3D-03` constava **ABERTA**, embora medida e verde desde a F3 | **ENCERRADA** em `docs/12` §11; `D-2.3D-11` não alterada |
| `Q-04` | P3 (observação) | A T-07 calcula `agora` em `concluir` e a revogação em massa calcula o seu próprio em `revogarTodasDoUsuarioEm` | **Sem efeito** — a diferença é de microssegundos e a ordem resultante reflete a ordem real das operações. Não corrigido |
...
| **T-07** | transação única | `:952`, `:997`, `:1209` | **`PARCIAL`** — a atomicidade está provada; a **auditoria** da revogação depende de `Q-01` |

**Nenhum item `SEM PROVA` ou `INCONSISTENTE`.** Os três `PARCIAL` são declarados, e nenhum decorre de defeito: dois são escopo não atribuído (AUT-002/AUT-003) e um é `Q-01`.

### 6-Q.5 Pendências que permanecem abertas

| Pendência | Estado | Razão |
| --- | --- | --- |
| **`Q-01`** | **ABERTA — BLOQUEANTE** | Conflito de fontes sobre a auditoria/autoria da revogação da T-07. Decide `L-F6-06` e `L-F6-07`. **Exige decisão de Bruno** |
...
```text
ETAPA 2.3D — NÃO ENCERRADA

F0..F6 = CONCLUÍDAS
F7     = EM CURSO — revisão independente executada; encerramento BLOQUEADO por Q-01

decisões normativas = D-2.3D-01..D-2.3D-17 (nenhuma criada nesta fatia)
L-F6-06 = ABERTA, dependente de Q-01
L-F6-07 = REABERTA, dependente de Q-01
P-2.3D-03 = ENCERRADA
...
**Bloqueador único do encerramento:** `Q-01`. Nenhum defeito de código impede o encerramento; o que impede é uma decisão normativa que pertence a Bruno.
...
11. **`F7 — consolidação da Etapa 2.3D`** — **EM CURSO; ETAPA 2.3D NÃO ENCERRADA** (§6-Q). Revisão independente do conjunto integrado F1..F6 e segunda passagem adversarial: **nenhum defeito de código novo**. A revisão levantou **`Q-01`**, conflito entre `docs/09` §5 e `docs/06` §12.7 sobre a auditoria e a autoria da revogação em massa da T-07, que decide simultaneamente `L-F6-06` e `L-F6-07` e **exige decisão expressa de Bruno** — nenhuma decisão normativa foi criada, e o comportamento vigente foi preservado. `P-2.3D-03` encerrada; baseline de §3 corrigida. Matriz normativa sem item `SEM PROVA` ou `INCONSISTENTE`; três `PARCIAL` declarados (AUT-002, AUT-003 e a auditoria da T-07). Passo seguinte: **decisão de Bruno sobre `Q-01`** e, só então, o encerramento formal da Etapa 2.3D.
...
| REV. | Data | Conteúdo |
| --- | --- | --- |
| **34** | **05/09/2026** | **ETAPA 2.3D-B / F7 — REVISÃO INDEPENDENTE EXECUTADA; ETAPA 2.3D NÃO ENCERRADA** (§6-Q). **Nenhuma alteração de código, teste, schema, migration, OpenAPI, dependência ou configuração; nenhuma decisão normativa criada — `D-2.3D-01`..`D-2.3D-17` intactas.** **(a) `Q-01` — achado bloqueante, normativo:** `docs/06` §12.7 define T-07 **exclusivamente** como “Recuperação de senha / revogação”, e `docs/09` §5 atribui `usuario.sessao.revogacao` à revogação “em massa **via T-07**” com ator **Administrador**, enquanto a linha seguinte audita a **mesma** transação como `usuario.senha.recuperacao_concluida` com ator **Usuário** — sendo que na conclusão **não há Administrador autenticado**. O conflito decide simultaneamente `L-F6-06` (autoria de `revogada_por_usuario_id`) e `L-F6-07` (emitir ou não o evento adicional). **Devolvido a Bruno** (TLF-BASE-V1 §15); registrado em `docs/12` §11 como pendência bloqueante; comportamento vigente **preservado**. **(b) Uma promoção foi redigida e RETIRADA:** `L-F6-06` chegou a ser promovida a `D-2.3D-18` nesta fatia, com fundamento em `docs/09` §5; a revisão da PR [#22](https://github.com/BrunoMNoronha/techlab-fisio/pull/22) mostrou que o fundamento não decide, porque a fonte não diz **qual** evento é o correspondente. **Nenhuma `D-2.3D-18` existe.** **(c) Revisão de F1..F6 + passagem adversarial sobre 17 hipóteses: nenhum defeito de código novo**; registrados como corretos `#registrarAtividade` (atômico, não TOCTOU), a recusa de usuário inativado por `resolverDoUsuario`, os guards fail-closed, a autoria não parametrizável e o fechamento do último TOCTOU do login por `M-03`. **(d) Matriz normativa** sem item `SEM PROVA` ou `INCONSISTENTE`; **três `PARCIAL` declarados** — AUT-002 (sem revogação administrativa de terceiro nem inativação), AUT-003 (escopo/recurso de `P2.2-05` não iniciados) e a auditoria da T-07 (depende de `Q-01`). **(e) Correções documentais:** baseline de §3 (`D-2.3D-01`..`D-2.3D-13`, desatualizada desde a F5) e `P-2.3D-03` **ENCERRADA** (medida e verde desde a F3; `D-2.3D-11` não alterada). **(f) `docs/12` REV. 9**, factual: F7 → EM CURSO, `Q-01` acrescentada a §11, AUT-002 → PARCIAL. |
