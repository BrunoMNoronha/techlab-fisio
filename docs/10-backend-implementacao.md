...
| **AUT-002 — revogação administrativa de sessão de terceiro (`sessoes.revogar_terceiro`)** | **DECISÃO NORMATIVA FECHADA em 06/09/2026 (`D-2.3D-19`, `docs/12` §5.19) / IMPLEMENTAÇÃO PENDENTE — `P-2.3D-07` (`docs/12` §11).** A homologação técnica independente de 06/09/2026 mediu, sobre `main` = `867e9d2` com diff vazio, que a operação **não existe** — sem rota, controller, service, DTO ou emissor de auditoria; `sessoes.revogar_terceiro` segue **catalogada e órfã** em `permissoes.catalogo.ts:40` e `catalogo-rbac.ts:85`, e `usuario.sessao.revogacao` segue **no catálogo sem emissor** (`audit.catalog.ts:82`, whitelist vazia). A lacuna era **normativa**: `D-2.3D-05` e `D-2.3D-18` derivam a autoria da própria linha (`SET revogada_por_usuario_id = usuario_id`, §6-E/§6-P), e `D-2.3D-18` declara expressamente **não** determinar o valor para a operação administrativa. `D-2.3D-19` fecha a lacuna abrindo **exceção delimitada** — o ator é o **Administrador autenticado executor**, derivado exclusivamente do contexto validado, persistido em `revogada_por_usuario_id` **e** em `evento_auditoria.ator_usuario_id`, nunca fornecido pelo cliente — e fixa operação individual por `sessao_id`, recusa de autorrevogação, `204` uniforme anti-enumeração, preservação da semântica de expiração, convergência sob concorrência, auditoria obrigatória **na mesma transação** e **zero drift**. **Logout (`D-2.3D-05`) e T-07 (`D-2.3D-18`) NÃO foram alterados.** Critérios de aceite `A-01`..`A-22` e as quatro provas de poder exigidas: `docs/12` §10.4. Fatia **própria**, fora da Etapa 2.3D, **sujeita a autorização própria**; nenhuma migration esperada. Encerra a dependência registrada do risco de volume **V0** de `PBACK-AUD-09` (§7.4.7), que conta com esta operação como resposta administrativa — **o risco V0 permanece aceito até a implementação** |
...
14. **`AUT-002` — revogação administrativa de sessão de terceiro** — **DECISÃO NORMATIVA FECHADA em 06/09/2026 (`D-2.3D-19`, `docs/12` §5.19, REV. 10); IMPLEMENTAÇÃO NÃO INICIADA (`P-2.3D-07`).** Rodada documental executada na branch `agent/aut-002-d19-decisao`, **sem commit, push, PR, merge ou deploy** e **sem tocar runtime, schema, migrations ou dependências**. A rodada seguinte é a **implementação**, por autorização própria: rota administrativa individual sob `@RequerPermissao("sessoes.revogar_terceiro")` no padrão vigente da API, ator derivado do `ContextoAutenticado`, transição `ATIVA -> REVOGADA` e evento `usuario.sessao.revogacao` na **mesma transação** pelo `AuditWriter` já existente (que recusa cliente não transacional), `204` uniforme, testes de concorrência contra PostgreSQL real e os quatro *mutation challenges* obrigatórios — provando `A-01`..`A-22` (`docs/12` §10.4), com **zero drift** e **sem** introduzir artefato de AUT-005. **AUT-005 permanece marco distinto e sem fatia atribuída.** **Atualização factual de 06/09/2026 (REV. 38):** a rodada documental foi publicada e **INTEGRADA NA `main`** — PR [#26](https://github.com/BrunoMNoronha/techlab-fisio/pull/26), commit `0444fef`, merge commit `aba939c` sobre `867e9d2`, CI verde (run 34009649437); somente `docs/10` e `docs/12`. A **implementação** (`P-2.3D-07`) permanece **NÃO INICIADA**.
