...
> **Revisão vigente:** REV. 33 (05/09/2026) — **ETAPA 2.3D-B / F6 (recuperação segura de senha — AUT-004, D-04, RN-005, T-07, `D-2.3D-08`, segunda metade de `D-2.3D-06`): HOMOLOGADA E INTEGRADA NA `main`** (§6-P; rito em §6-P.18). Integrada em 05/09/2026 por **merge commit `cbd02eb`** (PR [#20](https://github.com/BrunoMNoronha/techlab-fisio/pull/20)), sobre o HEAD `f6f507f`, a partir de `main` = `caf2280`. **CI verde sobre o HEAD integrado** (run 33952810239, 3m33s): `test:api` **24 · 595**; integração com PostgreSQL 18 real **10 · 386**; reconstrução from-scratch **10 · 87**; OpenAPI **19/19**; Argon2, smoke, `lint:migrations` e `schema:verify` (golden 60 169 bytes idêntico; `migrate diff` exit 0) verdes. **CORREÇÃO `M-03` APLICADA ANTES DO MERGE** (§6-P.17), respondendo ao achado P1 da revisão automatizada na PR: a emissão de sessão do login reconferia o digest **apenas** no caminho de rehash, de modo que um login em voo validado contra a credencial superada emitia sessão **depois** da revogação em massa de T-07 — frustrando RN-005/T-07; corrigido na origem por guarda de credencial corrente incondicional (`SELECT ... FOR UPDATE`), com prova de entrelaçamento determinístico (`401 → 200` na árvore sem a correção). **DUAS DECISÕES LOCAIS PROMOVIDAS A NORMATIVAS em 05/09/2026:** `L-F6-03` → **`D-2.3D-16`** (o início da recuperação é operação sobre terceiro; recusa uniforme) e `L-F6-05` → **`D-2.3D-17`** (a unidade contada no limite de conclusão é a **tentativa admitida**, com alcance restrito a essa metade de `D-2.3D-06`) — `docs/12` §5.16/§5.17, REV. 8. `L-F6-06` e `L-F6-07` **não** foram promovidas: já são sustentadas por `D-2.3D-05` e por `docs/09` §5. **ZERO drift de persistência** (nenhuma migration, coluna, índice, enum ou constraint); nenhuma dependência nova; nenhum canal de e-mail/SMS/WhatsApp; contrato OpenAPI em `0.2.0`, acréscimo aditivo. **AUT-004 = MATERIALIZADA E INTEGRADA**; AUT-005 permanece NÃO MATERIALIZADA; **F7 NÃO INICIADA**.
...
> **Atualização de 05/09/2026 (pós-integração).** Bruno decidiu expressamente as quatro. `L-F6-03` e `L-F6-05` foram **promovidas** a `D-2.3D-16` e `D-2.3D-17` (`docs/12` §5.16/§5.17) e passam a ter fonte normativa própria, permanecendo citáveis como aliases históricos. `L-F6-06` e `L-F6-07` **não** foram promovidas: a primeira reproduz a atribuição que `D-2.3D-05` já fixa para o logout, a segunda decorre diretamente de `docs/09` §5 — em nenhuma delas havia ambiguidade normativa a resolver. **Nenhum comportamento mudou por causa dessas decisões.** A tabela acima é preservada como registro do estado no ato da implementação.
...
| Item | Severidade | Situação |
| --- | --- | --- |
| Decisões locais `L-F6-03`, `L-F6-05`, `L-F6-06`, `L-F6-07` | **ENCERRADO em 05/09/2026** | Natureza comportamental (precedente `L-11`/`L-F5-07`/`L-F5-08`); implementadas na alternativa conservadora e reversíveis. Decididas expressamente por Bruno: `L-F6-03` → `D-2.3D-16` e `L-F6-05` → `D-2.3D-17` (`docs/12` §5.16/§5.17); `L-F6-06` e `L-F6-07` mantidas como locais. Detalhe em §6-P.18.4 |
...
| `L-F6-06` | **NÃO promovida** — mantida como decisão local. `revogada_por_usuario_id = usuario_id` é exatamente a atribuição que `D-2.3D-05` já fixa para o logout, e `NULL` na coluna já significa "não revogada" (estado de uma sessão `EXPIRADA`). Não há ambiguidade normativa a resolver |
