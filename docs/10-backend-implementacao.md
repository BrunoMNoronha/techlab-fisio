...
| `P-2.3D-04` (reavaliação de CSRF) | **ABERTA** — `apps/web` existe na `main` desde 05/09/2026 (PR #24, merge `80986e7`; fundação técnica **sem** integração com `apps/api`), o que torna a reavaliação **tecnicamente elegível**; ela **não foi realizada nem decidida** — nenhuma estratégia CSRF foi antecipada (`docs/08` REV. 22/24; `apps/web/README.md`). Synchronizer token só mediante necessidade demonstrada (`D-2.3D-07`) **Atualização factual de 06/09/2026 (REV. 38):** a Fatia 1 da integração `apps/web` ↔ `apps/api` foi integrada na `main` (PR #27, merge `0d76474`; `docs/08` REV. 26) e a reavaliação passou a ser **materialmente possível e medida** — o proxy same-origin preserva `Host` público, `Origin`, `Sec-Fetch-*` e `X-TLF-Requisicao`, e 3 provas exercitam a própria `ProtecaoCsrfGuard` (aprova o repasse legítimo; recusa `CABECALHO_REQUISICAO_AUSENTE`; recusa `ORIGIN_INVALIDA`). A baseline de `D-2.3D-07` foi **preservada sem enfraquecimento**: nenhum synchronizer token, nenhum CORS, nenhuma exceção no guard. A pendência **continua ABERTA** — a fatia mede, não homologa; o encerramento é ato de Bruno. O `README.md` de `apps/web`, como integrado, afirmava a pendência "resolvida ... aprovada por Bruno Menezes Noronha" sem lastro documental, e foi corrigido editorialmente nesta revisão (`docs/08` REV. 26, item e.1). |
