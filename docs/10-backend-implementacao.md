...
> **Revisão vigente:** REV. 59 (17/09/2026) — **REGISTRO PÓS-INTEGRAÇÃO DE `CFG-003` NA `main` (PR [#80](https://github.com/BrunoMNoronha/techlab-fisio/pull/80), MERGE `fd3c205`)** (§6-Z.7). Registro **exclusivamente factual**; nenhuma decisão normativa criada, alterada ou reaberta; nenhuma linha de código, teste, schema, migration, contrato, dependência ou CI alterada.
...
| Estados vivos | `CFG-003` **INTEGRADO**. Seguem as limitações de §6-Z.5. **Pendente:** alinhamento editorial de `docs/07` §10.1/§10.2 às restrições de `D-CFG-23` (não executado neste registro). Leitura por outros papéis e uso em agenda/pacotes seguem nas fatias futuras |
...
22. ~~**`CFG-003` — catálogo de serviços**~~ — **CONCLUÍDA / INTEGRADA NA `main` em 17/09/2026** (PR [#80](https://github.com/BrunoMNoronha/techlab-fisio/pull/80), merge commit `fd3c205f420b9cb201dad46fe946b3df7354b9ba`, CI runs `35203981793` e `35203985987` sobre o HEAD `10baa64`) (§6-Z, §6-Z.7). Pendente: alinhar `docs/07` §10.1/§10.2.
