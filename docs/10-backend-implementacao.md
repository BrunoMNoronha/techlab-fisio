...
> **Revisão vigente:** REV. 73 (18/09/2026) — **SINCRONIZAÇÃO FACTUAL PÓS-AGD-A/AGD-B**: AGD-A registrada como **integrada na `main`** pela PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`; superfície OpenAPI/verificadores da agenda reconciliada para a única ampliação de AGD-B (`POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta`), preservando negativas de AGD-C/AGD-D/AGD-E. Atualização factual sem criar/reabrir decisões e sem introduzir publicação não medida.
...
**Estado atual: AGD-A integrada na `main` (PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`).** O estado "implementada em branch local, não publicada" permanece como registro histórico da REV. 72. Decisões e autorização: `docs/15` (`D-AGD-01`..`D-AGD-17` e `P-AGD-01`..`P-AGD-12`, pacote `AGD-PREP0`, homologado integralmente por Bruno Menezes Noronha em 17/09/2026), com **autorização expressa e específica** da fatia **AGD-A** e da **alteração estrutural de `D-AGD-07`**. Nenhuma decisão criada, alterada ou reaberta; **nenhuma permissão nova, nenhuma ação de auditoria nova, nenhuma chave de `contexto`, nenhuma dependência nova**.
...
- **Registro:** `AgendaModule` no `AppModule`; listas fechadas de rotas em `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` sincronizadas para a superfície de agenda vigente, incluindo `POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta` como a única ampliação de AGD-B e mantendo negativas explícitas para AGD-C/AGD-D/AGD-E.
...
29. **`AGD-B` — check-in e falta** — **AUTORIZADA pela issue [#91](https://github.com/BrunoMNoronha/techlab-fisio/issues/91)**; nesta revisão, OpenAPI/verificadores/documentação factual foram sincronizados para a única ampliação de superfície (`POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta`). AGD-C, AGD-D e AGD-E permanecem em fatias próprias sob autorização específica.

## 11. Histórico de revisões

| REV. | Data | Conteúdo |
| --- | --- | --- |
| **73** | **18/09/2026** | **SINCRONIZAÇÃO FACTUAL PÓS-AGD-A/AGD-B**. Cabeçalho e §6-AI deixam de declarar AGD-A como "não publicada" e registram integração na `main` pela PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`. OpenAPI, verificadores e listas fechadas de rotas foram reconciliados para a única ampliação de AGD-B (`POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta`), mantendo negativas para AGD-C/AGD-D/AGD-E e sem criar/reabrir decisões `D-AGD-*`. |
