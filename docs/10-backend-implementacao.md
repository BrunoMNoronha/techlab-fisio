...
| **AUT-002 — revogação administrativa de sessão de terceiro (`sessoes.revogar_terceiro`)** | **IMPLEMENTADA E MEDIDA EM BRANCH PRÓPRIA (`agent/p-2.3d-07-revogacao-sessao-terceiro`) em 14/09/2026 — NÃO HOMOLOGADA / NÃO INTEGRADA NA `main`** (§6-R). Decisão normativa `D-2.3D-19` (`docs/12` §5.19) e critérios `A-01`..`A-22` (`docs/12` §10.4) integralmente materializados: endpoint `DELETE /auth/sessoes/:sessaoId`, `@RequerPermissao("sessoes.revogar_terceiro")`, ator extraído exclusivamente da sessão autenticada (`ContextoAutenticado.usuarioId`), resposta anti-oráculo uniforme `204 No Content` sem corpo e sem cookies, mutação atômica em transação com auditoria via `AuditWriter` (`usuario.sessao.revogacao`), inutilização imediata da sessão de terceiro revogada, autorrevogação no-op segura que mantém a sessão ativa e não emite auditoria. 4 mutation challenges `M1`..`M4` provados e revertidos com SHA-256 idêntico. Zero drift de persistência, zero dependência nova. **Encerra o risco de volume V0 de `PBACK-AUD-09` quanto à resposta administrativa.** Branch aguardando homologação e rito de integração |
...
14. **`AUT-002` — revogação administrativa de sessão de terceiro (`P-2.3D-07`)** — **IMPLEMENTADA E MEDIDA EM BRANCH PRÓPRIA (`agent/p-2.3d-07-revogacao-sessao-terceiro`) em 14/09/2026** (§6-R). Decisão `D-2.3D-19` (`docs/12` §5.19) materializada integralmente: `DELETE /auth/sessoes/:sessaoId` sob `@RequerPermissao("sessoes.revogar_terceiro")`, ator extraído exclusivamente da sessão autenticada, `204` uniforme anti-enumeração, mutação e auditoria (`usuario.sessao.revogacao`) atômicas via `AuditWriter`, inutilização imediata de sessão de terceiro, autorrevogação no-op segura que mantém a sessão ativa e não audita. Critérios `A-01`..`A-22` e mutation challenges `M1`..`M4` provados com sucesso. Zero drift de persistência, zero dependência nova. Aguarda homologação e rito de integração na `main`. **AUT-005 permanece marco distinto e sem fatia atribuída.**
...
**Fim — `docs/10-backend-implementacao.md` — REV. 39 — documento mutável; estado vivo das pendências da frente de backend.**
...
**Pendências vivas:** `P-BACK-01` EM ANDAMENTO; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE (§7.2); `P-2.3D-04`, `P-2.3D-05` e `P-2.3D-06` abertas/adiada (`P-2.3D-03` ENCERRADA — §6-Q, REV. 34); `R-2.3D-04`, `R-2.3D-05` e `R-2.3D-06` registrados sem encerramento autodeclarado. **Riscos residuais aceitos:** `R-2.3D-08` (BAIXO), `F-12` (BAIXO), `F3R-06` e `F3R-07` (OBSERVAÇÃO). `L-05` e `L-08` (política) DECIDIDAS em 05/09/2026 (`docs/09` §13); **`L-07` DECIDIDA (`PBACK-AUD-09`, `docs/09` §13.4.1), IMPLEMENTADA e INTEGRADA NA `main` em 05/09/2026 — PR #23, merge `e1459f6`** (§7.4, §7.4.8); **`L-06` ABERTA / BLOQUEADA** (§13.3), com retomada obrigatória antes de qualquer superfície de leitura clínica; materialização técnica de `L-08` e visualizador de auditoria PENDENTES; `R-BL-09` ABERTO/monitorado; `V-06.c` APROVADA em 05/09/2026 (`apps/web` integrado na `main` pela PR #24, merge `80986e7` — `docs/08` REV. 22–25); `R-BL-03` residual e ABERTO; **AUT-005 NÃO MATERIALIZADA**; revogação administrativa de sessão de terceiro **IMPLEMENTADA E MEDIDA EM BRANCH PRÓPRIA em 14/09/2026 (`agent/p-2.3d-07-revogacao-sessao-terceiro`, `D-2.3D-19`, `docs/10` §6-R, `docs/12` §5.19 e §10.4)** — `P-2.3D-07` com implementação concluída na branch e aguardando homologação e integração na `main`.

**Estado da `main` em 06/09/2026 (REV. 38):** `origin/main` = **`0d76474`**. `D-2.3D-19` integrada pela PR #26 (merge `aba939c`). Fatia 1 da integração `apps/web` ↔ `apps/api` integrada pela PR #27 (merge `0d76474`): proxy same-origin, client HTTP tipado e tela de login existem na `main`, com `apps/api` e `packages/` intocados — **zero drift, zero dependência nova**; `typecheck` 0, `build` 0 e `verify-web-integration.mjs` **24/24** medidos sobre a `main` integrada. **`P-2.3D-04` permanece ABERTA** — baseline de `D-2.3D-07` preservada sem enfraquecimento; encerramento formal é ato de Bruno. Registrados sem ação: `verify-web-integration.mjs` fora da CI e `URL_API_INTERNA` fora do `.env.example`.

**Fontes normativas:** `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md`, `docs/02`..`docs/07`, `docs/08` (baseline técnica), `docs/09` §12 (`D-AUD-01`..`D-AUD-08`), `docs/11` (`PROP-RN-2.3C-01`) e **`docs/12` (`D-2.3D-01`..`D-2.3D-19`)**.
