...
**Estado:** **IMPLEMENTADA E MEDIDA EM BRANCH PRÓPRIA (`agent/p-2.3d-08-sessao-atual-e2e`) em 15/09/2026** a partir de `main` = `b6427e1ab999aae40f41e7745fe374f32b719836`.
...
| `P-2.3D-04` (reavaliação de CSRF) | **ABERTA** — `apps/web` existe na `main` desde 05/09/2026 (PR #24, merge `80986e7`; fundação técnica **sem** integração com `apps/api`), o que torna a reavaliação **tecnicamente elegível**; ela **não foi realizada nem decidida** — nenhuma estratégia CSRF foi antecipada (`docs/08` REV. 22/24; `apps/web/README.md`). Synchronizer token só mediante necessidade demonstrada (`D-2.3D-07`) **Atualização factual de 06/09/2026 (REV. 38):** a Fatia 1 da integração `apps/web` ↔ `apps/api` foi integrada na `main` (PR #27, merge `0d76474`; `docs/08` REV. 26) e a reavaliação passou a ser **materialmente possível e medida** — o proxy same-origin preserva `Host` público, `Origin`, `Sec-Fetch-*` e `X-TLF-Requisicao`, e 3 provas avaliam a compatibilidade contra a função local de reprodução `avaliarGuard` sobre cabeçalhos simulados (aprova o repasse simulado; recusa `CABECALHO_REQUISICAO_AUSENTE`; recusa `ORIGIN_INVALIDA` — **sem executar o `ProtecaoCsrfGuard` real do backend** e sem executar o Route Handler real do Next.js). A baseline de `D-2.3D-07` foi **preservada sem enfraquecimento**: nenhum synchronizer token, nenhum CORS, nenhuma exceção no guard. A bateria (24 verificações) **não roda na CI** e a prova de fluxo integrado ponta a ponta com o backend real em runtime permanece não realizada. A pendência **continua ABERTA** — a fatia mede sob escopo simulado/local, não homologa; o encerramento é ato de Bruno. O `README.md` de `apps/web`, como integrado, afirmava a pendência "resolvida ... aprovada por Bruno Menezes Noronha" sem lastro documental, e foi corrigido editorialmente nesta revisão (`docs/08` REV. 26, item e.1). |
...
| **P-2.3D-08** — Consulta da sessão autenticada atual e prova E2E same-origin (`D-2.3D-20`) | **EM ANDAMENTO / IMPLEMENTADA EM BRANCH PRÓPRIA** | branch `agent/p-2.3d-08-sessao-atual-e2e`; `D-2.3D-20`; §6-S; REV. 40 |

**Pendências vivas:** `P-BACK-01` EM ANDAMENTO; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE (§7.2); `P-2.3D-04` (ABERTA — aguarda conclusão da prova E2E e PR documental de encerramento), `P-2.3D-05` e `P-2.3D-06` abertas/adiada (`P-2.3D-03` ENCERRADA — §6-Q, REV. 34); `R-2.3D-04`, `R-2.3D-05` e `R-2.3D-06` registrados sem encerramento autodeclarado. **Riscos residuais aceitos:** `R-2.3D-08` (BAIXO), `F-12` (BAIXO), `F3R-06` e `F3R-07` (OBSERVAÇÃO). `L-05` e `L-08` (política) DECIDIDAS em 05/09/2026 (`docs/09` §13); **`L-07` DECIDIDA (`PBACK-AUD-09`, `docs/09` §13.4.1), IMPLEMENTADA e INTEGRADA NA `main` em 05/09/2026 — PR #23, merge `e1459f6`** (§7.4, §7.4.8); **`L-06` ABERTA / BLOQUEADA** (§13.3), com retomada obrigatória antes de qualquer superfície de leitura clínica; materialização técnica de `L-08` e visualizador de auditoria PENDENTES; `R-BL-09` ABERTO/monitorado; `V-06.c` APROVADA em 05/09/2026 (`apps/web` integrado na `main` pela PR #24, merge `80986e7` — `docs/08` REV. 22–25); `R-BL-03` residual e ABERTO; **AUT-005 NÃO MATERIALIZADA**; `P-2.3D-07` CONCLUÍDA e INTEGRADA NA `main` em 14/09/2026 (PR #46, merge `b6427e1`).

**Estado da `main` em 15/09/2026 (REV. 40):** `origin/main` = **`b6427e1ab999aae40f41e7745fe374f32b719836`**. `P-2.3D-07` integrada pela PR #46 (merge `b6427e1`). `P-2.3D-08` em andamento em branch própria (`agent/p-2.3d-08-sessao-atual-e2e`). **`P-2.3D-04` permanece ABERTA** — baseline de `D-2.3D-07` preservada sem enfraquecimento; encerramento formal é ato de Bruno após prova E2E integrada na `main`.

**Fontes normativas:** `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md`, `docs/02`..`docs/07`, `docs/08` (baseline técnica), `docs/09` §12 (`D-AUD-01`..`D-AUD-08`), `docs/11` (`PROP-RN-2.3C-01`) e **`docs/12` (`D-2.3D-01`..`D-2.3D-20`)**.
