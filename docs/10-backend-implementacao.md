...
- **Registro:** `AgendaModule` no `AppModule`; listas fechadas de rotas em `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` sincronizadas com as rotas públicas vigentes da agenda.
...
- **Fronteira da fatia:** bloqueio, modalidade `PACOTE`, `reserva_sessao`, iniciar/concluir atendimento, `AGUARDANDO -> CANCELADO`, recorrência, notificações, lista de espera, agendamento público e sugestão automática de horários **não** foram implementados e não têm preparação alguma no código.
