...
**Estado atual: IMPLEMENTADA E MEDIDA EM BRANCH LOCAL (`agent/agd-a-agenda-core`, worktree dedicado `techlab-fisio-agd-a`, a partir de `origin/main` = `4b2e2c0`) — NÃO PUBLICADA.** Sem push, PR, merge ou deploy. Decisões e autorização: `docs/15` (`D-AGD-01`..`D-AGD-17` e `P-AGD-01`..`P-AGD-12`, pacote `AGD-PREP0`, homologado integralmente por Bruno Menezes Noronha em 17/09/2026), com **autorização expressa e específica** da fatia **AGD-A** e da **alteração estrutural de `D-AGD-07`** nesta execução. Nenhuma decisão criada, alterada ou reaberta; **nenhuma permissão nova, nenhuma ação de auditoria nova, nenhuma chave de `contexto`, nenhuma dependência nova**.
...
- **Registro:** `AgendaModule` no `AppModule`; listas fechadas de rotas em `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` atualizadas com as sete rotas, e a lista de termos proibidos trocada de `agenda` para os termos de AGD-B/C/D/E (`checkin`, `falta`, `bloqueio`, `pacotes`, `atendimento`, `reserva`).
...
28. **`AGD-A` — núcleo inicial da agenda** — **IMPLEMENTADA E MEDIDA EM BRANCH LOCAL `agent/agd-a-agenda-core`, NÃO PUBLICADA** (§6-AI; decisões `docs/15`). Pendente: **revisão técnica independente**, publicação e integração na `main`. Depois disso, nesta ordem provável e cada uma sob autorização própria: **AGD-B** (check-in e falta), **AGD-C** (bloqueios), **AGD-D** (modalidade `PACOTE` e `reserva_sessao`) e **AGD-E** (iniciar/concluir atendimento, dependente do prontuário e de `P2.2-05`). A interface de agenda em `apps/web` é fatia própria e não foi iniciada.
