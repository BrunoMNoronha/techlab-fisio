...
> **Revisão vigente:** REV. 56 (17/09/2026) — **`CFG-002` — HORÁRIO DE FUNCIONAMENTO DA CLÍNICA IMPLEMENTADO E MEDIDO EM BRANCH PRÓPRIA (`agent/cfg-002-horario-funcionamento`) — NÃO INTEGRADO NA `main`** (§6-Y). Materializa `D-CFG-13`..`D-CFG-21` (`docs/14` §3.10); nenhuma decisão criada, alterada ou reaberta; nenhuma migration, permissão, ação de auditoria, chave de `contexto` ou dependência nova.
