...
> **Revisão vigente:** REV. 7 (27/08/2026) — `P-2.3C-01` **RESOLVIDA** por `PROP-RN-2.3C-01` (`docs/11`); Etapa 2.3C **IMPLEMENTADA E MEDIDA nesta branch — NÃO INTEGRADA** (§6-C): primeiro fluxo real com whitelist POSITIVA (`cobranca.desconto_aplicado`)
...
**Estado: IMPLEMENTADA E MEDIDA nesta branch (`agent/fase2-etapa2.3c-desconto-whitelist`) — NÃO INTEGRADA; aguardando revisão independente/homologação.** Fonte normativa da regra: **`PROP-RN-2.3C-01`** (`docs/11`). **Zero mudança física de persistência** (schema, migrations, golden e `protected-objects.json` idênticos à `main` — diff vazio medido).
...
```text
R2.2-04 — ENCERRADO NA 2.3C (nesta branch) — efetivação na main acompanha a integração/homologação da etapa
```

A mitigação homologada (`docs/07` §22.3 → D-AUD-07/D-AUD-08) está agora **efetivamente provada nos dois regimes**: whitelist **VAZIA** (2.3B — `profissional.situacao.alterada`, §6-A.5) e whitelist **POSITIVA** (2.3C — `cobranca.desconto_aplicado`, §6-C.2), ambas ponta a ponta contra PostgreSQL real, incluindo a prova de que contexto inválido (chave extra, estrutura composta, número não finito, ação desconhecida) provoca **rollback integral do fluxo de negócio** — não apenas rejeição unitária do validator. Nenhum caminho de escrita em `evento_auditoria` existe fora de `AuditWriter → AuditContextValidator`. O encerramento é registrado **nesta branch** conforme a reautorização da 2.3C; torna-se efetivo na `main` com a homologação/integração da etapa. Reaberturas futuras exigiriam novo fluxo emissor que contornasse o writer — vedado por §6-A.3.
...
```text
T-AUD-CONTEXTO — CONCLUÍDO (EXECUTADO / PASSED — unitário + integração, incluindo emissor real com contexto NÃO vazio desde a 2.3C)
```
...
| `R2.2-04` | **ENCERRADO NA 2.3C (nesta branch)** (§7.2) — mitigação provada nos dois regimes de whitelist; efetivação na `main` com a integração da etapa |
...
3. **Etapa 2.3C — bloqueio `P-2.3C-01` RESOLVIDO (`PROP-RN-2.3C-01`, `docs/11`); implementação EXECUTADA E MEDIDA em 27/08/2026** (§6-B/§6-C): primeiro fluxo real com whitelist positiva (`cobranca.desconto_aplicado`). Pendente: revisão independente, homologação e rito de integração próprios — **nenhum PR/merge nesta execução**.
...
**Fim — `docs/10-backend-implementacao.md` — REV. 7 — documento mutável; estado vivo das pendências da frente de backend; Etapa 2.3A INTEGRADA NA `main` (merge `791e862`); Etapa 2.3B CONCLUÍDA/HOMOLOGADA/INTEGRADA NA `main` (merge `5aae8de`); Etapa 2.3C IMPLEMENTADA E MEDIDA nesta branch — NÃO INTEGRADA (§6-C; `P-2.3C-01` resolvida por `PROP-RN-2.3C-01`, `docs/11`); `P-BACK-01` EM ANDAMENTO; `R2.2-04` ENCERRADO NA 2.3C (efetivação na `main` com a integração). Fontes normativas permanecem `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md`, `docs/02`..`docs/07`, `docs/08` (baseline), `docs/09` §12 e `docs/11` (`PROP-RN-2.3C-01`).**
