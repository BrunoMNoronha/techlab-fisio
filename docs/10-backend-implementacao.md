...
> **Revisão vigente:** REV. 73 (18/09/2026) — **AGD-B — CHECK-IN E FALTA INTEGRADA NA `main` E SINCRONIZAÇÃO FACTUAL/OPENAPI**. AGD-A permanece integrada pela PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`. A AGD-B foi integrada pela PR [#99](https://github.com/BrunoMNoronha/techlab-fisio/pull/99), merge `ab6c60143d7195cfaf43fd809119ded7ec4ad54b`: `POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta`, permissões existentes `agenda.checkin`/`agenda.falta`, escopo por permissão efetiva, lock pessimista e histórico append-only, sem auditoria, migration, schema, dependência ou permissão nova. Esta revisão reconcilia OpenAPI, verificadores e documentação sem reabrir `D-AGD-*` e preserva AGD-C/D/E fora do escopo.
> **Estado anterior preservado:** REV. 72 (17/09/2026) — **`AGD-A` — NÚCLEO INICIAL DA AGENDA IMPLEMENTADO E MEDIDO EM BRANCH LOCAL (`agent/agd-a-agenda-core`, worktree `techlab-fisio-agd-a`, a partir de `origin/main` = `4b2e2c0`) — NÃO PUBLICADA** (§6-AI). Materializa `docs/15` `D-AGD-01`..`D-AGD-17`/`P-AGD-01`..`P-AGD-12` na fatia AGD-A, sob autorização expressa e específica de Bruno Menezes Noronha (17/09/2026), inclusive a alteração estrutural de `D-AGD-07`: migration `20260917230000_agendamento_cancelamento_coerente` (`ck_agendamento_cancelamento_coerente`). Sete rotas novas (`/agendamentos` e `GET /agenda/opcoes`) sob `agenda.gerenciar`; escopo operacional x próprio; histórico e auditoria transacionais e correlacionados; `AgendaModule` passa a ser o consumidor dos verificadores de RN-014 que CFG-002 e PRO-003 deixaram prontos. **Nenhuma decisão criada, alterada ou reaberta; nenhuma permissão, ação de auditoria, chave de `contexto` ou dependência nova; nenhuma rota de AGD-B, AGD-C, AGD-D ou AGD-E.** Sem push, PR, merge ou deploy.
> **Estado anterior preservado:** REV. 71 (17/09/2026) — **CONSOLIDAÇÃO LOCAL PRÉ-`AGD-A` NA BRANCH `integration/local-pre-agd-a` — NÃO PUBLICADA NA `main`** (§6-AH). Integração, na ordem `CFG-004`/`CFG-005` → `PRO-003`, do fechamento das duas frentes de configuração (REV. 69, §6-AF) e de `PRO-003` (REV. 70, §6-AG) sobre a `main` atual (`92bcf6e`, que já contém `CI-E2E0`/REV. 68). Registro da reconciliação documental, da cadeia de 17 migrations e da bateria integral medida na árvore consolidada. **Nenhuma decisão criada, alterada ou reaberta; nenhum comportamento de runtime, contrato, schema, migration, permissão ou dependência alterado nesta revisão** — só numeração de revisões, letra de seção e referências cruzadas.
...
> **Estado anterior preservado:** REV. 44 (16/09/2026) — **REGISTRO PÓS-INTEGRAÇÃO DE `D-AUD-09` / `D-AUD-09-A` E DO ENCERRAMENTO DE `R2.2-04` NA `main` (PR #49, MERGE `c2d57b8`)** (§7.5.9). Registro **exclusivamente factual**, no precedente `MEDIR → REGISTRAR` (§7.4.8, REV. 41). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-AUD-01`..`D-AUD-09`, `D-AUD-09-A` e `PBACK-AUD-01`..`PBACK-AUD-09` vigentes; Base Imutável V2 intacta; nenhuma linha de código, teste, schema, migration, golden, protected-objects, OpenAPI, contrato HTTP, dependência ou CI alterada. **(a) Integração:** a fatia das REV. 42 e REV. 43 foi integrada na `main` em 16/09/2026 por Bruno Menezes Noronha via PR [#49](https://github.com/BrunoMNoronha/techlab-fisio/pull/49) (commits `8bf9057963177e704b65a11117144668095e8e35` e `02f01fc17a765703941995bb63a34a2dcacf9b6d`, merge commit `c2d57b808eafffbffbeee765ccc64d992c21fb5b`), CI verde sobre o HEAD da PR (run `35076322751`). **(b) Validação pós-merge:** `pnpm verify:api-integration` sobre a `main` integrada — exit 0, 12 suítes · 424 passed · 2 skipped. **(c) Estados vivos inalterados:** `R2.2-04` **ENCERRADO** (§7.2 — REV. 43; não é declaração de risco zero); **`P-BACK-01` EM ANDAMENTO**; **`L-06` ABERTA / BLOQUEADA**; nenhuma rota de leitura clínica autorizada; `prontuario.acessado` não criada.
> **Estado anterior preservado:** REV. 43 (15/09/2026) — **`P-BACK-01` / `R2.2-04` — ENCERRAMENTO FORMAL DE `R2.2-04` APÓS A HOMOLOGAÇÃO DE `D-AUD-09-A` (`docs/09` §14.9)** (§7.2, §7.2.0). Bruno Menezes Noronha resolveu em 15/09/2026 a ambiguidade da cláusula final de `docs/09` §13.3 adotando a **leitura restrita** de “por esta via”: `L-06` **não** é pré-condição global de `R2.2-04` — são obrigações distintas (conteúdo de `contexto` × política de auditoria de leitura clínica). Com isso a **sétima** e última condição de §7.2 passa a **SIM** e `R2.2-04` fica **ENCERRADO**. Revisão **exclusivamente documental**: nenhuma linha de código, teste, schema, migration, golden, protected-objects, OpenAPI, contrato HTTP, dependência ou configuração de CI foi alterada por ela; `D-AUD-09` (§7.5) e todas as decisões anteriores permanecem **inalteradas e não reabertas**. **`L-06` permanece ABERTA / BLOQUEADA POR DEPENDÊNCIA FUNCIONAL-TÉCNICA**, com regra, gatilhos e dependências intactos; **nenhuma rota de leitura clínica autorizada**; **`prontuario.acessado` não criada**; **`P-BACK-01` permanece EM ANDAMENTO** (§7.1). Toda a fatia (REV. 42 + REV. 43) segue na branch `agent/p-back-01-r2-2-04-semantica-contexto`; publicação **não autorizada** nesta rodada.
> **Estado anterior preservado:** REV. 42 (15/09/2026) — **`P-BACK-01` / `R2.2-04` — MATERIALIZAÇÃO DE `D-AUD-09` (`docs/09` §14): VALIDAÇÃO SEMÂNTICA DAS DUAS WHITELISTS POSITIVAS REMANESCENTES** (§7.5). **`R2.2-04` NÃO FOI ENCERRADO** (§7.2, §7.2.0): a condição semântica que o mantinha aberto foi suprida, mas a inspeção final encontrou a cláusula de `docs/09` §13.3 que víncula esse encerramento a **`L-06`**, ainda aberta — divergência de leitura registrada e devolvida a Bruno em `docs/09` §14.8, sem resolução silenciosa. Decisão normativa `D-AUD-09` registrada em `docs/09` §14 e aqui apenas materializada; `D-AUD-01`..`D-AUD-08` e `PBACK-AUD-01`..`PBACK-AUD-09` **não** reabertas; Base Imutável V2 intacta; zero drift de persistência (`packages/database`, schema, migrations, golden e protected-objects intocados); zero dependências; OpenAPI e contratos HTTP intocados; nenhum emissor de domínio criado. **`P-BACK-01` permanece EM ANDAMENTO e `L-06` permanece ABERTA / BLOQUEADA** — o encerramento de `R2.2-04` não os alcança. Branch `agent/p-back-01-r2-2-04-semantica-contexto`; publicação **não autorizada** nesta rodada.
> **Estado anterior preservado:** REV. 41 (15/09/2026) — **REGISTRO PÓS-INTEGRAÇÃO DE `P-2.3D-08` / `D-2.3D-20` NA `main` (PR #47, MERGE `89fa492`) E ENCERRAMENTO FACTUAL DE `P-2.3D-04` MEDIANTE PROVA E2E REAL SAME-ORIGIN**. Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-20` vigentes; Base Imutável V2 intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependências adicionadas. **(a) Integração de `P-2.3D-08` na `main`:** integrada em 15/09/2026 via PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47), commit `4f78c4ed2de5b2e9f358f55accfa021b027fc124`, merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI verde run `34983136046`. Endpoint `GET /auth/sessao` em produção compilada; prova E2E real automatizada `verify-web-api-e2e.mjs` versionada e integrada à CI. **(b) Encerramento factual de `P-2.3D-04`:** Encerrada em 15/09/2026 após a integração do PR #47 (merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`) que versionou `apps/web/scripts/verify-web-api-e2e.mjs` e o passo correspondente na CI, executando simultaneamente PostgreSQL real em container, NestJS real compilado, Next.js real compilado com Route Handler de proxy same-origin (`/api/*`), `ProtecaoCsrfGuard` real e cookie real de sessão, provando que a baseline de CSRF de `D-2.3D-07` (`SameSite=Strict`, cabeçalho obrigatório `X-TLF-Requisicao` nas mutações, Fetch Metadata, validação de `Origin`, ausência de CORS) protege a fronteira de ponta a ponta sem necessidade de synchronizer token adicional.
> **Estado anterior preservado:** REV. 40 (15/09/2026) — **MATERIALIZAÇÃO DA DECISÃO NORMATIVA `D-2.3D-20` (CONSULTA DA SESSÃO AUTENTICADA ATUAL VIA `GET /auth/sessao`), PROVA E2E REAL SAME-ORIGIN (`verify-web-api-e2e.mjs`) E RECONCILIAÇÃO FACTUAL DA INTEGRAÇÃO DE `P-2.3D-07` NA `main`** (§6-S). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-20` vigentes; Base Imutável intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependência adicionada em `package.json` ou lockfiles. **(a) Reconciliação documental de `P-2.3D-07`:** integrada na `main` em 14/09/2026 via PR [#46](https://github.com/BrunoMNoronha/techlab-fisio/pull/46), merge commit `b6427e1ab999aae40f41e7745fe374f32b719836`. **(b) Materialização de `D-2.3D-20` no backend (`apps/api`):** endpoint `GET /auth/sessao` sob `@UseGuards(SessaoAutenticadaGuard)`, contrato OpenAPI v3.1 tipado com `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@HttpCode(200)`, `@Header("Cache-Control", "no-store")`, respostas `200, 401, 500` e DTO `ConsultarSessaoRespostaDto` `{ usuarioId, sessaoId }`. Proteção CSRF (`ProtecaoCsrfGuard`) movida da classe `AuthController` exclusivamente para as rotas mutantes `@Post("login")` e `@Post("logout")`; `GET /auth/sessao` opera sob método HTTP seguro sem CSRF guard, sem `X-TLF-Requisicao`, sem corpo e sem synchronizer token. Proibição expressa de emissão de auditoria respeitada (`usuario.sessao.consultada` proibida). `SessaoAutenticadaGuard` provido diretamente no `AuthModule` preservando integridade modular. `ErroAutenticacaoFilter` estendido para reconhecer `GET /auth/sessao`. **(c) Prova E2E real same-origin automatizada (`apps/web/scripts/verify-web-api-e2e.mjs`):** executa sobre PostgreSQL 18 real em contêiner descartável, NestJS compilado real (`main.js`), Next.js real (`next start`), Route Handler do proxy same-origin de Next.js, `ProtecaoCsrfGuard` real do backend e cookie de sessão real; integrada ao root `package.json` (`verify:web-api-e2e`) e à CI (`.github/workflows/ci.yml`). **(d) 4 mutation challenges executados e revertidos por hash (M1..M4):** M1 (proteção de sessão), M2 (Host no proxy Next.js), M3 (`X-TLF-Requisicao` no CSRF guard) e M4 (Origin estrita no CSRF guard). **(e) Critérios de aceite `A-01`..`A-22` integralmente provados.** `P-2.3D-04` permanece formalmente **ABERTA** nesta rodada de implementação, a ser encerrada exclusivamente em PR documental posterior após integração da prova na `main`. Branch de trabalho: `agent/p-2.3d-08-sessao-atual-e2e`.
> **Estado anterior preservado:** REV. 39 (14/09/2026) — **IMPLEMENTAÇÃO E VALIDAÇÃO DA REVOGAÇÃO ADMINISTRATIVA DE SESSÃO DE TERCEIRO (`P-2.3D-07` / `AUT-002` / `sessoes.revogar_terceiro` / `D-2.3D-19`) EM BRANCH PRÓPRIA (`agent/p-2.3d-07-revogacao-sessao-terceiro`) — NÃO HOMOLOGADA / NÃO INTEGRADA NA `main`** (§6-R). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-19` intactas; Base Imutável intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependência nova. **(a) Operação administrativa implementada:** rota `DELETE /auth/sessoes/:sessaoId` sob `@UseGuards(ProtecaoCsrfGuard)` e `@RequerPermissao("sessoes.revogar_terceiro")`, contrato OpenAPI v3.1 tipado com `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@HttpCode(204)` e resposta anti-oráculo uniforme `204 No Content` sem corpo e sem cookies em todos os desfechos autorizados (revogação efetiva de terceiro, sessão inexistente, já revogada, já expirada temporalmente e autorrevogação no-op). **(b) Ator estritamente derivado da sessão autenticada:** `ContextoAutenticado.usuarioId` extraído de `req.contextoAutenticado`, sem campos de ator no body, query, path ou headers do contrato HTTP. **(c) Inutilização imediata e atomicidade transacional:** `sessao_autenticacao.status` atualizado para `REVOGADA`, `revogada_em` e `revogada_por_usuario_id = atorUsuarioId` (ou inalterada na autorrevogação e expiração prévia) e evento de auditoria `usuario.sessao.revogacao` (`alvo_tipo = "sessao_autenticacao"`, `alvo_id = sessaoId`, `resultado = SUCESSO`, contexto vazio) registrados na **mesma transação** de banco via `AuditWriter`. Se a auditoria falhar, a transação sofre rollback e a sessão permanece ativa. A autorrevogação é no-op seguro: retorna 204 uniforme, a sessão permanece ativa e nenhum evento de auditoria falso é emitido. **(d) 4 mutation challenges executados, detectados e revertidos por hash (M1..M4):** M1 (origem do ator), M2 (RBAC / permissão), M3 (inutilização imediata da sessão) e M4 (atomicidade da auditoria). **(e) Critérios de aceite `A-01`..`A-22` integralmente provados.** Validação integral verde no host Windows: `typecheck` 0; `lint:migrations` 0; `build` 0; `verify:argon2-runtime` 16/16; `verify:openapi-runtime` 20/20; `test:api` 34 suítes / 728 testes; teste de integração da API contra PostgreSQL 18 real com 11 suítes / 414 aprovados / 2 pulados POSIX. Branch de trabalho: `agent/p-2.3d-07-revogacao-sessao-terceiro`; sem commit, push, PR, merge, release ou deploy nesta execução.
> **Estado anterior preservado:** REV. 38 (06/09/2026) — **REGISTRO PÓS-MEDIÇÃO DA INTEGRAÇÃO DAS PR #26 E #27 NA `main` — SINCRONIZAÇÃO DOCUMENTAL EXCLUSIVAMENTE FACTUAL.** Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-19` intactas; Base Imutável intacta; nenhuma linha de código de runtime, teste, dependência, schema ou contrato tocada. **(a) `D-2.3D-19` INTEGRADA NA `main` em 06/09/2026** — a rodada normativa da REV. 37, que ao ser escrita não tinha commit, push nem PR, foi publicada por Bruno Menezes Noronha (`BrunoMNoronha`) na PR [#26](https://github.com/BrunoMNoronha/techlab-fisio/pull/26) (`docs(auth): homologar D-2.3D-19 — revogação administrativa de sessão de terceiro (AUT-002)`), commit `0444fef`, CI verde (run [34009649437](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/34009649437), 3m46s), **merge commit `aba939c`** sobre `867e9d2`, em 06/09/2026 03:47:14Z; conteúdo integrado **exatamente** os 2 arquivos daquela rodada (`docs/10` e `docs/12`, +155/−18) — zero código, zero migration, zero dependência. A asserção da REV. 37 era verdadeira quando escrita e fica **preservada como registro histórico**; este item é o registro pós-merge que a supera. **(b) Fatia 1 da integração `apps/web` ↔ `apps/api` INTEGRADA NA `main` em 06/09/2026** — PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27), commits `c891a71` e `c435b00`, CI verde (run [34065299227](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/34065299227), 3m50s), **merge commit `0d76474`** sobre `aba939c`, em 06/09/2026 23:01:48Z; **9 arquivos, +761/−16, todos sob `apps/web/`** — proxy same-origin `app/api/[...caminho]/route.ts`, client HTTP tipado `lib/api-cliente.ts`, tela de login `app/login/` e a bateria `scripts/verify-web-integration.mjs`. **`apps/api` e `packages/` NÃO foram tocados** — zero drift, 10 migrations, golden inalterado, OpenAPI inalterado, `package-lock.json` intocado. Medições locais sobre a `main` integrada (`0d76474`, host Windows): `typecheck` **0**; `build` **0**; `npm run test --workspace @techlab-fisio/web` **0 — 24/24**. Registro detalhado em `docs/08` REV. 26. **(c) `P-2.3D-04` permanece ABERTA** — a fatia preserva integralmente a baseline de `D-2.3D-07` (`X-TLF-Requisicao` mais conferência `Origin` × `Host`), sem synchronizer token, sem CORS e **sem enfraquecer** a `ProtecaoCsrfGuard`; o encerramento formal é ato de Bruno. **(d) Fatos registrados sem ação:** `verify-web-integration.mjs` **não** é executado pela CI nem pelo `npm test` da raiz; `URL_API_INTERNA` (default `http://localhost:3001`, guarda fail-closed contra SSRF) **não** está em `.env.example`. **Estados preservados:** `AUT-002` PARCIAL e revogação administrativa **NÃO IMPLEMENTADA** (`P-2.3D-07` ABERTA); `AUT-005` NÃO MATERIALIZADA; `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `R-BL-03` residual e ABERTO. `origin/main` = `0d76474`. Atualizados: cabeçalho, §9, §10, §11 e rodapé.
> **Estado anterior preservado:** REV. 37 (06/09/2026) — **REGISTRO DA DECISÃO NORMATIVA `D-2.3D-19` — REVOGAÇÃO ADMINISTRATIVA DE SESSÃO DE TERCEIRO (AUT-002 / `sessoes.revogar_terceiro`). RODADA EXCLUSIVAMENTE NORMATIVA E DOCUMENTAL.** **Nenhuma alteração de código, teste, schema, migration, OpenAPI, dependência ou configuração**; nenhum registro histórico reescrito; nenhuma decisão anterior alterada ou reaberta. Antecedente: a **homologação técnica independente de 06/09/2026** mediu, sobre `main` = `origin/main` = `867e9d2` com árvore limpa e diff vazio, que a revogação administrativa **não existe** em nenhuma branch, worktree ou stash, e que `D-2.3D-19` **não existia** — o registro terminava em `D-2.3D-18`, que declara expressamente **não** determinar `revogada_por_usuario_id` para a operação administrativa. Veredito **`C — NÃO APTA`** por **ausência do objeto**, não por defeito. Esta revisão fecha a lacuna **normativa** correspondente: **`D-2.3D-19` homologada por Bruno Menezes Noronha** e registrada em `docs/12` §5.19 (REV. 10), com critérios de aceite `A-01`..`A-22` em `docs/12` §10.4, riscos `R-2.3D-09`/`R-2.3D-10`, alternativas rejeitadas A-15..A-18 e pendência **`P-2.3D-07`**. **A AUT-002 permanece NÃO IMPLEMENTADA** — a decisão é **DECIDIDA / IMPLEMENTAÇÃO PENDENTE**, fatia própria, fora da Etapa 2.3D (encerrada), sujeita a autorização própria e **sem migration esperada**. **Não ampliadas:** `D-AUD-01`, `D-AUD-02`, `D-AUD-03`, `D-AUD-07` e `PBACK-AUD-09` — `sessoes.revogar_terceiro` **não** entra na lista fechada de `autorizacao.negada`. **Estados preservados:** `AUT-005` NÃO MATERIALIZADA; `P-2.3D-04` ABERTA; `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `R-BL-03` residual e ABERTO. `origin/main` = `867e9d2`. Atualizados: cabeçalho, §9, §10 e §11.
> **Estado anterior preservado:** REV. 36 (05/09/2026) — **REGISTRO PÓS-MEDIÇÃO DA INTEGRAÇÃO DE `L-07` NA `main` (PR [#23](https://github.com/BrunoMNoronha/techlab-fisio/pull/23), merge commit `e1459f6`) E DA SPRINT `FRONT-F0` (PR [#24](https://github.com/BrunoMNoronha/techlab-fisio/pull/24), merge commit `80986e7`) — SINCRONIZAÇÃO DOCUMENTAL EXCLUSIVAMENTE FACTUAL.** Nenhuma alteração de código, teste, schema, migration, OpenAPI, dependência ou configuração; nenhuma decisão normativa criada, alterada ou reaberta; nenhum registro histórico reescrito. **`L-07` — INTEGRADA NA `main` em 05/09/2026** (§7.4.8): PR #23 aberta e mesclada por Bruno Menezes Noronha (`BrunoMNoronha`), merge commit `e1459f6` sobre `c4c9ba2`, commit da fatia `62862bf` com CI verde no HEAD da PR (run 33963479328); a decisão `D-6` de `PBACK-AUD-09` registrou a não autorização **no ato da decisão** e é preservada como tal — a publicação é ato posterior do próprio Bruno. **`FRONT-F0` / `apps/web` — INTEGRADA NA `main` em 05/09/2026**: PR #24 mesclada por Bruno, merge commit `80986e7` sobre `e1459f6`, commit `c004f62` com CI verde no HEAD da PR (run 33965553378); `apps/web` passou a existir na `main` como fundação técnica, **sem** funcionalidade de negócio e **sem** integração com `apps/api`; `V-06.c` **APROVADA** (`docs/08` REV. 22–25); `P-2.3D-04` **tecnicamente elegível** para reavaliação, **sem decisão**. `origin/main` = `80986e7`. **Estados preservados:** `AUT-005` NÃO MATERIALIZADA; revogação administrativa de sessão de terceiro (`AUT-002`/`sessoes.revogar_terceiro`) NÃO IMPLEMENTADA; `P-2.3D-04` ABERTA; `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `R-BL-03` residual e ABERTO. Atualizados: cabeçalho, §7.1, §7.4 (nota de estado corrente e novo §7.4.8), §9, §10, §11 e rodapé.
...
| `docs/09` §14 | **`D-AUD-09`** (15/09/2026) — semântica dos VALORES das três whitelists positivas: data civil gregoriana canônica `YYYY-MM-DD` e UUID textual canônico em minúsculas. Não altera ações, chaves, obrigatoriedade nem ownership |
| `docs/09` §14.9 | **`D-AUD-09-A`** (15/09/2026) — resolução da ambiguidade da cláusula final de `docs/09` §13.3: prevalece a **leitura restrita** de “por esta via”. `L-06` **não** é pré-condição global de `R2.2-04` — objetos distintos. Permite o encerramento de `R2.2-04` (§7.2); **não** altera `L-06`, **não** autoriza rota clínica, **não** cria `prontuario.acessado`, **não** encerra `P-BACK-01`, **não** altera código, teste ou schema |
| `docs/11` | `PROP-RN-2.3C-01` — regra de aplicação de desconto (FIN-003) que desbloqueou a Etapa 2.3C |
| **`docs/12`** | **Baseline normativa da Etapa 2.3D** — `D-2.3D-01`..`D-2.3D-20`: persistência de sessão, hash de senha, normalização do login, política de sessão, estados terminais, anti-abuso, cookie/CSRF, recuperação de senha, RBAC inicial, primeiro Administrador, OpenAPI, auditoria de autenticação, revogação administrativa de sessão de terceiro (`D-2.3D-19`) e consulta da sessão autenticada atual (`D-2.3D-20`). Este documento **referencia** `docs/12`, não o duplica nem o reinterpreta |
...
## 6-R. P-2.3D-07 / AUT-002 — Revogação administrativa de sessão de terceiro

**Estado:** **IMPLEMENTADA E MEDIDA EM BRANCH PRÓPRIA (`agent/p-2.3d-07-revogacao-sessao-terceiro`) em 14/09/2026** — **NÃO HOMOLOGADA / NÃO INTEGRADA NA `main`**.
**Publicação não autorizada nesta execução:** sem commit, push, PR, merge, rebase, tag, release ou deploy.
**Decisões e requisitos materializados:** `D-2.3D-19` (`docs/12` §5.19), `AUT-002 / sessoes.revogar_terceiro`, critérios de aceite normativos `A-01`..`A-22` (`docs/12` §10.4).
**Zero drift:** `packages/database` com diff vazio; zero migration nova (10 migrations preservadas); `schema.prisma`, `schema.golden.sql` e `protected-objects.json` estritamente idênticos; zero dependência adicionada em `package.json` ou lockfiles.

### 6-R.1 Componentes e arquitetura de runtime

A capacidade foi implementada no monolito modular NestJS sob o novo módulo `SessoesModule` (`apps/api/src/sessoes/`):

1. **`SessoesController` (`apps/api/src/sessoes/sessoes.controller.ts`)**:
   - Endpoint: `DELETE /auth/sessoes/:sessaoId`.
   - Proteção de borda: `@UseGuards(ProtecaoCsrfGuard)`.
   - Autorização RBAC: `@RequerPermissao("sessoes.revogar_terceiro")` (instala `SessaoAutenticadaGuard` e `PermissoesGuard` na ordem correta).
   - Documentação OpenAPI v3.1: `@ApiTags("Autenticação e Sessão")`, `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@HttpCode(204)`, `@ApiResponse({ status: 204 })`, respostas de erro tipadas para `400`, `401`, `403`, `413`, `500`.
   - Validação estrutural de UUID (`A-04`): conferência via `ehUuidValido` com retorno `400 REQUISICAO_INVALIDA` em formato inválido.
   - Origem estrita do ator (`A-02`): extraído unicamente de `req.contextoAutenticado.usuarioId`. Nenhum parâmetro de ator aceito em body, query, path ou headers.

2. **`SessoesService` (`apps/api/src/sessoes/sessoes.service.ts`)**:
   - Orquestra a revogação administrativa em transação atômica via `AuditWriter`.
   - Consulta a sessão-alvo dentro da transação (`SELECT id, usuario_id, status, expira_em FROM sessao_autenticacao WHERE id = $1 FOR UPDATE`).
   - Se inexistente (`A-07`), já revogada (`A-08`) ou expirada temporalmente (`A-09`): no-op com retorno imediato de sucesso, preservando `204` uniforme e sem emitir auditoria falsa de sucesso.
   - Se for a própria sessão do ator autenticado (`A-06`): no-op seguro, preservando o estado `ATIVA` da sessão e sem emitir auditoria (`D-2.3D-19` §5.19 / `A-06`).
   - Se for sessão ativa de terceiro: mutação atômica via `SessaoService.revogarPorAdministradorEm(tx, sessaoId, atorUsuarioId)` e registro imediato do evento de auditoria `usuario.sessao.revogacao` (`alvo_tipo = "sessao_autenticacao"`, `alvo_id = sessaoId`, `resultado = SUCESSO`, contexto vazio `{}`).
   - Atomicidade transacional e rollback (`A-13`): se o registro de auditoria falhar, a transação inteira é abortada, desfazendo a revogação no banco.

3. **`SessaoService` (`apps/api/src/auth/sessao.service.ts`)**:
   - Métodos adicionados: `revogarPorAdministradorEm(tx, sessaoId, atorUsuarioId)` e wrapper `revogarPorAdministrador(sessaoId, atorUsuarioId)`.
   - Atualização: `UPDATE sessao_autenticacao SET status = 'REVOGADA', revogada_em = $instante, revogada_por_usuario_id = $atorUsuarioId WHERE id = $sessaoId AND status = 'ATIVA'`.

4. **Tratamento de Exceções (`apps/api/src/auth/erro-autenticacao.filter.ts`)**:
   - Estendido para interceptar o método `DELETE` na fronteira de `/auth/sessoes/`, garantindo envelope padronizado `{ erro: "FALHA_INTERNA" }` com HTTP 500 para falhas técnicas inesperadas.

### 6-R.2 Verificação dos Critérios de Aceite Normativos (A-01 a A-22)

Todos os 22 critérios de `docs/12` §10.4 foram formalmente provados em testes automatizados:
- `A-01`: Exige permissão `sessoes.revogar_terceiro` (`403 ACESSO_NEGADO` sem auditoria deliberada adicional).
- `A-02`: Ator extraído exclusivamente da sessão autenticada; sem parâmetros de ator no contrato HTTP.
- `A-03`: Exige autenticação válida (`401 SESSAO_INVALIDA_OU_EXPIRADA`) e CSRF válido (`403 CSRF_INVALIDO`).
- `A-04`: Sessão alvo deve ser UUID válido; formato inválido devolve `400 REQUISICAO_INVALIDA`.
- `A-05`: Revogação efetiva de sessão de terceiro torna a sessão inutilizável imediatamente para qualquer rota autenticada.
- `A-06`: Autorrevogação via endpoint administrativo é no-op seguro: retorna `204`, sessão permanece `ATIVA` e utilizável, nenhum evento de auditoria emitido.
- `A-07`: Sessão inexistente devolve `204` uniforme sem corpo e sem emitir auditoria.
- `A-08`: Sessão já revogada devolve `204` uniforme, mantém `revogada_por_usuario_id` e `revogada_em` originais, sem reauditar.
- `A-09`: Sessão já expirada por tempo devolve `204` uniforme, status permanece temporalmente expirado, sem emitir auditoria.
- `A-10`: Concorrência de expiração e revogação: sessão expirada não é reclassificada como revogada.
- `A-11`: Emissão obrigatória do evento `usuario.sessao.revogacao` com `alvo_tipo = "sessao_autenticacao"`, `alvo_id = sessaoId`, `ator_usuario_id = atorId` e contexto vazio.
- `A-12`: Sessão-alvo tem `status = REVOGADA`, `revogada_por_usuario_id = atorId` e `revogada_em` preenchido.
- `A-13`: Atomicidade transacional estrita: falha na gravação do evento de auditoria cancela a revogação no banco (rollback).
- `A-14`: Resposta uniforme `204 No Content` sem corpo (envelope idêntico para sucesso efetivo, inexistente, já revogada, expirada e autorrevogação).
- `A-15`: Resposta uniforme `204` não emite cookies (`Set-Cookie` ausente).
- `A-16`: Chamadas concorrentes convergem deterministicamente para `204` com exatamente um evento de auditoria emitido.
- `A-17`: Sem enumeração ou oráculo de existência de sessão através do envelope HTTP.
- `A-18`: Nenhuma mutação de banco além de `sessao_autenticacao` e `evento_auditoria`.
- `A-19`: Zero drift de persistência: zero migrations criadas, schema e golden inalterados.
- `A-20`: Contrato OpenAPI v3.1 reflete a rota `DELETE /auth/sessoes/{sessaoId}` com responses `204`, `400`, `401`, `403`, `413`, `500`.
- `A-21`: Execução e verificação dos 4 mutation challenges obrigatórios (`M1`..`M4`).
- `A-22`: Bateria completa verde de typecheck, build, lint de migrations e testes.

### 6-R.3 Mutation Challenges Obrigatórios (M1..M4)

Os 4 mutation challenges definidos em `docs/12` §10.4 foram executados, provados reprovados pelo mutante e revertidos com verificação estrita de hash SHA-256:

- **M1 (Origem do ator):**
  - Mutação: Injeção de UUID fixo para `atorUsuarioId` no controller, ignorando `req.contextoAutenticado`.
  - Resultado: Reprovado imediatamente por `test/sessoes.controller.spec.ts`.
  - Reversão: Hash SHA-256 conferido idêntico (`4adb9bbd493b586ace170c0e25bc0636408916c5222522666ffbd1e56c2cd54c`).

- **M2 (RBAC / permissão):**
  - Mutação: Remoção do decorator `@RequerPermissao("sessoes.revogar_terceiro")` no controller.
  - Resultado: Reprovado imediatamente por `test/sessoes-dto.spec.ts`.
  - Reversão: Hash SHA-256 conferido idêntico (`4adb9bbd493b586ace170c0e25bc0636408916c5222522666ffbd1e56c2cd54c`).

- **M3 (Inutilização imediata):**
  - Mutação: Remoção da checagem de `status === StatusSessao.REVOGADA` em `SessaoService.validar`.
  - Resultado: Reprovado por 4 suítes de teste de integração (`sessoes-revogacao.integration.spec.ts`, `sessao.integration.spec.ts`, `authz-rbac.integration.spec.ts`, `recuperacao-senha.integration.spec.ts`).
  - Reversão: Hash SHA-256 conferido idêntico (`68ed9538145903258d3c2630a006bf3bc8bfed7eed01c9fea1e882a552fc8d79`).

- **M4 (Atomicidade da auditoria):**
  - Mutação: Inclusão de bloco `try/catch` engolindo erro de `auditWriter.registrar` em `SessoesService`.
  - Resultado: Reprovado imediatamente por `test/sessoes.service.spec.ts` (teste de rollback e propagação de falha).
  - Reversão: Hash SHA-256 conferido idêntico (`eb862f744a29cdfa95d1483f72d54b73194357c0a1bc2775b571eaf88a6b244c`).

---

## 6-S. P-2.3D-08 / D-2.3D-20 — Consulta da sessão autenticada atual e prova E2E real same-origin

**Estado:** **CONCLUÍDA / HOMOLOGADA / INTEGRADA NA `main` em 15/09/2026** por Bruno Menezes Noronha via PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47) (commit `4f78c4ed2de5b2e9f358f55accfa021b027fc124`, merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI verde run `34983136046`).
**Decisões e requisitos materializados:** `D-2.3D-20` (`docs/12` §5.20), critérios de aceite normativos `A-01`..`A-22` (`docs/12` §10.5).
**Zero drift:** `packages/database` com diff vazio; zero migration nova (10 migrations preservadas); `schema.prisma`, `schema.golden.sql` e `protected-objects.json` estritamente idênticos; zero dependência adicionada em `package.json` ou lockfiles.

### 6-S.1 Componentes e arquitetura de runtime

1. **Contrato DTO (`apps/api/src/auth/auth.dto.ts`)**:
   - `ConsultarSessaoRespostaDto`: tipado para OpenAPI v3.1 com `usuarioId: string` (uuid v4) e `sessaoId: string` (uuid v4).

2. **Endpoint no Controller (`apps/api/src/auth/auth.controller.ts`)**:
   - Endpoint: `GET /auth/sessao`.
   - Método seguro HTTP: safe GET method sem CSRF guard, sem exigência de `X-TLF-Requisicao`, sem corpo de requisição, sem synchronizer token.
   - Proteção CSRF refatorada: `@UseGuards(ProtecaoCsrfGuard)` e `@ApiHeader(...)` movidos do nível de classe para os métodos mutantes específicos (`@Post("login")` e `@Post("logout")`).
   - Autenticação: protegido por `@UseGuards(SessaoAutenticadaGuard)`.
   - Cabeçalhos de resposta: `@HttpCode(HttpStatus.OK)`, `@Header("Cache-Control", "no-store")`.
   - OpenAPI v3.1: `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@ApiResponse({ status: 200, type: ConsultarSessaoRespostaDto })`, `@ApiResponse({ status: 401 })`, `@ApiResponse({ status: 500 })`.
   - Extração do contexto autenticado: `@UsuarioAutenticado() contexto: ContextoAutenticado`.
   - Resposta: `{ usuarioId: contexto.usuarioId, sessaoId: contexto.sessaoId }`.
   - Emissão de cookies: nenhuma (`Set-Cookie` ausente).
   - Política estrita de auditoria: expressamente **PROIBIDO** emitir evento de auditoria para a consulta de sessão (`usuario.sessao.consultada` proibida).

3. **Módulo de Autenticação (`apps/api/src/auth/auth.module.ts`)**:
   - `SessaoAutenticadaGuard` adicionado diretamente aos `providers` de `AuthModule`, preservando os `imports` (`[DatabaseModule, AuditModule]`) intactos e sem dependências circulares.

4. **Filtro de Erros (`apps/api/src/auth/erro-autenticacao.filter.ts`)**:
   - Predicado `ehOperacaoDaFronteira` estendido para reconhecer `GET /auth/sessao`, garantindo envelope uniforme `{ erro: "FALHA_INTERNA" }` em caso de erro 500 inesperado.

5. **Prova E2E Real Same-Origin (`apps/web/scripts/verify-web-api-e2e.mjs`)**:
   - Executa sobre infraestrutura real completa:
     - PostgreSQL 18 real em contêiner descartável (`instancia-descartavel.mjs`) com migrações aplicadas (`migrateDeploy`).
     - Administrador provisionado via CLI real compilado (`apps/api/dist/provisionamento/cli.js provisionar`).
     - Backend NestJS real compilado (`node apps/api/dist/main.js`) rodando em porta dinâmica (`node:net`).
     - Frontend Next.js real (`apps/web/node_modules/next/dist/bin/next start`) rodando em porta dinâmica (`node:net`) com `URL_API_INTERNA` apontando para a API.
     - Route Handler do proxy same-origin de Next.js (`apps/web/app/api/[...caminho]/route.ts`) intermediando todas as chamadas.
     - `ProtecaoCsrfGuard` real do backend em execução.
     - Cookie de sessão real emitido pelo backend, repassado pelo proxy Next.js e reenviado nas consultas.
   - Cenários cobertos:
     1. Login com sucesso via proxy same-origin, conferindo `Host` repassado, `X-TLF-Requisicao`, extração e validação do cookie de sessão.
     2. `GET /api/auth/sessao` via proxy same-origin com cookie válido: retorna 200, `Cache-Control: no-store`, `{ usuarioId, sessaoId }` com UUIDs idênticos aos persistidos, sem `Set-Cookie`, sem cabeçalhos CORS.
     3. `GET /api/auth/sessao` sem cookie: retorna 401 com envelope `{ erro: "SESSAO_INVALIDA" }`.
     4. `GET /api/auth/sessao` com cookie inválido/malformado: retorna 401 com envelope `{ erro: "SESSAO_INVALIDA" }`.
     5. `POST /api/auth/logout` com sucesso via proxy same-origin, seguido de `GET /api/auth/sessao` com o cookie revogado: retorna 401 com envelope `{ erro: "SESSAO_INVALIDA" }`.
     6. Verificação de guarda CSRF em mutação no proxy: recusa de `POST /api/auth/login` sem `X-TLF-Requisicao` (403) e recusa com `Origin` divergente do `Host` (403).
   - Integrado ao root `package.json` como `"verify:web-api-e2e"` e ao fluxo `.github/workflows/ci.yml`.

### 6-S.2 Critérios de Aceite Normativos (A-01 a A-22)

Todos os 22 critérios de aceite normativos de `docs/12` §10.5 foram integralmente satisfeitos e provados:
- `A-01`: Endpoint `GET /auth/sessao` implementado e exposto em `apps/api`.
- `A-02`: Método HTTP seguro `GET` sem exigência de cabeçalho `X-TLF-Requisicao`.
- `A-03`: Rota `GET /auth/sessao` não submetida à `ProtecaoCsrfGuard` nem a synchronizer token.
- `A-04`: Protegida obrigatoriamente por `SessaoAutenticadaGuard`.
- `A-05`: Requisição com cookie de sessão válido retorna HTTP 200.
- `A-06`: Resposta HTTP 200 contém exclusivamente `{ usuarioId: string, sessaoId: string }` em conformidade com o DTO.
- `A-07`: `usuarioId` e `sessaoId` correspondem aos identificadores autoritativos da sessão e do usuário no banco.
- `A-08`: Cabeçalho `Cache-Control: no-store` presente na resposta HTTP 200.
- `A-09`: Resposta HTTP 200 não emite cookies (`Set-Cookie` ausente).
- `A-10`: Requisição sem cookie de sessão retorna HTTP 401 (`SESSAO_INVALIDA_OU_EXPIRADA`).
- `A-11`: Requisição com cookie de sessão malformado, inexistente ou inválido retorna HTTP 401.
- `A-12`: Requisição com sessão revogada retorna HTTP 401.
- `A-13`: Requisição com sessão expirada temporalmente retorna HTTP 401.
- `A-14`: A consulta de sessão não emite qualquer evento de auditoria (`usuario.sessao.consultada` expressamente proibida).
- `A-15`: Contrato OpenAPI v3.1 reflete `GET /auth/sessao` com respostas 200, 401 e 500.
- `A-16`: Prova E2E automatizada (`verify-web-api-e2e.mjs`) executa PostgreSQL 18 real em contêiner descartável.
- `A-17`: Prova E2E executa build real compilado de `apps/api` (`node apps/api/dist/main.js`).
- `A-18`: Prova E2E executa servidor real Next.js (`next start`) com Route Handler do proxy same-origin.
- `A-19`: Prova E2E executa `ProtecaoCsrfGuard` real em `apps/api` contra requisições repassadas pelo proxy.
- `A-20`: Prova E2E valida fluxo completo: login → obtenção do cookie → consulta de sessão 200 → logout → consulta de sessão 401.
- `A-21`: 4 mutation challenges executados, comprovadamente reprovados pelos mutantes e revertidos por hash SHA-256.
- `A-22`: Bateria integral local verde: typecheck, lint, build, testes unitários, testes de integração e prova E2E.

### 6-S.3 Mutation Challenges Obrigatórios (M1..M4)

Os 4 mutation challenges obrigatórios estabelecidos em `docs/12` §10.5 foram testados e verificados:
- **M1 (Proteção de sessão):** Remoção de `@UseGuards(SessaoAutenticadaGuard)` de `GET /auth/sessao`. Reprovado imediatamente pelos testes unitários e de integração.
- **M2 (Integridade do Host no Proxy):** Adulteração do cabeçalho `Host` repassado pelo proxy same-origin de Next.js. Reprovado pela prova E2E no teste de login/CSRF.
- **M3 (Guarda CSRF — X-TLF-Requisicao):** Supressão da validação de `X-TLF-Requisicao` em `ProtecaoCsrfGuard`. Reprovado pelo teste de mutação CSRF na prova E2E.
- **M4 (Guarda CSRF — Origin estrita):** Aceitação de `Origin` arbitrária estrangeira em `ProtecaoCsrfGuard`. Reprovado pelo teste de mutação CSRF na prova E2E.

---

## 6-T. P-2.3D-09 / AUT-005 / D-2.3D-21 — Ativação e inativação administrativa de usuário

**Estado:** **CONCLUÍDA / HOMOLOGADA / INTEGRADA NA `main` em 17/09/2026** por Bruno Menezes Noronha via PR [#52](https://github.com/BrunoMNoronha/techlab-fisio/pull/52) (commits da fatia `7d16ded` (implementação e testes) e `dbef1fb9c4483bd2d9c261a77fb11270b7ffef70` (documentação), merge commit **`817da427de84fe452230df233cb4ae15c8a43593`** (merge commit, sem squash/rebase), 17/09/2026 03:08:37Z, CI verde na run `35176158028`).
**Decisões e requisitos materializados:** AUT-005; `D-2.3D-21` (`docs/12` §5.21, `D21-01`..`D21-03`), critérios `A-01`..`A-16` (`docs/12` §10.6).
**Zero drift:** `packages/database` intocado; nenhuma migration, alteração de schema, golden ou protected-objects; nenhuma dependência nova; nenhuma permissão ou ação de auditoria nova (`usuarios.gerenciar` e `usuario.situacao.alterada` já existiam).

### 6-T.1 Componentes

1. **`apps/api/src/auth/usuarios.module.ts`** — módulo próprio (`AuthzModule` + `AuditModule`), registrado no `AppModule`; o `AuthModule` não ganha rota.
2. **`apps/api/src/auth/usuarios.controller.ts`** — `PATCH /auth/usuarios/:usuarioId/situacao`, `@RequerPermissao("usuarios.gerenciar")`, `ProtecaoCsrfGuard`, ator via `@UsuarioAutenticado`; mapeia `USUARIO_INEXISTENTE` → `404` e `AUTO_INATIVACAO_PROIBIDA` → `422`; OpenAPI com `200, 400, 401, 403, 404, 413, 422, 500`.
3. **`apps/api/src/auth/usuarios.dto.ts`** — DTOs de requisição/resposta/erro e validação pura fail-closed (UUID; corpo exatamente `{ ativo: boolean }`).
4. **`apps/api/src/auth/usuarios.service.ts`** — transação única: `SELECT ... FOR UPDATE` do alvo; `D21-01`; `D21-03` (no-op sem mutação nem auditoria); reativação (`D21-02`) ou inativação + revogação em massa; `usuario.situacao.alterada` com `contexto` vazio na mesma transação.
5. **`SessaoService.revogarTodasPorAdministradorEm`** — dois UPDATEs condicionais sobre a transação do chamador: vencidas → `EXPIRADA`; demais `ATIVA` → `REVOGADA` com `revogada_por_usuario_id` = Administrador executor.

### 6-T.2 Verificação

- `typecheck` e `build` verdes.
- Unitários: 35 suítes / 847 testes verdes (inclui `usuarios.service.spec.ts` e o bloco AUT-005 de `openapi.spec.ts`).
- Integração (PostgreSQL 18 real): 13 suítes / 436 verdes, 2 puladas por desenho no Windows (`provisionamento-cli`); `usuarios-situacao.integration.spec.ts` integralmente verde, incluindo rollback por falha de auditoria e concorrência (inativações simultâneas; login em voo).
- `verify:openapi-runtime`: 24/24.
- Provas de fronteira atualizadas: `auth.module.integration.spec.ts`, `openapi.spec.ts` e `verify-openapi-runtime.mjs` passam a exigir exatamente a rota nova.

### 6-T.3 Reforço de provas pós-integração (REV. 47)

**Estado:** **INTEGRADO NA `main` em 17/09/2026** — PR [#55](https://github.com/BrunoMNoronha/techlab-fisio/pull/55), commit `4d95c386d15d08060c319300a09a1f4eef5f3590` (somente `apps/api/test/integration/usuarios-situacao.integration.spec.ts`), merge commit **`bce22e8265317be16bf78fcb1861aafdc4b05b0c`** (merge commit, sem squash/rebase), 17/09/2026 03:54:24Z, CI verde na run `35179564801`. Sem mudança normativa: `D-2.3D-21` e `A-01`..`A-16` inalterados; detalhamento factual em `docs/12` §10.6.1.

- **Provas acrescentadas** em `usuarios-situacao.integration.spec.ts` (7 testes; suíte passa de 12 para 19): rollback conjunto por falha na revogação em massa; C2 ativação × inativação; C5 revogação individual (`D-2.3D-19`) × inativação; C6 conclusão de recuperação (T-07) × inativação; C7 requisições autenticadas em voo; C3 e C6 **determinísticos** (ordem adversarial forçada por spy, independente de agendamento). Cenários concorrentes aleatórios repetidos em 5 rodadas.
- **Mutation challenges** aplicados e revertidos localmente, todos detectados: M1 ator do path (13 falhas); M2 permissão trocada (múltiplas); M3 sem revogação (6); M4 auditoria fora da transação (2); M5 lock do login sem `ativo = true` (2); M6 `contexto` não homologado (11); M7a `inativado_por_usuario_id = NULL` (7); M7b sessões apagadas (5).
- **Verificação local:** `typecheck` verde; unitários 847/847; integração 443 aprovados e 2 ignorados preexistentes.
- **Nenhum defeito real encontrado**; nenhum código de produção alterado. Zero drift.

## 6-U. CI-E2E0 — Smoke E2E Playwright obrigatório na CI

**Estado:** **INTEGRADO NA `main` em 17/09/2026** — três PRs, todas por merge commit (sem squash/rebase):

| PR | Conteúdo | Merge commit | CI |
| --- | --- | --- | --- |
| [#58](https://github.com/BrunoMNoronha/techlab-fisio/pull/58) | `ci.yml`: `playwright install --with-deps chromium` → `pnpm --filter @techlab-fisio/web run test:e2e` após o build; em falha, artefato `playwright-report-ci-e2e0` (`playwright-report/` + `test-results/`, 7 dias) | `4156689f78fae9f27241d7ed85e417b9486ede98` (04:26:50Z) | run `35181522998` verde |
| [#59](https://github.com/BrunoMNoronha/techlab-fisio/pull/59) | `F-E2E0-01`: `trace` passa de `on-first-retry` (inerte com `retries: 0`) para `retain-on-failure` | `63bb058c2134aac9c57f8fca8245588c915826e5` (04:41:17Z) | run `35182242657` verde — `Running 3 tests` / `3 passed` |
| [#61](https://github.com/BrunoMNoronha/techlab-fisio/pull/61) | `README.md` e `apps/web/README.md` sincronizados; gatilho `pull_request` para `main` no `ci.yml` | `e7abe300e49b0fb1427fc19ef7843774b67212d5` (05:06:36Z) | runs `35184127602` (push) e `35184130714` (pull_request) verdes |

**Suíte:** `apps/web/e2e/smoke.spec.ts` (`FRONT-E2E0`, 3 testes: `/`, `/login`, 404), Chromium único, `workers: 1`, `retries: 0`, `forbidOnly` sob `CI`; `webServer` = `next start` do build de produção em `127.0.0.1:3100`, readiness pela URL (120 s), `reuseExistingServer: false`.

**Verificação local (17/09/2026, reprodução integral do job `integracao` com `CI=true`, sobre `63bb058` + documentação da PR #61):** todos os 16 passos com exit 0 — inclusive unitários 847/847, `test:api` 87/87, `verify:web-api-e2e` 24/24 e `test:e2e` 3/3. **Prova contra falso positivo:** spec temporário com falha (não versionado, removido) → `1 failed, 3 passed`, exit 1 propagado por `pnpm --filter`.
...
## 6-V. P-2.3D-10 / AUT-002 / D-2.3D-22 — Listagem administrativa das sessões ativas de um usuário

**Estado:** **INTEGRADA NA `main` em 17/09/2026** — PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), commits da fatia `254a677` (implementação e testes), `58c4741` (`docs/12` REV. 18) e `400377e` (`docs/10` REV. 49), merge commit **`bc6a763f3525b16384022ad5b43b95a542be5b2f`** (merge commit, sem squash/rebase, por auto-merge autorizado por Bruno Menezes Noronha), 17/09/2026 05:48:03Z, CI verde nas runs `35186845908` (push) e `35186860133` (pull_request) (§6-V.4). Implementada e validada na branch `agent/sessoes-admin-next`, base `origin/main` `21e2fa0`. Decisão normativa: `docs/12` §5.22; critérios de aceite: `docs/12` §10.7.

### 6-V.1 Runtime

- **Rota:** `GET /auth/usuarios/:usuarioId/sessoes` → `200 { sessoes: [{ sessaoId, criadaEm, ultimaAtividadeEm, expiraEm }] }`, `Cache-Control: no-store`; `400` para UUID inválido; `401`/`403`/`500` pelos contratos vigentes.
- **Controller:** a rota vive no `SessoesController` existente, cujo prefixo passou de `auth/sessoes` para `auth` (a revogação continua em `DELETE /auth/sessoes/:sessaoId`, contrato inalterado). A `ProtecaoCsrfGuard` deixou de ser guard de classe e passou a ser declarada **somente** no handler `DELETE`, **abaixo** de `@RequerPermissao` — decorators aplicam de baixo para cima e o Nest concatena guards nessa ordem, de modo que a CSRF continua avaliada **antes** da sessão e da permissão (provado pela metadata `__guards__` e por `L-14`). A fronteira do `SessoesModule` (imports, controllers, exports) permanece igual.
- **Service:** `SessoesService.listarSessoesAtivasDoUsuario` consulta `sessao_autenticacao` por `usuario_id`, `estado = ATIVA`, `expira_em > agora` e `ultima_atividade_em > agora - POLITICA_SESSAO.timeoutOciosoMs`, com o `RELOGIO_SESSAO` injetado (exportado por `AuthModule` via `AuthzModule`), projeção `id/criada_em/ultima_atividade_em/expira_em` e ordem `criada_em, id`. **Somente leitura**: não persiste a detecção de expiração e não emite auditoria.
- **DTOs:** `SessaoAtivaUsuarioDto` e `ListarSessoesUsuarioRespostaDto` em `sessoes.dto.ts`.

### 6-V.2 Provas

- Integração com PostgreSQL 18 real: `test/integration/sessoes-listagem.integration.spec.ts`, `L-01`..`L-14`.
- Unitários: `sessoes.service.spec.ts` (predicados, projeção, sem escrita/auditoria), `sessoes.controller.spec.ts` (400, lista vazia, serialização ISO), `sessoes-dto.spec.ts` (permissão da listagem, guards sem CSRF no GET, ordem CSRF → sessão → permissão no DELETE).
- Contrato: `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` passam a exigir exatamente a rota nova, seus status `200,400,401,403,500` e a projeção fechada do DTO.
- Mutation challenges M1..M4 detectados e revertidos por SHA-256 (`docs/12` §10.7). Uma primeira execução combinada registrou falhas em massa não atribuíveis às mutações; a reexecução imediata com as mesmas mutações isolou exatamente as 6 falhas esperadas.

### 6-V.3 Verificação local

- `pnpm typecheck` verde; `pnpm --filter @techlab-fisio/api run build` verde.
- `test:api`: 35 suítes / 861 testes verdes.
- `verify:api-integration` (PostgreSQL 18 descartável): 14 suítes / 455 aprovados / 2 pulados preexistentes.
- `verify:openapi-runtime`: 26/26.
- Zero drift: `packages/database`, migrations, golden, `protected-objects.json`, lockfile, `.github/workflows/**`, `playwright.config.*` e testes E2E intocados.

### 6-V.4 Registro pós-integração e correções da revisão automatizada (REV. 51)

- **Integração:** PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), commits da fatia `254a677` (implementação e testes), `58c4741` (`docs/12` REV. 18) e `400377e` (`docs/10` REV. 49), merge commit **`bc6a763f3525b16384022ad5b43b95a542be5b2f`** (merge commit, sem squash/rebase, por auto-merge autorizado por Bruno Menezes Noronha), 17/09/2026 05:48:03Z, CI verde nas runs `35186845908` (push) e `35186860133` (pull_request).
- **Revisão:** revisão automatizada do Codex sobre `400377e`, recebida após o merge, com dois achados P2: **(1)** falha técnica da listagem saía no corpo padrão do Nest (`{"statusCode":500,"message":...}`) porque `FiltroErroAutenticacao` não reconhecia a rota — **defeito real, corrigido**: a rota exata `GET /auth/usuarios/:usuarioId/sessoes` entra no escopo fechado do filtro, com `L-15` (integração) e testes unitários de escopo; mutação que remove a rota do filtro detectada por `L-15`; **(2)** "a consulta não escreve nada" era afirmado sem ressalva, mas a guarda de sessão do **operador** registra atividade (e detecta expiração) como em toda rota autenticada, inclusive `GET /auth/sessao` (`D-2.3D-04`/`D-2.3D-20`) — **esclarecimento de escopo, sem alteração de comportamento nem de decisão**: a ausência de escrita refere-se às sessões do **alvo**; provado por `L-16`.
- **Arquivos da correção:** `src/auth/erro-autenticacao.filter.ts` (constante `ROTA_LISTAGEM_SESSOES_USUARIO`, casamento exato de um segmento), `test/erro-autenticacao.filter.spec.ts` (rota interceptada; escopo não ampliado para POST, sub-rotas ou segmentos extras), `test/integration/sessoes-listagem.integration.spec.ts` (`L-15`, `L-16`).
- **Verificação local:** ver REV. 51 (§11). Mutação M5 detectada por `L-15`; como nas rodadas anteriores, uma execução de integração logo após outra apresentou falhas em massa não atribuíveis à mutação, e a reexecução isolou exatamente `L-15`.
...
### 6-X.5 Registro pós-integração de `CFG-001A` e `CFG-001B` (17/09/2026)

Registro **exclusivamente factual** (`CFG-POST1`), no precedente `MEDIR → REGISTRAR` de §7.6.7. Nenhuma medição de §6-W.1..§6-W.5 e §6-X.1..§6-X.4 foi reescrita; os estados "IMPLEMENTADA/O E MEDIDA/O EM BRANCH PRÓPRIA — NÃO INTEGRADA/O" daquelas subseções ficam como registro histórico.

| Item | `CFG-001A` | `CFG-001B` |
| --- | --- | --- |
| PR | [#65](https://github.com/BrunoMNoronha/techlab-fisio/pull/65) — "feat(api): dados da clinica unica — GET/PUT /clinica (CFG-001A)"; `MERGED` por `BrunoMNoronha` em 17/09/2026 06:11:48Z | [#69](https://github.com/BrunoMNoronha/techlab-fisio/pull/69) — "feat(api): provisionar a linha unica da clinica via CLI (CFG-001B)"; `MERGED` por `BrunoMNoronha` em 17/09/2026 07:28:13Z |
| HEAD da PR | `a8269a5e525f305230a9f24640bcb7019a7b28ab` | `abb3a8a72329f0378b3d9f6ae8f788d1cc6c9a81` |
| Integração | commit **`6ee19f27afc5d7c55667ef910536baa924ecc990`** — **squash** (pai único `bc6a763`); árvore idêntica à do HEAD da PR (`55b29513`) | merge commit **`02cc93d5770003775d3417b1d2ee08a874675d30`** (pais `4f1721b`, `abb3a8a`); árvore idêntica à do HEAD da PR (`b4834700`) |
| CI | sobre o HEAD `a8269a5`: runs [35188452695](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35188452695) (`push`) e [35188456326](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35188456326) (`pull_request`), `success` | sobre o HEAD `abb3a8a`: runs [35194249385](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35194249385) (`push`) e [35194253079](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35194253079) (`pull_request`), `success` |
| Artefatos presentes na `main` | migration `20260917060000_clinica_linha_unica` (`ux_clinica_linha_unica`); `apps/api/src/clinica/**` (`GET`/`PUT /clinica` sob `clinica.configurar`, `ProtecaoCsrfGuard` só no `PUT`); `clinica-dto.spec.ts`; `integration/clinica-configuracao.integration.spec.ts` | `provisionamento/bootstrap-clinica.{module,service}.ts`; subcomando `bootstrap-clinica` em `provisionamento/cli.ts`; `bootstrap-clinica.service.spec.ts`; `integration/provisionamento-clinica.integration.spec.ts` |

- **Validações:** nenhuma CI foi executada sobre os commits de integração `6ee19f2` e `02cc93d` (`gh run list --commit` vazio para ambos); as baterias de §6-W.4 e §6-X e as runs acima referem-se aos HEADs das PRs. A equivalência com a `main` decorre da identidade de árvore medida, não de execução sobre o merge.
- **Estados vivos:** `CFG-001A` e `CFG-001B` **INTEGRADAS**. A limitação de §6-W.5 "provisionamento sem fatia atribuída" foi **superada** pela integração de `CFG-001B`. Seguem vigentes as demais limitações de §6-W.5 e §6-X.4; logotipo e duração padrão (`D-CFG-07`) **FUTURO DO MVP**; `CFG-002`..`CFG-005` **NÃO INICIADOS**; inclusão da clínica em `provisionar` **não autorizada** (`D-CFG-12`).

## 6-Y. `CFG-002` — Horário de funcionamento da clínica

**Estado: IMPLEMENTADO E MEDIDO EM BRANCH PRÓPRIA (`agent/cfg-002-horario-funcionamento`, a partir de `main` = `788915b`) — NÃO INTEGRADO.** Decisões: `docs/14` §3.10 (`D-CFG-13`..`D-CFG-21`). Auditoria: `docs/09` §13.6 com a leitura homologada de `D-CFG-17`.

### 6-Y.1 Componentes

- **Persistência:** tabela `horario_funcionamento` e CHECKs protegidos **preexistentes**; **nenhuma migration**, golden, `schema.prisma` ou `protected-objects.json` alterado.
- **`apps/api/src/clinica/horario-funcionamento.dto.ts`:** validação pura e fail-closed — corpo estrito `{ janelas }`, janela estrita `{ diaSemana, horaInicio, horaFim }`; `diaSemana` inteiro 0..6 sem coerção; `HH:MM` 00:00..23:59; `horaFim > horaInicio`; no máximo 4 janelas/dia (28 no total); sem sobreposição e sem adjacência no mesmo dia; devolve a grade em ordem canônica (dia, início).
- **`horario-funcionamento.service.ts`:** transação única — `SELECT id FROM clinica FOR UPDATE` → leitura da grade sob o lock → grade idêntica = no-op (sem escrita, sem evento) → `DELETE` das linhas da clínica + `INSERT` das novas → `AuditWriter.registrar` com `configuracao.alterada`, `alvo_tipo = clinica`, `alvo_id = clinica.id`, `contexto` vazio.
- **`horario-funcionamento.controller.ts`:** `GET /horario-funcionamento` (sem CSRF, `Cache-Control: no-store`) e `PUT /horario-funcionamento` (`ProtecaoCsrfGuard` antes da sessão), ambos sob `@RequerPermissao("clinica.configurar")`; reutiliza `FiltroErroClinica`, `ErroClinicaDto` e `CLINICA_NAO_CONFIGURADA`.
- **Registro:** `ClinicaModule` passa a declarar o controller e o serviço. Listas fechadas de rotas em `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` incluem `/horario-funcionamento`.
- **Escolhas técnicas locais (reversíveis, sem decisão de produto):** forma `{ janelas: [...] }` do corpo e da resposta; resposta ordenada por dia e início; identificadores das linhas gerados por `randomUUID()` (v4) no INSERT bruto.

### 6-Y.2 Critérios × prova

| Critério | Prova | Status |
| --- | --- | --- |
| 401 sem sessão; 403 sem `clinica.configurar`; 403 CSRF no `PUT`, avaliada antes da sessão (`D-CFG-21`) | integração | **OK** |
| Grade vazia → `200 { janelas: [] }`; sem clínica → `404 CLINICA_NAO_CONFIGURADA` (`D-CFG-21`) | integração | **OK** |
| `GET` ordenado, `HH:MM`, `no-store`, sem auditoria | integração | **OK** |
| Substituição persiste, remove as linhas anteriores e emite 1 evento alvo `clinica` sem valores (`D-CFG-15`, `D-CFG-17`) | integração | **OK** |
| Grade idêntica em qualquer ordem → sem escrita e sem evento | integração | **OK** |
| Sobreposição, adjacência, meia-noite, segundos, dia fora de 0..6, coerção, 5 janelas/dia, chaves extras → `400` sem mutação (`D-CFG-13`, `D-CFG-14`, `D-CFG-18`) | unitário (31) + integração | **OK** |
| Falha da auditoria → `500 FALHA_INTERNA` e grade anterior intacta | integração (sabotagem do `AuditWriter`) | **OK** |
| Leitura sob lock (bloqueio real observado em `pg_stat_activity`); PUTs concorrentes serializados, sem mistura de grades | integração | **OK** |
| OpenAPI: `GET` 200/401/403/404/500, `PUT` 200/400/401/403/404/413/500, CSRF só no `PUT` | `openapi.spec.ts` + `verify:openapi-runtime` | **OK** |

### 6-Y.3 Mutation challenges (aplicados, observados e revertidos; SHA-256 conferido)

| # | Mutação | Resultado |
| --- | --- | --- |
| M1 | aceitar adjacência (`<=` → `<`) | **DETECTADA** (unitário) |
| M2 | limite de 5 janelas/dia | **DETECTADA** (unitário) |
| M3 | aceitar `horaFim = horaInicio` | **DETECTADA** (unitário) |
| M4 | remover `typeof` e `Number.isInteger` de `diaSemana` | **DETECTADA** (unitário, 2 testes). Remover só o `typeof` é mutante equivalente (`Number.isInteger` já rejeita não números) |
| M5 | remover `FOR UPDATE` | **DETECTADA** (integração, 2 testes) |
| M6 | remover a auditoria | **DETECTADA** (integração, 16 testes) |
| M7 | tratar grade idêntica como alteração | **DETECTADA** (integração, 2 testes) |

As mutações de integração rodaram só a suíte `horario-funcionamento` (24 testes) em instância descartável, com uma cópia temporária do script de integração, removida depois.

### 6-Y.4 Baterias medidas (17/09/2026, host Windows, árvore da branch)

- `pnpm run typecheck`: **verde**.
- `pnpm run test:api`: **39 suites · 1079 passed · 3 skipped** (os 3 são os testes de `dist/` de `provisionamento.fronteira.spec.ts`, pulados porque `dist/` ainda não existia naquela execução).
- `pnpm run verify:api-integration` (PostgreSQL 18 descartável, `tlf_app`): **exit 0 — 18 suites · 568 passed · 2 skipped** (inclui `horario-funcionamento.integration.spec.ts`, **24 testes**).
- `verify:openapi-runtime` sobre `dist/`: **32 verificações OK**.
- `lint:migrations` (Guarda 1): **OK**, 11 migrations, nenhuma remoção de objeto protegido.
- A CI da PR é a prova canônica.

### 6-Y.5 Limitações declaradas

- **Atualização perdida** entre administradores concorrentes não é detectada (última grade válida prevalece, como em `D-CFG-05`).
- **Estado anterior** da grade não é preservado na trilha (`docs/09` §13.6) e as linhas substituídas são removidas fisicamente (`D-CFG-15`).
- Linhas gravadas fora da API com segundos seriam lidas truncadas em `HH:MM`; a API nunca as produz.
- **Erros do body parser** (JSON malformado, `413`) seguem com o corpo padrão da plataforma, como em §6-W.5.
- **Fora da fatia:** exceções/feriados (`D-CFG-19`); aplicação de RN-014 pela agenda e reavaliação da troca de fuso (`D-CFG-20`); CFG-003..CFG-005.

### 6-Y.6 Registro pós-integração (17/09/2026)

Registro **exclusivamente factual**, no precedente `MEDIR → REGISTRAR` de §6-X.5. Nenhuma medição de §6-Y.1..§6-Y.5 foi reescrita.

| Item | Fato medido (`git`/`gh`, 17/09/2026) |
| --- | --- |
| PR | PR [#76](https://github.com/BrunoMNoronha/techlab-fisio/pull/76) — "feat(api): horário de funcionamento da clínica (CFG-002)"; `MERGED` por `BrunoMNoronha` em 17/09/2026 08:41:22Z |
| Commits da fatia | `e546973` — implementação, testes e registro (`docs/10` REV. 56, `docs/14`); `12948f4` — merge de `origin/main` (`7fa114c`, com CFG-PREP3 e correções de `docs/14`), conflito só em `docs/14` §5/§6, resolvido preservando as duas frentes (a revisão desta fatia passou a `docs/14` REV. 10). Sem rebase e sem force-push |
| Integração | merge commit **`ad2bcf8041635f469db6c799e7405f455d836d30`** (pais `7fa114c`, `12948f4`), com `--match-head-commit` no HEAD validado; **árvore idêntica à do HEAD `12948f4`** (`3e542caf`) |
| CI | sobre `e546973`: run [35198946802](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35198946802) (`push`) `success`. Sobre o HEAD `12948f4`: runs [35200219052](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35200219052) (`push`) e [35200224060](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35200224060) (`pull_request`), `success`. **Nenhuma** run sobre o merge commit `ad2bcf8` (`gh run list --commit` vazio); a equivalência decorre da identidade de árvore |
| Validação local | As baterias de §6-Y.4 foram medidas sobre `e546973`, **antes** do merge de `origin/main`; o merge alterou apenas `docs/14` |
...
### 6-Z.7 Registro pós-integração (17/09/2026)

Registro **exclusivamente factual**, no precedente `MEDIR → REGISTRAR` de §6-Y.6. Nenhuma medição de §6-Z.1..§6-Z.6 foi reescrita; o estado "NÃO INTEGRADA" de §6-Z fica como registro histórico.

| Item | Fato medido (`git`/`gh`, 17/09/2026) |
| --- | --- |
| PR | PR [#80](https://github.com/BrunoMNoronha/techlab-fisio/pull/80) — "feat(api): catálogo de serviços — /servicos (CFG-003)"; `MERGED` por `BrunoMNoronha` em 17/09/2026 09:19:13Z |
| Commits da fatia | `92754c0` — implementação, migration, golden e testes; `ea1f76a` — registro (`docs/10`, `docs/14`); `c46e42c` — correção da resolução de merge em `openapi.spec.ts` e §6-Z.6; `ba0ae3b` — remoção de linha residual em `docs/14` §5; merges de `origin/main` `2e934f6` (PR #76, conflitos em listas de rotas e documentos), `a47e528` (PR #77), `732230d` (PR #78), `50415b5` (PR #79) e `10baa64` (PR #81) — os quatro últimos com conflito só documental. Sem rebase e sem force-push |
| Integração | merge commit **`fd3c205f420b9cb201dad46fe946b3df7354b9ba`** (pais `727d2df`, `10baa64`), com `--match-head-commit` no HEAD validado; **árvore idêntica à do HEAD `10baa64`** (`edbce97a`) |
| CI | sobre o HEAD `10baa64`: runs [35203981793](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35203981793) (`push`) e [35203985987](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35203985987) (`pull_request`), `success`. CI verde também sobre os HEADs intermediários `a47e528`, `ba0ae3b` e `50415b5`. **Nenhuma** run sobre o merge commit `fd3c205` (`gh run list --commit` vazio); a equivalência decorre da identidade de árvore |
...
## 6-AI. `AGD-A` — Núcleo inicial da Agenda (consulta, criação avulsa, confirmação, remarcação e cancelamento)

**Estado atual: AGD-A integrada na `main` pela PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`; AGD-B integrada pela PR [#99](https://github.com/BrunoMNoronha/techlab-fisio/pull/99), merge `ab6c60143d7195cfaf43fd809119ded7ec4ad54b`.** O estado local da REV. 72 permanece preservado como histórico. As decisões `D-AGD-01`..`D-AGD-17` permanecem homologadas; AGD-B materializa somente check-in e falta conforme a autorização da issue #91, sem ampliar AGD-C/D/E e sem criar permissão, ação de auditoria, migration, schema ou dependência.

Dependências confirmadas na `origin/main` **antes** de implementar, por leitura do próprio commit `4b2e2c0` (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89)): `CFG-004`, `CFG-005`, `PAC-A`, `PRO-A` e `PRO-003` — controllers, migrations e regras presentes. `FA-05` de `docs/15` ("nenhuma rota de pacientes, profissionais, disponibilidade, motivos ou agenda") descreve a **base medida em 17/09/2026** e estava superada quanto a tudo menos a agenda.

### 6-AI.1 Componentes

- **Persistência:** migration `20260917230000_agendamento_cancelamento_coerente` (`D-AGD-07`) — **uma** constraint: `ck_agendamento_cancelamento_coerente`, com a expressão literal da decisão. **Nada além disso:** sem coluna, índice, trigger, função, enum ou privilégio novo; nenhuma migration anterior editada, renomeada ou compactada. A ausência prévia da constraint foi provada no golden versionado (`agendamento` tinha apenas `ck_agendamento_intervalo` e `ck_agendamento_modalidade_pacote`); a ausência de dados legados, por leitura do histórico — nenhuma migration insere em `agendamento` e não há seed ou fixture versionada que a popule. Golden regerado por reconstrução limpa (`schema:golden:update`); **diff de exatamente uma linha**. `schema.prisma`: comentário da CHECK no modelo `Agendamento` (não representável na Prisma Schema Language). `protected-objects.json` e o inventário fixo de `verify:from-scratch` **inalterados** — mesmo precedente de §6-Z.1, §6-AC.1, §6-AD.1 e §6-AG.1.
- **`apps/api/src/agenda/` — módulo novo, e o CONSUMIDOR que faltava:**
  - `agenda.module.ts` (novo): registra `AgendamentosController` e `AgendaOpcoesController` e passa a prover `VerificadorHorarioFuncionamento` (CFG-002, `docs/14` §5) e `VerificadorDisponibilidadeProfissional` (PRO-003, §6-AG.1) — os dois componentes que existiam prontos, testados e **sem consumidor**. As regras foram **integradas, não reimplementadas**: nenhuma linha de `horario-funcionamento.regra.ts`, `disponibilidade-profissional.regra.ts` ou dos dois verificadores foi alterada.
  - `agendamentos.controller.ts` (novo): as seis rotas de `D-AGD-05`, todas sob `@RequerPermissao("agenda.gerenciar")`; CSRF só nas mutações, declarada abaixo da permissão; GETs com `Cache-Control: no-store`; **sem `DELETE`, `PUT` ou `PATCH`**. Ator só da sessão.
  - `agenda-opcoes.controller.ts` / `agenda-opcoes.service.ts` (novos): `GET /agenda/opcoes` (`D-AGD-13`), leitura única e coerente de serviços ativos (sem preço), motivos ativos, grade vigente e fuso.
  - `agendamentos.service.ts` (novo): a ordem literal dos 11 passos de `D-AGD-04`; `FOR UPDATE` no agendamento antes de avaliar o estado; `FOR SHARE` no motivo; tradução restrita do `23P01`; histórico e auditoria na mesma transação.
  - `agenda.estados.ts` (novo): máquina de estados e catálogo fechado de operações — funções **puras**, testáveis sem banco.
  - `agenda.escopo.ts` (novo): `D-AGD-12` materializado sobre a pergunta certa — *quais papéis do ator **concedem** `agenda.gerenciar`* —, com `RestricaoLeitura` de três casos para que "ator sem vínculo" nunca colapse em "sem filtro".
  - `agenda.dto.ts` (novo): corpos estritos, resposta exata, janela de consulta. Reutiliza `analisarInstante` de `audit/audit-query.dto.ts` (AUD-004) em vez de redefinir o formato ISO-8601.
  - `erro-agenda.filter.ts` (novo): contrato fechado `{ erro }`, mesmo desenho dos filtros de controller já vigentes.
- **Registro:** `AgendaModule` no `AppModule`; listas fechadas de rotas em `openapi.spec.ts`, `auth.module.integration.spec.ts` e `verify-openapi-runtime.mjs` sincronizadas para a superfície vigente, incluindo exclusivamente `POST /agendamentos/{agendamentoId}/check-in` e `POST /agendamentos/{agendamentoId}/falta` como ampliação AGD-B, com negativas explícitas para AGD-C/AGD-D/AGD-E e métodos indevidos.
- **Testes existentes ajustados, com o motivo registrado:** `apps/api/test/integration/motivos-cancelamento.integration.spec.ts` — as fixtures que criavam agendamento `CANCELADO` passaram a preencher os três campos de cancelamento, e o teste que afirmava `D-CFG-51` ("cancelado sem motivo continua aceito pelo banco") foi **substituído** pela medição da restrição que passou a valer. `D-CFG-51` foi **superada** por `D-AGD-07`, que a resolve expressamente; nenhuma decisão de CFG-005 foi reaberta.

### 6-AI.2 Decisões locais de implementação, declaradas

1. **Correlação histórico x auditoria sem coluna nova.** `historico_agendamento` **não tem** `correlacao_id`, e esta fatia só foi autorizada a criar o CHECK de `D-AGD-07`. O **mesmo UUID** é usado como `historico_agendamento.id` e como `evento_auditoria.correlacao_id`, de modo que a junção `evento_auditoria.correlacao_id = historico_agendamento.id` é exata e `docs/07` §22.4 fica satisfeito sem tocar no schema. Alternativa registrada e **não** adotada: acrescentar a coluna, o que exigiria decisão estrutural própria.
2. **Códigos de papel repetidos em `agenda.escopo.ts`.** Importar `provisionamento/catalogo-rbac.ts` arrastaria a fatia de provisionamento (CLI, seed, bootstrap de Administrador) para o fecho transitivo da aplicação — o que `provisionamento.fronteira.spec.ts` prova mecanicamente que não pode acontecer. Os dois literais são repetidos e **guardados por teste**: `agenda-estados-escopo.spec.ts` importa o catálogo (é teste, está fora daquele fecho) e falha se um dos códigos divergir.
3. **Estado anterior/novo preenchidos também na remarcação de `AGENDADO`.** `D-AGD-09` manda preencher "quando aplicáveis"; a linha registra o estado antes e depois da operação inclusive quando ele não muda, o que torna o histórico reconstruível sem inferência e uniformiza o caso `CONFIRMADO -> AGENDADO`.
4. **Ordem escopo -> regra temporal -> elegibilidade.** `D-AGD-04` coloca "sessão; permissão; CSRF" no passo 1; o escopo de `D-AGD-12` é decisão de autorização e roda logo após o corpo ser validado, antes de qualquer `422`. Consequência: criar para outro profissional devolve `403` mesmo que o intervalo também fosse inválido.

### 6-AI.3 Matriz `TA-01`..`TA-19` x prova

| TA | Cenário | Prova | Status |
| --- | --- | --- | --- |
| TA-01 | Criação válida por Recepção | integração — `201`, `AGENDADO`, 1 histórico `CRIADO`, 1 `agendamento.criado`, mesmo `correlacao_id`, resposta exata de `D-AGD-05` | **OK** |
| TA-02 | Paciente/profissional/serviço inexistente ou inativo | integração — os seis casos; mesmo código para inexistente e inativo; zero linhas | **OK** |
| TA-03 | Serviço não habilitado | integração — `422 SERVICO_NAO_HABILITADO` sem mutação | **OK** |
| TA-04 | Fora da grade da clínica | integração — dia sem janela, antes, depois e cruzando o intervalo entre janelas; bordas `08:00`/`12:00` aceitas; sem evento | **OK** |
| TA-05 | Fora da disponibilidade | integração — dentro da grade e fora da disponibilidade; profissional sem versão (fail-closed); ordem grade->disponibilidade provada | **OK** |
| TA-06 | Sobre bloqueio | integração — `409 CONFLITO_BLOQUEIO`; borda adjacente aceita; bloqueio de outro profissional não interfere | **OK** |
| TA-07 | Conflito do profissional + borda | integração — quatro sobreposições em `409`; `10:00`/`10:00` em `201`; cancelado libera o horário | **OK** |
| TA-08 | Conflito do paciente | integração — `409 CONFLITO_PACIENTE`, corpo só com `{ erro }` | **OK** |
| TA-09 | Concorrência real | integração contra **PostgreSQL real** — duas requisições HTTP simultâneas; exatamente uma `201`, a outra `409 CONFLITO_PROFISSIONAL`; a perdedora não deixa histórico nem evento | **OK** |
| TA-10 | Início no passado | integração — `422 AGENDAMENTO_NO_PASSADO`; mais seis corpos malformados em `400` | **OK** |
| TA-11 | Confirmação e no-op | integração — histórico sem evento; reconfirmar `200` sem linha nova | **OK** |
| TA-12 | Remarcação válida | integração — histórico `REMARCADO` com antes/depois, evento correlacionado, `CONFIRMADO` volta a `AGENDADO`; mesmo intervalo é no-op | **OK** |
| TA-13 | Remarcação rejeitada | integração — conflito, fora da grade, fora da disponibilidade, bloqueio, passado e profissional inativado; intervalo e estado originais intactos | **OK** |
| TA-14 | Cancelamento | integração — motivo ativo (`200`, quatro campos, histórico, evento); sem motivo (`400`); inexistente, inativo e catálogo vazio (`422`) | **OK** |
| TA-15 | Estado terminal | integração — confirmar/remarcar/cancelar um `CANCELADO` em `409`; remarcar o mesmo intervalo de um terminal é `409`, não no-op; estados de AGD-B/AGD-E também recusam | **OK** |
| TA-16 | Escopo próprio | integração — lista filtrada mesmo com `profissionalId` alheio; `404` por id alheio nas quatro operações; `403` na criação para outro; sem vínculo -> escopo vazio; papel customizado e `RECEPCIONISTA` que não concede a permissão -> próprio | **OK** |
| TA-17 | Autenticação, RBAC e CSRF | integração — `401` nas sete rotas; Gestor `403` sem evento; quatro mutações sem CSRF em `403` sem mutação; GETs com `no-store`; id malformado em `400` | **OK** |
| TA-18 | Janela de consulta | integração — intersecção meio aberta, todos os estados, ordem `inicio, id`; oito consultas inválidas em `400`; 7 dias exatos aceitos; filtro por profissional | **OK** |
| TA-19 | `GET /agenda/opcoes` | integração — só ativos, sem preço, grade e fuso; catálogo vazio de motivos; `404` sem clínica; `/servicos` segue exigindo `clinica.configurar` | **OK** |

Cobertura adicional além da matriz: atomicidade de `agendamento` x `historico_agendamento` x `evento_auditoria` sob falha injetada na auditoria (criação e cancelamento); fronteira da fatia (nenhuma rota de check-in, falta, bloqueio ou atendimento responde; `DELETE` em `404`); `fim` divergente da duração do serviço (20 e 180 min); camada pura de DTO, estados e escopo em dois specs unitários (35 asserções); efeito físico da nova CHECK na Guarda 2 (aceite coerente, quatro rejeições, sentido inverso e o **limite declarado** do resíduo parcial).

### 6-AI.4 Baterias medidas (17/09/2026, host Windows, Node 24.19.0, pnpm 12.3.4, Docker 29.7.2, worktree `techlab-fisio-agd-a` sobre `origin/main` = `4b2e2c0`)

| Verificação | Exit | Resultado |
| --- | ---: | --- |
| `pnpm run typecheck` | 0 | verde (raiz, `packages/database`, `apps/api`, `apps/web`) |
| `pnpm run build` | 0 | verde |
| `pnpm run lint:migrations` | 0 | **18 migrations · 37 objetos protegidos** |
| `pnpm run test:api` | 0 | **54 suites · 1622 passed** (antes: 52 · 1574) |
| `pnpm run schema:verify` | 0 | dump **byte a byte idêntico** ao golden (**62338 bytes**); `migrate diff` exit 0 — **sem drift** |
| `pnpm --filter @techlab-fisio/api verify:openapi-runtime` | 0 | **77 verificações OK** (antes: 67) |
| `pnpm run verify:api-integration` | 0 | **27 suites · 1006 passed · 2 skipped · 0 resíduos** (antes: 26 · 952 · 2 skipped) |
| `pnpm run smoke:api` | 0 | verde |
| `pnpm run verify:from-scratch` | 0 | **18/18 migrations**, catálogo OK, **10 suites · 99 passed**, árvore limpa (antes: 17/17 · 92) |
| `pnpm --filter @techlab-fisio/web test` | 0 | **78 verificações OK** |
| `pnpm run verify:web-api-e2e` | 0 | **50 verificações OK** |
| `pnpm run test:e2e` (Playwright) | 0 | **13 passed** |
| Suíte direcionada de AGD-A | 0 | unitários `agenda*`: **4 suites · 68 passed** (dos quais 35 são os dois specs novos de DTO/estados/escopo); integração `agenda*`: **3 suites · 75 passed**, dos quais **54** são a suíte nova `agenda-agd-a.integration.spec.ts` |

Todas as execuções com instância descartável destruíram a sua: **0 containers e 0 volumes remanescentes**. **CI remota não executada.**

### 6-AI.5 Limitações declaradas

- **Race residual aceita** (`D-AGD-11`, `R2.2-09`, `D-CFG-63`): grade da clínica, disponibilidade do profissional e ausência de bloqueio são validadas **sem lock**. Uma alteração concorrente de qualquer uma das três entre a validação e a gravação **não** é detectada. Somente o conflito temporal **entre agendamentos** é estrutural, garantido pelas exclusion constraints. Limitação **homologada**, não dívida nova.
- **Escopo derivado do código do papel** (`D-AGD-12`): mantido como decidido, com a mesma limitação declarada; `P2.2-05` continua **não iniciada** e nada foi antecipado dela.
- **CHECK de `D-AGD-07` — resíduo parcial tolerado:** a expressão homologada compara a **conjunção** dos três campos; um agendamento não cancelado com apenas `cancelado_em` preenchido mantém a conjunção falsa e **não** é rejeitado. Nenhum caminho da aplicação produz esse estado (o cancelamento grava os quatro campos numa única instrução), e o limite está registrado em teste de efeito e em `docs/07` §10.2 (nota da REV. 2.6). Endurecê-lo exigiria alterar a expressão homologada — decisão própria.
- **`protected-objects.json` não ampliado:** a nova CHECK fica fora das Guardas 1 e 2 do inventário fixo, coberta pelo golden (Guarda 3) e pelo teste de efeito. Acrescenta **um** objeto à dívida transversal já registrada em §6-AF, §6-AG.5 e §6-AH.6, que **exige fatia própria sob autorização expressa**.
- **Fronteira da fatia:** check-in, falta, bloqueio, modalidade `PACOTE`, `reserva_sessao`, iniciar/concluir atendimento, `AGUARDANDO -> CANCELADO`, recorrência, notificações, lista de espera, agendamento público e sugestão automática de horários **não** foram implementados e não têm preparação alguma no código.
- **Sem interface:** AGD-A entrega **apenas** a fronteira HTTP. Nenhuma tela de agenda foi criada em `apps/web`; a suíte Playwright permanece exatamente como estava.
- **Corpo JSON `null` ou malformado** nas mutações responde `400` com o corpo padrão da plataforma, não com `{ erro }`: o body parser rejeita antes do roteamento. Limite já vigente em toda a API.
- **Worktree:** a suíte Jest não roda a partir de um diretório cujo caminho contenha `.claude/worktrees/` — o `testMatch` do Jest é um glob e a barra invertida do caminho o quebra, resultando em `0 matches`. Por isso a fatia foi desenvolvida no worktree dedicado `techlab-fisio-agd-a`, no mesmo padrão das demais frentes do projeto. Nenhum arquivo de configuração foi alterado por causa disso.

---


### 6-AI.6 `AGD-B` — check-in e falta (integrada na `main` em 18/09/2026)

**Publicação medida:** PR [#99](https://github.com/BrunoMNoronha/techlab-fisio/pull/99), merge `ab6c60143d7195cfaf43fd809119ded7ec4ad54b`; CI `ci` do head `c70973b14479ccf614cee9cce0c77150e278fe7d` concluído com sucesso (run `35307940120`). A Base V2 permaneceu inalterada.

- **Contratos HTTP e RBAC:** `POST /agendamentos/:agendamentoId/check-in` exige `agenda.checkin`; `POST /agendamentos/:agendamentoId/falta` exige `agenda.falta`. Corpo estrito `{}`, resposta `200` com a projeção vigente do agendamento; UUID inválido/corpo inválido seguem `400`; sessão, CSRF e permissão preservam os precedentes vigentes.
- **Máquina de estados:** check-in aceita somente `AGENDADO|CONFIRMADO -> AGUARDANDO`; falta aceita somente `AGENDADO|CONFIRMADO -> FALTA`. Check-in repetido, `AGUARDANDO -> FALTA` e demais origens retornam `409 TRANSICAO_INVALIDA`. Estado é validado antes da janela temporal.
- **Tempo e fuso:** check-in exige que `agora` e `inicio` pertençam à mesma data civil no fuso IANA configurado da clínica; falta exige estritamente `agora > inicio`, rejeitando igualdade. Origem válida fora da janela retorna `422 FORA_DA_JANELA_TEMPORAL`.
- **Escopo e persistência:** a transação resolve escopo pela permissão efetiva da operação, lê o agendamento com `SELECT ... FOR UPDATE`, captura `agora` uma única vez após lock/estado, atualiza o estado e insere exatamente uma linha `CHECKIN` ou `FALTA` em `historico_agendamento` na mesma transação. Recurso inexistente e fora do escopo permanecem indistinguíveis por `404`. Check-in/falta geram **zero evento de auditoria**.
- **Concorrência:** provas reais em PostgreSQL 18 para check-in×check-in, falta×falta e check-in×falta produzem exatamente uma aceitação `200`, uma rejeição `409`, uma linha AGD-B de histórico e zero auditoria adicional, sem depender da ordem do vencedor.
- **Cobertura e mutações:** a ampliação de integração do PR #99 executou 18 cenários AGD-B e detectou **11/11 mutation challenges**, todos restaurados. Entre os detectores: remoção de `FOR UPDATE`, `>`→`>=`, uso de data UTC, inversão estado/tempo, `AGUARDANDO -> FALTA`, check-in como no-op, permissão trocada, escopo por `agenda.gerenciar`, bypass de escopo próprio, omissão de histórico e emissão indevida de auditoria.
- **Validações registradas no PR #99:** `pnpm install --frozen-lockfile`, `pnpm run typecheck`, build da API, `verify:openapi-runtime` com 79 verificações, integração AGD-B isolada e `git diff --check`; o workflow CI associado ao head concluiu com `success`.
- **Persistência/escopo:** nenhuma migration, alteração de Prisma schema/golden, dependência, permissão ou ação de auditoria foi introduzida por AGD-B. AGD-C, AGD-D e AGD-E permanecem fora desta frente.
...
> **Nota da REV. 43 (15/09/2026).** `R2.2-04` foi **encerrado** nesta data (§7.2), depois de `D-AUD-09-A` (`docs/09` §14.9) resolver a ambiguidade de §13.3. Isso **não** encerra `P-BACK-01`, que segue **EM ANDAMENTO** pelas pendências próprias listadas abaixo — em especial **`L-06` ABERTA / BLOQUEADA**, os emissores de domínio ausentes e o visualizador AUD-004. **`L-06` não foi tocada**: regra, gatilhos e dependências (`P2.2-05`, módulo de prontuário, superfície real de leitura clínica) permanecem exatamente como §13.3 as fixou, e **nenhuma rota de leitura clínica é autorizada**.

> **Nota da REV. 42 (15/09/2026), preservada.** O encerramento de `R2.2-04` (§7.2) **não** encerra `P-BACK-01`: são itens distintos. `R2.2-04` é o risco de **conteúdo** do `contexto`, cuja política está agora integralmente definida e exercida; `P-BACK-01` é a **frente de auditoria como um todo**, que segue com pendências próprias — em especial **`L-06` ABERTA / BLOQUEADA**, os emissores de domínio ausentes e o visualizador AUD-004 (política fechada, implementação pendente). Nenhuma delas é alcançada por `D-AUD-09`.
...
- Consulta da trilha (AUD-004) — **INTEGRADA NA `main` em 17/09/2026** (PR [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63), merge `27012ff`; §7.6.7). *Histórico:* **política FECHADA, implementação PENDENTE** (`docs/09` §13.9): usará exclusivamente `auditoria.ler`, já homologada; **nenhuma permissão nova**; somente leitura, filtros fechados sobre colunas próprias, paginação obrigatória, sem busca livre e **sem resolver/expandir** `alvo_tipo`+`alvo_id`. O endpoint **não** foi implementado. **Atualização REV. 52 (17/09/2026): IMPLEMENTADA EM BRANCH / NÃO INTEGRADA** — §7.6. **REV. 53: INTEGRADA NA `main` em 17/09/2026** (PR [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63), merge `27012ff`) — §7.6.7.
...
```text
R2.2-04 — ENCERRADO (15/09/2026, REV. 43)
          Política de conteúdo de `evento_auditoria.contexto` integralmente definida e
          exercida pela aplicação; as sete condições de §7.2.0 verificadas uma a uma.
          A última delas — o vínculo normativo da cláusula final de docs/09 §13.3 — foi
          resolvida por D-AUD-09-A (docs/09 §14.9): leitura restrita homologada, L-06 NAO
          é pré-condição global deste risco.
          Encerramento de risco, não declaração de risco zero: a proteção é fail-closed e
          revisão de código permanece exigida a cada nova chave, ação ou emissor (docs/07
          §24.4 — residual "médio, depende de revisão de código").
          L-06 segue ABERTA / BLOQUEADA; P-BACK-01 segue EM ANDAMENTO.
```

*(Histórico preservado: a REV. 7 registrara "ENCERRADO NA 2.3C"; a REV. 8 **retirou** essa classificação por determinação da revisão independente de `F-2.3C-REV-01`, e o risco permaneceu **ABERTO / MITIGADO PARCIALMENTE** da REV. 8 à REV. 41 — estado preservado em §7.2.1. A REV. 42 **não** reabilitou a classificação da REV. 7 e **não** encerrou o risco: registrou que a condição cuja ausência a REV. 8 apontou — regra semântica homologada para datas e uuid — fora suprida por `D-AUD-09`, e que o encerramento esbarrava em **outra** condição, apurada apenas na inspeção final. A **REV. 43** encerra o risco por fundamento próprio e expresso — a decisão de autoridade `D-AUD-09-A` (`docs/09` §14.9) —, **não** por retorno à classificação da REV. 7, que segue retirada. O histórico permanece em §11.)*

#### 7.2.0 Encerramento — a última condição normativa foi RESOLVIDA por `D-AUD-09-A` (`docs/09` §14.9)

**Decisão que destravou o encerramento.** Em **15/09/2026**, **Bruno Menezes Noronha** homologou `D-AUD-09-A` (`docs/09` §14.9), resolvendo a ambiguidade da cláusula final de `docs/09` §13.3: prevalece a **leitura restrita** de “por esta via”. `L-06` impede que `R2.2-04` seja tido por encerrado **pela decisão relativa à auditoria de leitura clínica** — e só por ela; a cláusula **não** cria dependência global entre os dois itens. Os objetos são distintos: `R2.2-04` é **vazamento de conteúdo clínico por `evento_auditoria.contexto`** (`docs/07` §22.3/§24.4 — política de chave/valor), enquanto `L-06` é **quando o acesso de leitura a dados clínicos deve gerar auditoria** (AUD-002). Com isso a **condição 7** da tabela abaixo passa a **SIM** e as sete condições do risco estão satisfeitas.

**`R2.2-04` fica ENCERRADO** nesta data, na terminologia de encerramento já adotada neste documento (cf. `R-BL-02`, `F-2.3C-REV-01`, `P-2.3D-03`): a obrigação que definia o risco está **integralmente materializada e provada**, e não resta trabalho técnico nem condição normativa pendente associada a ele. **Isto não é declaração de risco zero.** A mitigação homologada (`docs/07` §24.4) tem residual declarado **“médio — depende de revisão de código”**, e esse residual permanece: toda nova ação, chave de whitelist ou emissor exige revisão explícita contra esta política, e a proteção é **fail-closed por construção**, não por ambiente.

**Provas objetivas do encerramento** (todas já existentes nesta branch, nenhuma criada pela REV. 43): whitelist **fail-closed por ação** e exaustiva (`WHITELIST_CONTEXTO` sob `satisfies`, 25 ações · 3 positivas · 22 vazias); **rejeição de chave fora da whitelist** (`CHAVE_FORA_DA_WHITELIST`) e de contexto não vazio em whitelist vazia (`CONTEXTO_NAO_VAZIO_EM_WHITELIST_VAZIA`), cobertas ação a ação; **regras semânticas das três whitelists positivas homologadas** (`ehDecimalMonetarioCanonico` por `F-2.3C-REV-01`; `ehDataCivilCanonica` e `ehUuidCanonico` por `D-AUD-09`) — **não resta chave homologada sob barreira apenas estrutural**; **validação de tipos/escalares** anterior à semântica (objeto, array, `NaN` e `±Infinity` → `VALOR_NAO_ESCALAR`), barreira estrutural contra serialização genérica de DTO/entidade dentro de `contexto`; **ausência de normalização** em qualquer predicado (valor fora da forma é rejeitado, nunca consertado, e o candidato não é mutado); **mensagens que citam ação e nomes de chave, nunca valores**; e os **testes correspondentes** — unitários e de integração pelo container `AppModule` real (§8.1, §7.5.3), além das provas ponta a ponta contra PostgreSQL real dos dois regimes de whitelist (§6-A.5 e §6-C.2/§6-C.4). **Nenhum caminho de escrita em `evento_auditoria` existe fora de `AuditWriter → AuditContextValidator`**, e nenhum emissor serializa entidade ou DTO em `contexto`: as únicas chaves que existem no código emissor real são as homologadas, com valor escalar validado.

**O que o encerramento de `R2.2-04` NÃO faz.** Não encerra `P-BACK-01` (§7.1). Não altera `L-06`, que permanece **ABERTA / BLOQUEADA POR DEPENDÊNCIA FUNCIONAL-TÉCNICA** com regra, gatilhos e dependências intactos. Não autoriza rota de leitura clínica alguma — a segunda metade da cláusula de §13.3 segue vinculante. Não cria `prontuario.acessado` nem qualquer outra ação, chave ou permissão. Não toca schema, migration, golden, protected-objects, OpenAPI, contrato HTTP ou dependência.

##### 7.2.0.1 Registro do estado anterior — por que o encerramento não fora declarado pela REV. 42 (preservado)

*Parágrafos preservados como registro do estado vigente entre a materialização de `D-AUD-09` (REV. 42) e a homologação de `D-AUD-09-A` (§14.9), ambas em 15/09/2026. A única alteração é esta nota introdutória — o texto abaixo não foi reescrito.*

A última consequência operacional de `docs/09` §13.3 (`PBACK-AUD-02`, homologada em 05/09/2026) determina:
...
`L-06` permanece **ABERTA / BLOQUEADA** — por dependência funcional-técnica real (`P2.2-05` não materializada, módulo de prontuário inexistente), não por omissão desta fatia. A expressão "por esta via" admite leitura restrita (a cláusula alcançaria apenas a própria §13) e leitura ampla (alcançaria qualquer via). A divergência foi registrada, **não resolvida silenciosamente**, e devolvida a Bruno Menezes Noronha em `docs/09` §14.8, com as duas leituras e suas consequências. Enquanto não houve resposta, prevaleceu a leitura conservadora (`TLF-BASE-V2` §15): **o risco permaneceu aberto**. — **Superado na mesma data: a resposta veio, e foi a leitura restrita (`D-AUD-09-A`, `docs/09` §14.9); ver o início de §7.2.0.**

O que isso **não** significava: não havia trabalho técnico pendente na auditoria de `contexto`. As seis condições substantivas do risco já estavam verificadas e verdes — tabela abaixo. O que faltava era exclusivamente a resolução normativa do vínculo com `L-06`, hoje **suprida**.

##### 7.2.0.2 Condições do risco, uma a uma — estado vivo

*(Fim do registro histórico de §7.2.0.1. Daí em diante, esta subseção descreve o estado **vigente**.)*

**Condições substantivas, uma a uma (origem da enumeração: `docs/09` §14.8; condição 7 atualizada pela REV. 43 após `docs/09` §14.9):**

| # | Condição | Estado | Evidência |
| --- | --- | --- | --- |
| 1 | Whitelist fail-closed implementada | **SIM** | `WHITELIST_CONTEXTO` exaustiva (`satisfies`) + `AuditContextValidator`; 25 ações · 3 positivas · 22 vazias |
| 2 | `T-AUD-CONTEXTO` verde | **SIM** | §8.1 — unitário + integração pelo container `AppModule` real |
| 3 | Chave fora da whitelist rejeitada | **SIM** | `CHAVE_FORA_DA_WHITELIST` / `CONTEXTO_NAO_VAZIO_EM_WHITELIST_VAZIA`, cobertos ação a ação |
| 4 | Valores das chaves **monetárias** validados | **SIM** | `F-2.3C-REV-01` — `ehDecimalMonetarioCanonico`, comportamento **inalterado** pela REV. 42 |
| 5 | Valores das **datas** validados | **SIM** | `D-AUD-09` — `ehDataCivilCanonica` (§7.5) |
| 6 | `registro_original_id` validado | **SIM** | `D-AUD-09` — `ehUuidCanonico` (§7.5) |
| 7 | Nenhuma outra condição normativa associada a `R2.2-04` pendente | **SIM** (REV. 43) | a busca global em `docs/07`..`docs/13` confirmou que a definição substantiva do risco (`docs/07` §22.3 e tabela de riscos §24.4 — "lista branca por ação, validada na aplicação; teste que rejeita chaves fora da lista") está inteiramente satisfeita. A única outra condição encontrada — a cláusula final de `docs/09` §13.3 — foi **resolvida em 15/09/2026 por `D-AUD-09-A`** (`docs/09` §14.9, leitura restrita homologada): `L-06` **não** é pré-condição global de `R2.2-04`. *(Estado anterior preservado: até a homologação de §14.9 esta condição constava como **NÃO** — §7.2.0.1.)* |

**Por que a ausência de emissor não é, ela própria, fundamento para manter o risco aberto.** `R2.2-04` é risco de **política de conteúdo** do `contexto` — vazamento clínico por chave ou valor indevido —, não de cobertura funcional. A enforcement é fail-closed e está pronta **antes** da existência dos emissores de `cobranca.data_referencia_recalculada` e `retificacao_clinica.efetivada_terceiro`; nenhum caminho de escrita em `evento_auditoria` existe fora de `AuditWriter → AuditContextValidator`. Um emissor futuro que tentasse gravar valor fora da forma homologada seria rejeitado com rollback integral — exatamente como o emissor real de `cobranca.desconto_aplicado` já demonstra ponta a ponta contra PostgreSQL (§6-C.2/§6-C.4).

**O que as REV. 42 e 43 NÃO alteram.** `P-BACK-01` permanece **EM ANDAMENTO** (§7.1) e **`L-06` permanece ABERTA / BLOQUEADA** (`docs/09` §13.3), com suas dependências funcionais próprias (`P2.2-05`, módulo de prontuário, superfície real de leitura clínica) — esta fatia não tocou nenhuma delas. Os emissores ausentes das demais ações homologadas continuam registrados em §9. *(Texto original da REV. 42 preservado: “**Caminho para o encerramento de `R2.2-04`:** ou a resposta de Bruno à questão de `docs/09` §14.8 adota a leitura restrita de §13.3, ou `L-06` é decidida — nenhum dos dois exige implementação adicional nesta frente.” — **ocorreu a primeira hipótese**, em 15/09/2026, por `D-AUD-09-A`, e de fato nenhuma implementação adicional decorreu dela.)*

#### 7.2.1 Estado anterior (preservado — vigente da REV. 8 à REV. 41)

*Os dois parágrafos abaixo são o registro do estado anterior, preservados na íntegra — a única alteração é a concordância verbal para o passado ("estava", "impedia"), porque descrevem um estado que deixou de vigorar. O segundo descreve a condição que `D-AUD-09` supriu; o caminho ali exigido — decidir formalmente a regra, sem reabrir `D-AUD-07` e sem inventar formatos por implicação — foi o efetivamente seguido.*

O que estava **comprovadamente protegido**: os dois regimes de whitelist têm emissor real seguro provado ponta a ponta contra PostgreSQL — whitelist **VAZIA** (2.3B — `profissional.situacao.alterada`, §6-A.5) e whitelist **POSITIVA** (2.3C — `cobranca.desconto_aplicado`, §6-C.2/§6-C.4), esta última agora também com **validação SEMÂNTICA fail-closed** das chaves monetárias na camada de política (F-2.3C-REV-01): valor semanticamente inválido em chave homologada → rejeição + rollback integral, mesmo se enviado diretamente ao `AuditWriter` por um emissor incorreto. Nenhum caminho de escrita em `evento_auditoria` existe fora de `AuditWriter → AuditContextValidator`.

O que **impedia o encerramento global**: as demais ações de whitelist positiva (`cobranca.data_referencia_recalculada` — par de datas; `retificacao_clinica.efetivada_terceiro` — `registro_original_id`) **não tinham validação semântica definida/implementada** (sem regra de formato homologada) e seus emissores não existiam. O risco genérico de vazamento via `contexto` permanecia portanto **ABERTO / MITIGADO PARCIALMENTE**. **Pendência registrada (§9)**: definir/implementar a validação semântica dessas chaves quando seus emissores forem implementados ou sua regra for formalmente decidida — sem reabrir D-AUD-07 e sem inventar formatos de data/uuid por implicação.

**Superado em 15/09/2026** por `D-AUD-09` (`docs/09` §14) e sua materialização (§7.5): a regra foi **formalmente decidida antes** de ser implementada, `D-AUD-07` não foi reaberta e nenhum formato foi inventado por implicação.
...
### 7.5 `R2.2-04` / `D-AUD-09` — validação semântica das whitelists positivas remanescentes (implementação, 15/09/2026)

```text
D-AUD-09 — HOMOLOGADA (docs/09 §14) / MATERIALIZADA / INTEGRADA NA main em 16/09/2026
           (PR #49, merge commit c2d57b8 — registro pós-integração em §7.5.9, REV. 44)
           Estado anterior preservado (REV. 42/43): MATERIALIZADA E MEDIDA EM BRANCH PRÓPRIA /
           NÃO INTEGRADA — publicação NÃO AUTORIZADA naquela rodada
```

Branch `agent/p-back-01-r2-2-04-semantica-contexto`, a partir de `main` = `origin/main` = `3d8cbfc` (0 à frente / 0 atrás no início da fatia). Fatia **exclusivamente de política de auditoria**: nenhum módulo de domínio, nenhum emissor, nenhum endpoint.

> **`R2.2-04` não é encerrado por esta subseção** — ver §7.2.0. A condição semântica que o mantinha aberto está suprida e provada aqui; o encerramento formal do risco decorre de **`D-AUD-09-A`** (`docs/09` §14.9, 15/09/2026) e está registrado em §7.2/§7.2.0 pela REV. 43. Nada nesta subseção foi alterado por aquela decisão: o código, os testes e as medições abaixo são exatamente os da REV. 42. *(Texto original preservado: o encerramento esbarrava no vínculo normativo de `docs/09` §13.3 com `L-06`, matéria devolvida a Bruno em `docs/09` §14.8 e não resolvida naquela rodada.)*

#### 7.5.1 O que foi implementado

| Arquivo | Situação | Responsabilidade |
| --- | --- | --- |
| `apps/api/src/audit/audit.catalog.ts` | **ALTERADO** | helpers `ehDataCivilCanonica` e `ehUuidCanonico`; três novas entradas em `SEMANTICA_CONTEXTO`; comentários citam `D-AUD-09` |
| `apps/api/src/audit/audit-context.validator.ts` | **ALTERADO** | mensagem de `VALOR_SEMANTICAMENTE_INVALIDO` tornada genérica; documentação do item 6 reescrita em termos de `SEMANTICA_CONTEXTO`. **Nenhuma mudança de lógica** |
| `apps/api/test/audit-context.validator.spec.ts` | **ALTERADO** | matrizes de data e uuid; precedência estrutural × semântica; ausência de normalização; não obrigatoriedade; mensagem genérica. Teste do estado anterior **substituído** |
| `apps/api/test/audit.module.integration.spec.ts` | **ALTERADO** (+5 provas) | as mesmas regras exercidas pelo provider resolvido do `AppModule` real |
| `docs/09` §14 | **NOVO** | registro normativo de `D-AUD-09` |

**Intocados:** `packages/**` (schema, migrations, golden, protected-objects), `package.json`/`pnpm-lock.yaml`, OpenAPI, controllers e services de domínio, `audit-writer.ts`, `audit.types.ts`, `audit.module.ts`, `authz/**`, configuração de CI.

#### 7.5.2 `ehDataCivilCanonica` — data civil, não instante

Predicado puro, sem dependência externa e **sem `Date`**: (1) exige `string`; (2) casa `^(\d{4})-(\d{2})-(\d{2})$`; (3) exige ano > 0 — `0000` é rejeitado; (4) exige mês em `1..12`; (5) compara o dia com o número **real** de dias do mês; (6) aplica a regra gregoriana de bissexto `ano % 400 === 0 || (ano % 4 === 0 && ano % 100 !== 0)`; (7) **não** converte, **não** normaliza, **não** interpreta fuso e **não** apara espaços.

Consequências medidas: `2000-02-29` aceita · `1900-02-29` rejeitada · `2024-02-29` aceita · `2023-02-29` rejeitada · `2026-04-31`, `2026-02-30`, `2026-13-01`, `2026-00-01` rejeitadas · `2026-9-15`, `15/09/2026`, `2026-09-15T00:00:00Z`, `" 2026-09-15 "` (com espaços) e `""` rejeitadas · `number`/`boolean`/`null` rejeitados.

A escolha de **não** usar `Date` é deliberada e não é preferência de estilo: `new Date("2026-02-30")` normaliza silenciosamente em vez de recusar, e o construtor introduz interpretação de fuso — os dois comportamentos que `D-AUD-09` proíbe. `cobranca.data_referencia` é `date` (`docs/07` §20), data civil no fuso da clínica, não instante.

#### 7.5.3 `ehUuidCanonico` — representação textual

Predicado puro: `string` que case `^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$`. Rejeita maiúsculas, caixa mista, ausência de hífens, `{uuid}`, `urn:uuid:`, espaços, comprimento incorreto, caractere não hexadecimal, string vazia e não-strings. **Sem** restrição de versão ou variante, **sem** consulta ao banco, **sem** verificação da existência da linha, **sem** normalização de caixa — uuid maiúsculo é **rejeitado**, jamais convertido.

#### 7.5.4 Separação catálogo × validator preservada

O **catálogo declara** as regras semânticas (`SEMANTICA_CONTEXTO`); o **validator executa** a política. Nenhuma lógica concreta de data ou uuid foi movida para `audit-context.validator.ts` — ele continua iterando predicados opacos. Efeito colateral positivo: `F-2.3C-REV-03` (§9), que registrava que a mensagem de `VALOR_SEMANTICAMENTE_INVALIDO` citava "chave monetária" e deveria ser desacoplada ao surgir a primeira regra não monetária, fica **atendido** — a mensagem passou a ser genérica, preservando o motivo tipado e a regra de nunca citar valores.

#### 7.5.5 Ordem das barreiras — inalterada

`ação → objeto → whitelist vazia → chave fora da whitelist → escalar JSON → semântica`. `D-AUD-09` **não** alterou a precedência: objeto, array, `NaN` e `±Infinity` continuam recusados como `VALOR_NAO_ESCALAR` **antes** da etapa semântica, e `autor_original_usuario_id` continua recusada como `CHAVE_FORA_DA_WHITELIST` mesmo acompanhada de uuid canônico. A ordem é comportamento observável e está travada por teste.

#### 7.5.6 Obrigatoriedade — inalterada

Contexto ausente, contexto `{}` e subconjunto das chaves permitidas continuam aceitos para as três ações de whitelist positiva. `D-AUD-09` restringe **apenas o valor da chave presente**.

#### 7.5.7 Provas medidas (host Windows, 15/09/2026)

| Comando | Resultado | Números |
| --- | --- | --- |
| `jest --runTestsByPath test/audit-context.validator.spec.ts test/audit.module.integration.spec.ts` | **verde** | 2 suítes · **265** testes |
| `pnpm --filter @techlab-fisio/api run test` | **verde** | 34 suítes · **835** testes |
| `pnpm run verify:api-integration` (PostgreSQL 18 real, banco descartável) | **verde** | 12 suítes · **424** passados · 2 skipped (426) |
| `pnpm run typecheck` | **verde** | `apps/api` (src + test), `apps/web`, `packages/database` |
| `pnpm --filter @techlab-fisio/api run build` | **verde** | `tsc -p tsconfig.build.json` |
| `pnpm run lint:migrations` | **verde** | 10 migrations varridas · 37 objetos protegidos · nenhuma remoção |

**Por que não há teste transacional sintético nesta fatia.** As duas ações alcançadas por `D-AUD-09` **não possuem emissor de domínio real**. Criar um emissor falso e um fluxo de banco artificial apenas para exibir "válido → commit / inválido → rollback" produziria evidência desproporcional ao custo e sem poder de mutação sobre código de produção — a fronteira única `AuditWriter → AuditContextValidator` já tem essa propriedade provada contra PostgreSQL real pelo emissor de `cobranca.desconto_aplicado` (§6-C.4). A suíte de integração PostgreSQL foi executada integralmente como **regressão global**. Quando os emissores reais existirem, a atomicidade deles será provada nas respectivas fatias funcionais.
...
#### 7.5.9 Registro pós-integração (16/09/2026)

Registro **exclusivamente factual**, no precedente `MEDIR → REGISTRAR` de §7.4.8 e da REV. 41. Nenhuma medição de §7.5.1..§7.5.8 nem de §7.2/§7.2.0 foi reescrita.

| Item | Fato medido (`git`/`gh`, 16/09/2026) |
| --- | --- |
| PR | [#49](https://github.com/BrunoMNoronha/techlab-fisio/pull/49) — "feat(audit): validar semantica das whitelists positivas e encerrar R2.2-04"; branch `agent/p-back-01-r2-2-04-semantica-contexto` → `main`; estado `MERGED` |
| Commits da fatia | `8bf9057963177e704b65a11117144668095e8e35` — materializa `D-AUD-09`, registra `D-AUD-09-A` e encerra `R2.2-04` (REV. 42 + REV. 43); `02f01fc17a765703941995bb63a34a2dcacf9b6d` — sincroniza §7.5.8 com o encerramento (1 linha, exclusivamente documental). `02f01fc` tem `8bf9057` como pai: nenhuma reescrita |
| CI | HEAD da PR (`02f01fc`) **verde** — run [35076322751](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35076322751), `success` |
| Merge | merge commit **`c2d57b808eafffbffbeee765ccc64d992c21fb5b`** (pais: `3d8cbfc`, `02f01fc`), em 16/09/2026 09:08:42Z; método merge commit, sem squash, sem rebase, sem force-push; árvore do merge idêntica à de `02f01fc` |
| Arquivos integrados | **6** — `apps/api/src/audit/audit-context.validator.ts`, `apps/api/src/audit/audit.catalog.ts`, `apps/api/test/audit-context.validator.spec.ts`, `apps/api/test/audit.module.integration.spec.ts`, `docs/09-pacote-decisao-p-e14-01.md` e `docs/10-backend-implementacao.md` |
| Persistência | `packages/**` não tocado pela PR — zero drift; 10 migrations; golden inalterado |
| CI sobre a `main` integrada | **Não medida por run própria**: o workflow da `main` é acionado manualmente (`workflow_dispatch`); a evidência é a do HEAD da PR, cuja árvore é idêntica à do merge, e a validação local abaixo |
| Validação local pós-merge | `pnpm verify:api-integration` sobre `main` = `origin/main` = `c2d57b8`, árvore limpa: **exit 0** — `Test Suites: 12 passed, 12 total` · `Tests: 2 skipped, 424 passed, 426 total` · *"PROVA CONCLUÍDA: integração da API com PostgreSQL real verde."* |
| Estados vivos | **inalterados pela integração**: `R2.2-04` ENCERRADO (§7.2 — REV. 43; não é declaração de risco zero, residual de `docs/07` §24.4 preservado); `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; nenhuma rota de leitura clínica autorizada; `prontuario.acessado` não criada |

### 7.6 `PBACK-AUD-08` / AUD-004 — consulta da trilha de auditoria (implementação, 17/09/2026)

> **Estado:** **`PBACK-AUD-08` / AUD-004 — IMPLEMENTADA EM BRANCH / NÃO INTEGRADA.** Branch `agent/p-back-01-aud-004-consulta-trilha-r2`, base `origin/main` = `5ff445263b4230e8a38a5cbcc166e77a2c209c1f`, atualizada por fast-forward para `c9b33f21c03ef5ef4b16ccca2ac141c2a5d021fb` depois para `21e2fa06c7d621fa4b7e0731b17e03781cf51920` e, por merge de `origin/main`, integrada com `bc6a763` (`P-2.3D-10`) `9b403a8` (`CFG-001A`) e `4225b63` (`P-2.3D-10`, revisão). Sem commit, push, PR ou merge nesta rodada. **`P-BACK-01` NÃO é encerrada** — seguem `L-06` (ABERTA / BLOQUEADA), a materialização técnica de `L-08` e os emissores de domínio ausentes.

#### 7.6.1 Política aplicada — normativa, sem reinterpretação (`docs/09` §13.9)

| Regra de §13.9 | Materialização |
| --- | --- |
| Permissão exclusiva `auditoria.ler`; nenhuma nova | `@RequerPermissao("auditoria.ler")` — `SessaoAutenticadaGuard` + `PermissoesGuard` vigentes; nenhuma checagem por papel; catálogo de permissões intocado |
| Somente leitura (AUD-005) | Só `GET`; `POST`/`PUT`/`PATCH`/`DELETE` no caminho → `404` |
| Filtros fechados sobre colunas próprias | `ocorridoDe`/`ocorridoAte` (`ocorrido_em`), `atorUsuarioId`, `acao`, `alvoTipo`, `alvoId`, `correlacaoId`, `resultado` — igualdade exata (período: `>=`/`<=`); qualquer outro parâmetro → `400` |
| `acao` restrita ao catálogo | `ehAcaoAuditoria` do catálogo **vigente** (25 ações — `PBACK-AUD-09` acrescentou `autorizacao.negada` depois de §13.9, que citava 24) |
| Paginação obrigatória com limite do servidor | limite padrão 20, máximo 50; acima → `400` |
| Vedado: busca livre, filtro por `contexto`/`justificativa`, resolver/expandir alvo, join pessoal/clínico | `select` explícito das 10 colunas próprias, sem `include`, sem relação `ator`, sem segunda consulta; nenhum operador `contains`/`startsWith`/`search` |
| `auditoria.consultada` não criada | A consulta não depende do `AuditWriter`; nenhum evento emitido em `200`/`400`/`401`/`403` |

**Campos devolvidos.** `id`, `ocorridoEm`, `atorUsuarioId`, `acao`, `alvoTipo`, `alvoId`, `resultado`, `correlacaoId`, `contexto`. `criado_em` (metadado de persistência) não é exposto.

**Decisões de Bruno Menezes Noronha (17/09/2026), após a revisão independente da fatia:**

- **`justificativa` é OMITIDA da resposta.** É texto livre do operador e, sem controle de conteúdo, poderia levar dado pessoal ou clínico ao visualizador — que §13.9 impede de virar canal indireto de acesso. A coluna não é lida pela consulta (`select` sem ela) e continua não filtrável. `contexto` permanece exposto: seu conteúdo já é restrito na escrita às whitelists (`D-AUD-07`/`08`, `D-AUD-09`).
- **Contrato HTTP de §7.6.2 ACEITO** como escolha técnica local: path `GET /auditoria/eventos`, limite 20/50 com rejeição (sem clamp) e cursor keyset.

**Errata da primeira redação desta seção (mesma rodada, não integrada).** O texto anterior afirmava que “nenhum emissor atual grava `justificativa`”. **Era falso:** o bootstrap do Administrador (`apps/api/src/provisionamento/bootstrap-administrador.service.ts`, `usuario.papeis.alterados`, `D-2.3D-10`) grava justificativa operacional obrigatória em texto livre. A revisão independente apontou o erro; ele sustentava a leitura de que exibir o campo era inócuo e motivou a devolução da questão a Bruno.

#### 7.6.2 Escolhas técnicas locais (reversíveis, sem decisão de produto)

Nenhuma fonte homologada fixa o contrato HTTP. Adotado, por coerência com os precedentes (`GET /auth/sessao`, filtro de erro fechado, `Cache-Control: no-store`):

- `GET /auditoria/eventos`, tag OpenAPI `Auditoria`; resposta `{ itens, proximoCursor }`; erros `{ "erro": "<CODIGO>" }` (`REQUISICAO_INVALIDA`, `SESSAO_INVALIDA`, `ACESSO_NEGADO`, `FALHA_INTERNA`) via `FiltroErroAutenticacao` (uma entrada de rota acrescentada);
- sem `ProtecaoCsrfGuard`, sem `X-TLF-Requisicao`, sem corpo; `Cache-Control: no-store`;
- paginação por **cursor keyset** opaco (base64url de `{o, i}`) sobre `(ocorrido_em DESC, id DESC)` — o `id` desempata instantes iguais; offset descartado por perder/duplicar linhas sob inserção concorrente numa trilha append-only;
- instantes ISO-8601 com fuso explícito (`Z` ou `±HH:MM`), até milissegundo; data impossível (ex.: 30/02) rejeitada; `ocorridoDe > ocorridoAte` → `400`;
- UUIDs pela forma já usada na API (`ehUuidValido`); `alvoTipo` pela forma de nome físico de tabela (`^[a-z][a-z0-9_]{0,62}$`), sem catálogo novo; nenhum valor é aparado ou convertido de caixa; parâmetro repetido ou aninhado → `400`;
- módulo próprio `AuditoriaConsultaModule` (em `apps/api/src/audit/`): o `AuthzModule` já importa o `AuditModule`, e registrar a rota protegida dentro dele criaria importação circular. `AuditModule` inalterado.

**Premissa de precisão do cursor:** `ocorrido_em` é `timestamptz(6)`, mas o `AuditWriter` grava a partir de `Date` (milissegundos); o cursor, também em milissegundos, é exato para toda linha escrita pela aplicação.

#### 7.6.3 Índices — inspeção, nenhuma migration

Valores de teste medidos sobre a árvore final, após merge de `origin/main` = `4225b63`.
...
#### 7.6.4 Provas medidas (host Windows, 17/09/2026)

| Verificação | Resultado |
| --- | --- |
| `pnpm run typecheck` | exit 0 |
| `pnpm --filter @techlab-fisio/api run test` | 36 suítes · 949 passed (com `dist/` compilado; sem `dist/`, 3 testes de `provisionamento.fronteira.spec.ts` são pulados por desenho) |
| `pnpm --filter @techlab-fisio/api run build` | exit 0 |
| `pnpm run verify:api-integration` (PostgreSQL 18 real, descartável) | exit 0 — 15 suítes · 489 passed · 2 skipped; suíte AUD-004 isolada: 34 passed (executada com prefixo de container próprio, para não colidir com execução concorrente de outra frente) |
| `pnpm --filter @techlab-fisio/api run verify:openapi-runtime` | 28/28 OK |
| `pnpm run lint:migrations` | exit 0 — 10 migrations, 37 objetos protegidos |
| `pnpm run schema:verify` | exit 0 — dump idêntico ao golden (60169 bytes); `prisma migrate diff` exit 0 |
| `git diff --check` | limpo |

**Mutation challenges** (cada um revertido e conferido por SHA-256 contra o original):

| Mutante | Morto por |
| --- | --- |
| M1 — remover `@RequerPermissao("auditoria.ler")` | unit (metadata da rota) + 6 de integração (401/403/200) |
| M2 — remover limite máximo | unit `limite 51 é 400` + integração `51 → 400` |
| M3 — aceitar filtro `contexto` | 3 unit (conjunto fechado) + integração `?contexto=x → 400` |
| M4a — join `ator { nome, email }` | 2 unit (`select`/chaves) + 2 integração (sem dado pessoal; chaves exatas) |
| M4b — resolver `alvo_id` em `usuario` | 3 unit + 2 integração |
| M5 — emitir `auditoria.consultada` | integração (contagem de eventos inalterada) + unit |
| M6 — remover desempate por `id` | unit (`orderBy`) + integração (paginação sob empate: perda/duplicidade) |
| M7 — voltar a devolver `justificativa` | 3 unit (`select`, colunas, texto ausente) + 3 integração (evento com justificativa preenchida; chaves exatas) |

#### 7.6.5 Arquivos

Novos: `apps/api/src/audit/audit-query.dto.ts`, `audit-query.service.ts`, `audit.controller.ts`, `auditoria-consulta.module.ts`; `apps/api/test/audit-query.spec.ts`; `apps/api/test/integration/auditoria-consulta.integration.spec.ts`. Alterados (fiação mínima): `apps/api/src/app.module.ts`, `apps/api/src/auth/erro-autenticacao.filter.ts` (uma entrada de rota), `apps/api/test/openapi.spec.ts`, `apps/api/test/auth.module.integration.spec.ts` e `apps/api/scripts/verify-openapi-runtime.mjs` (igualdade exata de rotas), `apps/api/test/erro-autenticacao.filter.spec.ts`. **Não tocados:** `packages/**`, `apps/web/**`, sessões, login, logout, recuperação de senha, usuários/AUT-005, catálogo e whitelist de auditoria.

#### 7.6.6 Revisão independente (17/09/2026)

Revisão adversarial somente leitura: **APROVADA COM RESSALVAS**, sem achado bloqueante. Tratamento:

| Achado | Tratamento |
| --- | --- |
| Afirmação falsa sobre emissores de `justificativa` | corrigida (errata em §7.6.1) |
| Exibição de `justificativa`/`contexto` é decisão de privacidade | devolvida a Bruno: `justificativa` omitida, `contexto` mantido |
| Contrato HTTP vira público após integração | aceito por Bruno como escolha local |
| Base desatualizada | fast-forward para `21e2fa0`; bateria reexecutada |
| Cursor depende de `ocorrido_em` em milissegundos | premissa mantida e documentada (§7.6.2); todos os escritores atuais usam `AuditWriter` com `Date` |
| Modo do query parser não travado por teste; `HEAD` fora do contrato fechado de erro; ano `0000` rejeitado | notas sem ação — comportamento atual fail-closed |

**Pendente:** necessidade de índice para consultas só por período/ação/resultado — não medida sob volume.

#### 7.6.7 Registro pós-integração (17/09/2026)

Registro **exclusivamente factual**, no precedente `MEDIR → REGISTRAR` de §7.5.9. Nenhuma medição de §7.6.1..§7.6.6 foi reescrita; o estado "IMPLEMENTADA EM BRANCH / NÃO INTEGRADA" daquelas subseções fica como registro histórico da rodada de implementação.

| Item | Fato medido (`git`/`gh`, 17/09/2026) |
| --- | --- |
| PR | [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63) — "feat(audit): consultar trilha de auditoria (AUD-004 / PBACK-AUD-08)"; branch `agent/p-back-01-aud-004-consulta-trilha-r2` → `main`; estado `MERGED`, por `BrunoMNoronha` |
| Commits da fatia | `23583e6` — implementação e testes; `93768d3` — registro em `docs/10`. Merges de `origin/main` na branch, com conflitos resolvidos mantendo as duas fatias: `a5c43f2` (`P-2.3D-10`), `1b97310` (`CFG-001A`) e `6111662` (revisão de `P-2.3D-10`). Sem rebase e sem force-push |
| CI | HEAD da PR (`6111662`) **verde** — runs [35191204044](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35191204044) (`push`) e [35191207118](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/35191207118) (`pull_request`), ambas `success` |
| Merge | merge commit **`27012ff82f58aa4bca9d6f5f2b264662359b839d`** (pais: `4225b63`, `6111662`), em 17/09/2026 06:55:47Z; método merge commit, com `--match-head-commit` no HEAD validado; **árvore do merge idêntica à de `6111662`** (`117e480a`) |
| Arquivos integrados | **13** — `apps/api/src/audit/` (`audit-query.dto.ts`, `audit-query.service.ts`, `audit.controller.ts`, `auditoria-consulta.module.ts`), `apps/api/src/app.module.ts`, `apps/api/src/auth/erro-autenticacao.filter.ts`, `apps/api/scripts/verify-openapi-runtime.mjs`, `apps/api/test/audit-query.spec.ts`, `apps/api/test/auth.module.integration.spec.ts`, `apps/api/test/erro-autenticacao.filter.spec.ts`, `apps/api/test/integration/auditoria-consulta.integration.spec.ts`, `apps/api/test/openapi.spec.ts` e `docs/10-backend-implementacao.md` |
| Persistência | `packages/**` não tocado pela PR — zero drift; nenhuma migration, índice ou alteração de schema |
| Validação local | Sobre o HEAD `6111662`, árvore limpa (idêntica à do merge): integração completa da API contra PostgreSQL real **exit 0** — `Test Suites: 16 passed, 16 total` · `Tests: 2 skipped, 522 passed, 524 total`; zero bancos, containers e volumes remanescentes. Executada com prefixo de container próprio, para não colidir com execuções concorrentes de outras frentes. Na mesma árvore: `typecheck` 0, `build` 0, testes da API 37 suítes · 1034 passed, `verify:openapi-runtime` 30/30 |
| Estados vivos | AUD-004 **INTEGRADA**; `justificativa` omitida e contrato local aceitos (decisões de 17/09/2026, §7.6.1); **`P-BACK-01` EM ANDAMENTO** — seguem `L-06` (ABERTA / BLOQUEADA), a materialização técnica de `L-08` e os emissores de domínio ausentes; índice para consultas só por período/ação/resultado **não medido sob volume** (§7.6.3) |
...
```text
T-AUD-CONTEXTO — CONCLUÍDO (EXECUTADO / PASSED — unitário + integração; emissor real com contexto NÃO vazio desde a 2.3C; validação SEMÂNTICA de chave monetária desde F-2.3C-REV-01; validação SEMÂNTICA das TRÊS whitelists positivas desde D-AUD-09 — REV. 42, §7.5)
```
...
| --- | --- |
| `P-BACK-01` | **EM ANDAMENTO** (§7.1) — fluxos reais de whitelist vazia (2.3B) e positiva (2.3C) entregues; **lacunas normativas decididas em 05/09/2026** por `P-BACK-01-D2` (`docs/09` §13); **`L-07` decidida (`PBACK-AUD-09`), implementada (§7.4) e integrada na `main` em 05/09/2026 (PR #23, merge `e1459f6` — §7.4.8) — primeiro emissor de `NEGADO`**. **Visualizador de auditoria (AUD-004) INTEGRADO na `main` em 17/09/2026** (PR [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63), merge `27012ff`; §7.6, §7.6.7). **Resta**: `L-06` (ABERTA / BLOQUEADA), a materialização técnica de `L-08` e os emissores das 17 ações que ainda não têm módulo de domínio. A pendência de exposição HTTP de `profissional.situacao.alterada` com `profissionais.gerenciar` é **fatia de domínio**, não lacuna normativa de auditoria. **REV. 42 (15/09/2026):** `D-AUD-09` materializada (§7.5); `R2.2-04` permanecia ABERTO por vínculo normativo com `L-06` (§7.2.0.1). **REV. 43 (15/09/2026):** `D-AUD-09-A` (`docs/09` §14.9) afastou esse vínculo e **`R2.2-04` foi ENCERRADO** (§7.2) — `P-BACK-01` **permanece EM ANDAMENTO**, porque o encerramento daquele risco não alcança a frente: seguiam pendentes, à época, **`L-06` (ABERTA / BLOQUEADA)**, a materialização técnica de `L-08`, os emissores de domínio ausentes e a implementação do visualizador AUD-004 — este último integrado depois, na REV. 53 |
| `R2.2-04` | **ENCERRADO em 15/09/2026** (§7.2, §7.2.0 — REV. 43). As **três** whitelists positivas têm validação semântica fail-closed (`D-AUD-07` + **`D-AUD-09`**, `docs/09` §14; §7.5), a whitelist é fail-closed por ação, chave fora da lista é rejeitada, escalares são validados antes da semântica, não há normalização, não há serialização genérica de entidade/DTO em `contexto` e os testes correspondentes (unitários, container real e ponta a ponta contra PostgreSQL) estão verdes — **nenhum trabalho técnico pendente**. A última condição normativa — a cláusula final de `docs/09` §13.3 — foi resolvida por **`D-AUD-09-A`** (`docs/09` §14.9): leitura restrita homologada, `L-06` **não** é pré-condição global deste risco. **Não é declaração de risco zero** — o residual de `docs/07` §24.4 (“médio, depende de revisão de código”) segue exigindo revisão explícita a cada nova ação, chave ou emissor. Estados anteriores preservados em §7.2.0.1 (REV. 42) e §7.2.1 (REV. 8–REV. 41) |
...
| `F-2.3C-REV-03` | **ATENDIDO em 15/09/2026** (§7.5.4) — a condição que o registro previa ocorreu: `D-AUD-09` acrescentou a primeira regra semântica **não** monetária, e o detalhe da mensagem de `VALOR_SEMANTICAMENTE_INVALIDO` deixou de citar "chave monetária", tornando-se genérico. O motivo tipado e a regra de nunca citar valores foram preservados. *(Texto original preservado: INFORMATIVO — NÃO corrigido por decisão na revisão de `b386c03`.)* |
| Validação semântica das demais whitelists positivas | **RESOLVIDA em 15/09/2026** — a regra foi **formalmente decidida** por `D-AUD-09` (`docs/09` §14) e materializada em §7.5 (`ehDataCivilCanonica`, `ehUuidCanonico`), pelo caminho que esta própria pendência previa: por decisão expressa, **sem reabrir `D-AUD-07`** e sem inventar formato por implicação |
...
| **AUT-002 — revogação administrativa de sessão de terceiro (`sessoes.revogar_terceiro` / `P-2.3D-07`)** | **CONCLUÍDA / INTEGRADA NA `main` em 14/09/2026 (PR [#46](https://github.com/BrunoMNoronha/techlab-fisio/pull/46), merge commit `b6427e1ab999aae40f41e7745fe374f32b719836`)** (§6-R). Decisão normativa `D-2.3D-19` (`docs/12` §5.19) e critérios `A-01`..`A-22` (`docs/12` §10.4) integralmente materializados no `SessoesModule`: endpoint `DELETE /auth/sessoes/:sessaoId`, `@RequerPermissao("sessoes.revogar_terceiro")`, ator extraído exclusivamente da sessão autenticada (`ContextoAutenticado.usuarioId`), resposta anti-oráculo uniforme `204 No Content` sem corpo e sem cookies, mutação atômica em transação com auditoria via `AuditWriter` (`usuario.sessao.revogacao`), inutilização imediata da sessão de terceiro revogada, autorrevogação no-op segura que mantém a sessão ativa e não emite auditoria. 4 mutation challenges `M1`..`M4` provados e revertidos. Zero drift de persistência, zero dependência nova. Encerra o risco de volume V0 de `PBACK-AUD-09` quanto à resposta administrativa. |
| **`P-2.3D-08` / `D-2.3D-20` — Consulta da sessão autenticada atual e prova E2E real same-origin** | **CONCLUÍDA / INTEGRADA NA `main` em 15/09/2026 (PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47), merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`)** (§6-S). Decisão normativa `D-2.3D-20` (`docs/12` §5.20) materializada em `apps/api`: endpoint `GET /auth/sessao` sob `SessaoAutenticadaGuard`, safe GET, sem CSRF, sem `X-TLF-Requisicao`, sem body, sem synchronizer token, `Cache-Control: no-store`, resposta tipada `{ usuarioId, sessaoId }`. Proteção CSRF (`ProtecaoCsrfGuard`) movida para `@Post("login")` e `@Post("logout")`. Proibição expressa de evento de auditoria (`usuario.sessao.consultada` proibida). Prova E2E automatizada e versionada (`apps/web/scripts/verify-web-api-e2e.mjs`) executando PostgreSQL 18 real, NestJS compilado real, Next.js real com proxy Route Handler same-origin, `ProtecaoCsrfGuard` real e cookie de sessão real; integrada à CI e script raiz `verify:web-api-e2e`. 4 mutation challenges `M1`..`M4` provados. Zero drift de persistência, zero dependência nova. |
...
| Consulta da trilha de auditoria (AUD-004) | **INTEGRADA NA `main` em 17/09/2026** (PR [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63), merge `27012ff`; §7.6, §7.6.7) — `GET /auditoria/eventos` sob `auditoria.ler`; `justificativa` omitida da resposta. **Nada pendente nesta linha.** *Histórico:* **POLÍTICA FECHADA / IMPLEMENTAÇÃO PENDENTE** até a REV. 51 — `docs/09` §13.9 (`PBACK-AUD-08`): usará exclusivamente `auditoria.ler` (**nenhuma permissão nova**); somente leitura; filtros fechados sobre colunas próprias; paginação obrigatória; sem busca livre, sem filtro por `contexto`/`justificativa` e **sem resolver/expandir** `alvo_tipo`+`alvo_id`. `auditoria.consultada` não criada. Endpoint **não** implementado — unidade de trabalho própria. **REV. 52: IMPLEMENTADA EM BRANCH / NÃO INTEGRADA** (§7.6). **REV. 53: INTEGRADA NA `main` em 17/09/2026** (PR [#63](https://github.com/BrunoMNoronha/techlab-fisio/pull/63), merge `27012ff`; §7.6.7) |
...
| ~~`P-2.3D-04`~~ (reavaliação de CSRF) | **ENCERRADA em 15/09/2026** — Encerrada em 15/09/2026 após a integração do PR #47 (merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`) que versionou `apps/web/scripts/verify-web-api-e2e.mjs` e o passo correspondente na CI, executando simultaneamente PostgreSQL real em container, NestJS real compilado, Next.js real compilado com Route Handler de proxy same-origin (`/api/*`), `ProtecaoCsrfGuard` real e cookie real de sessão, provando que a baseline de CSRF de `D-2.3D-07` (`SameSite=Strict`, cabeçalho obrigatório `X-TLF-Requisicao` nas mutações, Fetch Metadata, validação de `Origin`, ausência de CORS) protege a fronteira de ponta a ponta sem necessidade de synchronizer token adicional. |
...
| Escopo da agenda materializado por código de papel (`D-AGD-12`) | **RISCO BAIXO DECLARADO E HOMOLOGADO** (§6-AI.5): AGD-A é o **primeiro endpoint de domínio** a materializar a célula `C` de `docs/04` §4, e o faz para a agenda derivando o escopo do CÓDIGO do papel que concede `agenda.gerenciar` — operacional para `ADMINISTRADOR`/`RECEPCIONISTA`, próprio para os demais, fail-closed. É exatamente o que `D-AGD-12` decidiu, com a limitação ali declarada. **`P2.2-05` continua NÃO INICIADA** e nada foi antecipado dela; o mecanismo geral, quando existir, substituirá esta derivação. Permissões clínicas (`prontuario.*`) permanecem sem endpoint e sem escopo |
...
14. ~~**`AUT-002` — revogação administrativa de sessão de terceiro (`P-2.3D-07`)**~~ — **CONCLUÍDA / INTEGRADA NA `main` em 14/09/2026** (PR [#46](https://github.com/BrunoMNoronha/techlab-fisio/pull/46), merge commit `b6427e1ab999aae40f41e7745fe374f32b719836`). Decisão `D-2.3D-19` (`docs/12` §5.19) materializada integralmente: `DELETE /auth/sessoes/:sessaoId` sob `@RequerPermissao("sessoes.revogar_terceiro")`, ator extraído exclusivamente da sessão autenticada, `204` uniforme anti-enumeração, mutação e auditoria atômicas via `AuditWriter`, inutilização imediata de terceiro, autorrevogação no-op segura. Critérios `A-01`..`A-22` e mutation challenges `M1`..`M4` provados com sucesso. Zero drift de persistência, zero dependência nova. **AUT-005 permanece marco distinto e sem fatia atribuída.**
13. **`FRONT-F0` — fundação do frontend (`apps/web`)** — **INTEGRADA NA `main` em 05/09/2026** (PR [#24](https://github.com/BrunoMNoronha/techlab-fisio/pull/24), merge commit `80986e7`; registro em `docs/08` REV. 22–25). Fundação técnica **sem** funcionalidade de negócio e **sem** integração com `apps/api`; `V-06.c` aprovada; **`P-2.3D-04` (CSRF × `apps/web` real) tecnicamente elegível, sem decisão**. Próximo marco do backend permanece **AUT-005** e a revogação administrativa de sessão de terceiro — sem fatia atribuída, por autorização própria. **Superada em 06/09/2026 pela Fatia 1** (item 15): `apps/web` **deixou de estar sem integração com `apps/api`**.
15. **Fatia 1 da integração `apps/web` ↔ `apps/api` (proxy same-origin, client HTTP e tela de login)** — **INTEGRADA NA `main` em 06/09/2026** (PR [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27), commits `c891a71` e `c435b00`, merge commit `0d76474` sobre `aba939c`, CI verde na run [34065299227](https://github.com/BrunoMNoronha/techlab-fisio/actions/runs/34065299227)); registro pós-medição em `docs/08` REV. 26. **9 arquivos, todos sob `apps/web/`; `apps/api` e `packages/` intocados — zero drift, zero dependência nova.** Medido sobre a `main` integrada: `typecheck` 0, `build` 0, `verify-web-integration.mjs` **24/24**. **`P-2.3D-04` ENCERRADA em 15/09/2026** (§9) após a prova E2E real same-origin integrada à CI por `P-2.3D-08` (PR #47).
16. ~~**`P-2.3D-08` — Consulta da sessão autenticada atual e prova E2E real same-origin (`D-2.3D-20`)**~~ — **CONCLUÍDA / INTEGRADA NA `main` em 15/09/2026** (PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47), merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI run `34983136046`) (§6-S). Materialização de `D-2.3D-20` (`GET /auth/sessao`) sob `SessaoAutenticadaGuard`, safe GET method, sem CSRF, sem `X-TLF-Requisicao`, sem body, sem synchronizer token, `Cache-Control: no-store`, resposta tipada `{ usuarioId, sessaoId }`. Proteção CSRF (`ProtecaoCsrfGuard`) movida exclusivamente para rotas mutantes (`login`/`logout`). Proibição expressa de evento de auditoria (`usuario.sessao.consultada` proibida). Prova E2E automatizada e versionada (`apps/web/scripts/verify-web-api-e2e.mjs`) exercitando PostgreSQL 18 real, NestJS real compilado, Next.js real com proxy Route Handler same-origin, `ProtecaoCsrfGuard` real e cookie de sessão real; integrada ao root `package.json` (`verify:web-api-e2e`) e ao fluxo `.github/workflows/ci.yml`. Critérios `A-01`..`A-22` e mutation challenges `M1`..`M4` provados. Zero drift de persistência, zero dependência nova. Encerramento factual de `P-2.3D-04` homologado em PR 2.
17. ~~**`P-BACK-01` / `R2.2-04` — `D-AUD-09` e `D-AUD-09-A`**~~ — **INTEGRADA NA `main` em 16/09/2026** (PR [#49](https://github.com/BrunoMNoronha/techlab-fisio/pull/49), commits `8bf9057` e `02f01fc`, merge commit `c2d57b808eafffbffbeee765ccc64d992c21fb5b`, CI run `35076322751`) (§7.5, §7.5.9). Validação semântica fail-closed das duas whitelists positivas remanescentes materializada; `R2.2-04` **ENCERRADO** (§7.2 — REV. 43), sem declaração de risco zero. **`P-BACK-01` permanece EM ANDAMENTO** — seguem `L-06` (ABERTA / BLOQUEADA), a materialização técnica de `L-08`, os emissores de domínio ausentes e o visualizador AUD-004. **AUT-005 permanece marco distinto e sem fatia atribuída.**
18. ~~**`P-2.3D-10` — listagem administrativa das sessões ativas de um usuário (`D-2.3D-22`)**~~ — **CONCLUÍDA / INTEGRADA NA `main` em 17/09/2026** (PR [#64](https://github.com/BrunoMNoronha/techlab-fisio/pull/64), merge `bc6a763`; §6-V). Correções da revisão automatizada pós-merge em §6-V.4.
...
23. ~~**`CFG-005` — motivos de cancelamento**~~ — **PUBLICADA NA `main` em 17/09/2026** (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§6-AB, §6-AE). `docs/07` §10.1/§10.2 alinhado na REV. 2.4. Interface administrativa segue em fatia própria. *(Correção factual da REV. 72.)*
24. ~~**`PAC-A` — pacientes (cadastro administrativo, localização, duplicidade, situação)**~~ — **PUBLICADA NA `main` em 17/09/2026** (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§6-AC, §6-AE; decisões `docs/17`). *(Correção factual da REV. 72.)*
25. ~~**`CFG-004` — formas de pagamento**~~ — **PUBLICADA NA `main` em 17/09/2026** (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§6-AA, §6-AE). `docs/07` §10.1/§10.2 alinhado na REV. 2.4. *(Correção factual da REV. 72.)*
26. ~~**`PRO-A` — cadastro de profissionais**~~ — **PUBLICADA NA `main` em 17/09/2026** (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§6-AD, §6-AE; decisões `docs/18`). *(Correção factual da REV. 72.)*
27. ~~**`PRO-003` — disponibilidade e horários do profissional**~~ — **PUBLICADA NA `main` em 17/09/2026** (PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`) (§6-AG; decisões `docs/16`). *(Correção factual da REV. 72: até a REV. 71 este item dizia "NÃO PUBLICADA", o que era verdade na data daquele registro.)* A regra de `D-PRO3-04` **deixou de estar sem consumidor**: AGD-A a consome no passo 9 de `D-AGD-04` (§6-AI). Bloqueio de agenda, férias e exceções por data seguem fora de escopo.
28. **`AGD-A` — núcleo inicial da agenda** — **INTEGRADA NA `main` em 17/09/2026** (PR [#90](https://github.com/BrunoMNoronha/techlab-fisio/pull/90), merge `ec688866af4934817730947607d6e4491459c1ed`) (§6-AI; decisões `docs/15`).
29. **`AGD-B` — check-in e falta** — **INTEGRADA NA `main` em 18/09/2026** (PR [#99](https://github.com/BrunoMNoronha/techlab-fisio/pull/99), merge `ab6c60143d7195cfaf43fd809119ded7ec4ad54b`). As issues #92, #97, #95, #93 e #94 estão concluídas; a issue #96 é tratada pela sincronização OpenAPI/documental deste PR #103, e seu encerramento depende da integração deste PR. AGD-C, AGD-D e AGD-E permanecem em fatias próprias.

## 11. Histórico de revisões

| REV. | Data | Conteúdo |
| --- | --- | --- |
| **73** | **18/09/2026** | **AGD-B INTEGRADA NA `main` + SINCRONIZAÇÃO FACTUAL/OPENAPI**. Registra PR #99 (merge `ab6c601`), contratos de check-in/falta, permissões `agenda.checkin`/`agenda.falta`, máquina de estados, precedência 409/422, regras temporais/fuso, escopo pela permissão efetiva, `FOR UPDATE`, histórico append-only sem auditoria, três corridas concorrentes, 18 cenários de integração e 11/11 mutation challenges; reconcilia listas fechadas e provas de UUID/corpo estrito. Nenhuma decisão `D-AGD-*`, migration, schema, golden, dependência, permissão ou ação de auditoria criada/alterada. |
| **72** | **17/09/2026** | **`AGD-A` — NÚCLEO INICIAL DA AGENDA IMPLEMENTADO E MEDIDO EM BRANCH LOCAL, NÃO PUBLICADA** (§6-AI). `D-AGD-01`..`D-AGD-17` materializadas na fatia AGD-A sob autorização expressa; migration `20260917230000_agendamento_cancelamento_coerente` (CHECK de coerência do cancelamento, `D-AGD-07`, que resolve `D-CFG-51`); `GET`/`POST /agendamentos`, `GET /agendamentos/:id`, confirmação, remarcação e cancelamento, mais `GET /agenda/opcoes`, todas sob `agenda.gerenciar`; escopo operacional (Administrador/Recepcionista) x próprio (demais, fail-closed); uma linha de `historico_agendamento` por mutação efetiva e um evento por criação/remarcação/cancelamento, na mesma transação e com o mesmo `correlacao_id`; confirmação só no histórico. `TA-01`..`TA-19` cobertos, `TA-09` contra PostgreSQL real. typecheck, build, lint:migrations (18 · 37), test:api 54 suites · 1622 passed, schema:verify byte a byte (62338 bytes) e `migrate diff` exit 0, verify:openapi-runtime 77, verify:api-integration 27 suites · 1006 passed · 2 skipped, smoke:api, verify:from-scratch 18/18 · 10 suites · 99 passed, web 78, verify:web-api-e2e 50, Playwright 13 — todas verdes, 0 resíduos. `docs/07` REV. 2.6 e `docs/15` REV. 6 acompanham. Nenhuma permissão, ação de auditoria, chave de `contexto` ou dependência nova. |
...
| **45** | **16/09/2026** | **AUT-005 / `D-2.3D-21` — ativação e inativação administrativa de usuário IMPLEMENTADA EM BRANCH PRÓPRIA (`agent/aut-005-situacao-usuario`); NÃO INTEGRADA** (§6-T). `D-2.3D-21` homologada por Bruno Menezes Noronha em 16/09/2026 (`docs/12` §5.21, REV. 15). Rota `PATCH /auth/usuarios/:usuarioId/situacao`; critérios `A-01`..`A-16` atendidos; unitários 847/847, integração 436 verdes (2 puladas por desenho), `verify:openapi-runtime` 24/24. Zero drift de persistência e de dependências. |
| **44** | **16/09/2026** | **REGISTRO PÓS-INTEGRAÇÃO DE `D-AUD-09` / `D-AUD-09-A` E DO ENCERRAMENTO DE `R2.2-04` NA `main` (PR #49, MERGE `c2d57b8`)** (§7.5.9). Registro **exclusivamente factual**; nenhuma decisão normativa criada, alterada, renumerada ou reaberta; nenhuma linha de código, teste, schema, migration, golden, protected-objects, OpenAPI, contrato HTTP, dependência ou CI alterada; Base Imutável V2 intacta. **(a) Integração:** PR [#49](https://github.com/BrunoMNoronha/techlab-fisio/pull/49) integrada em 16/09/2026 por Bruno Menezes Noronha com os commits `8bf9057963177e704b65a11117144668095e8e35` (REV. 42 + REV. 43) e `02f01fc17a765703941995bb63a34a2dcacf9b6d` (sincronização de §7.5.8), merge commit `c2d57b808eafffbffbeee765ccc64d992c21fb5b` (pais `3d8cbfc` e `02f01fc`), método merge commit; CI verde sobre o HEAD da PR (run `35076322751`). **(b) Validação pós-merge:** `pnpm verify:api-integration` sobre a `main` integrada — exit 0, 12 suítes · 424 passed · 2 skipped. **(c) Atualizados:** cabeçalho, bloco de estado de §7.5, nova §7.5.9, item 17 de §10, pendências vivas e estado da `main`. **(d) NÃO alterados:** §7.2/§7.2.0 (`R2.2-04` ENCERRADO — REV. 43; não é declaração de risco zero), `P-BACK-01` (EM ANDAMENTO), `L-06` (ABERTA / BLOQUEADA), catálogo de ações (25), whitelist (3 positivas · 22 vazias), `docs/09`. As menções de "publicação não autorizada" das REV. 42 e REV. 43 permanecem como registro histórico daquelas rodadas. |
| **43** | **15/09/2026** | **`P-BACK-01` / `R2.2-04` — ENCERRAMENTO FORMAL DE `R2.2-04` APÓS A HOMOLOGAÇÃO DE `D-AUD-09-A` (`docs/09` §14.9)** — revisão **exclusivamente documental**, sem uma única linha de código, teste, schema, migration, golden, protected-objects, OpenAPI, contrato HTTP, dependência ou CI alterada. **(a) Decisão normativa:** em 15/09/2026 Bruno Menezes Noronha resolveu a ambiguidade da cláusula final de `docs/09` §13.3 adotando a **leitura restrita** de “por esta via” — `L-06` impede o encerramento de `R2.2-04` apenas **pela decisão relativa à auditoria de leitura clínica**, e **não** cria dependência global entre os dois itens, que têm objetos distintos (conteúdo de `evento_auditoria.contexto` × política de auditoria de acesso de leitura). Registrada em `docs/09` §14.9 como `D-AUD-09-A`; §14.8 preservada na íntegra como registro histórico da divergência, com a pendência substituída pela decisão. **(b) Efeito:** a **sétima** condição de §7.2.0 passou a **SIM** e **`R2.2-04` ficou ENCERRADO** (§7.2), apontadas as provas existentes — whitelist fail-closed por ação, rejeição de chave fora da whitelist, regras semânticas das três whitelists positivas, validação de tipos/escalares antes da semântica, ausência de normalização, ausência de serialização genérica de entidade em `contexto` e os testes correspondentes. **Não** se declarou risco zero: o residual “médio — depende de revisão de código” de `docs/07` §24.4 permanece. **(c) Revisão técnica independente de toda a branch:** `ehDataCivilCanonica` (somente `string`, forma estrita `YYYY-MM-DD`, sem `Date`, sem normalização, ano `0000` rejeitado, meses 01..12, dias reais por mês, bissexto por 400 / 4-e-não-100, `1900-02-29` rejeitado, `2000-02-29` aceito, datetimes e espaços rejeitados) e `ehUuidCanonico` (somente `string`, hexadecimal minúsculo 8-4-4-4-12, sem exigência de versão ou variante, maiúsculas rejeitadas sem conversão, sem hífens/`{}`/`urn:uuid:` rejeitados, sem trim) conferidos linha a linha; ownership catálogo-declara / validator-executa preservado; precedência estrutural × semântica preservada (objeto, array, `NaN`, `±Infinity` → `VALOR_NAO_ESCALAR`); mensagens sem valores; `autor_original_usuario_id` segue recusada antes da semântica; nenhuma whitelist ampliada; nenhuma chave tornada obrigatória. **Nenhum defeito encontrado; nenhuma correção de código foi necessária.** **(d) Revalidação sobre a árvore final:** suite da API, integração PostgreSQL real, typecheck, build, `lint:migrations` e `git diff --check`. **(e) NÃO alterados:** `L-06` (**ABERTA / BLOQUEADA**, regra/gatilhos/dependências intactos), `P-BACK-01` (**EM ANDAMENTO**), catálogo de ações (25), whitelist (3 positivas · 22 vazias), `D-AUD-01`..`D-AUD-08`, `PBACK-AUD-01`..`PBACK-AUD-09`, `D-AUD-09` (§14.1..§14.7), Base Imutável. **Nenhuma rota de leitura clínica autorizada; `prontuario.acessado` não criada.** Publicação **não autorizada** nesta rodada; branch `agent/p-back-01-r2-2-04-semantica-contexto`, sem commit, push, PR ou merge. |
| **42** | **15/09/2026** | **`P-BACK-01` / `R2.2-04` — MATERIALIZAÇÃO DE `D-AUD-09` (`docs/09` §14): VALIDAÇÃO SEMÂNTICA DAS DUAS WHITELISTS POSITIVAS REMANESCENTES** (§7.5). **`R2.2-04` NÃO FOI ENCERRADO** (§7.2, §7.2.0): a condição semântica que o mantinha aberto foi suprida, mas a inspeção final encontrou a cláusula de `docs/09` §13.3 que víncula esse encerramento a **`L-06`**, ainda aberta — divergência de leitura registrada e devolvida a Bruno em `docs/09` §14.8, sem resolução silenciosa. **(a) Decisão normativa:** `D-AUD-09` homologada por Bruno Menezes Noronha em 15/09/2026 e registrada em `docs/09` §14 — as chaves `data_referencia_anterior`/`data_referencia_nova` exigem **data civil gregoriana canônica `YYYY-MM-DD` existente no calendário** (bissexto por 400/4-e-não-100; ano zero rejeitado; sem fuso, sem `Date`, sem normalização) e `registro_original_id` exige **UUID textual canônico em minúsculas** (sem restrição de versão/variante, sem consulta ao banco, sem normalização de caixa). **(b) Materialização:** helpers puros `ehDataCivilCanonica` e `ehUuidCanonico` no catálogo e três novas entradas em `SEMANTICA_CONTEXTO`; a mensagem de `VALOR_SEMANTICAMENTE_INVALIDO` tornou-se **genérica**, o que **atende** `F-2.3C-REV-03` (§7.5.4); a separação catálogo-declara / validator-executa foi preservada. **(c) Testes:** o teste que materializava o estado anterior (`"qualquer-string"` aceita em datas/uuid) foi **substituído**, nunca mantido em paralelo; acrescentadas as matrizes de válidos/inválidos, a precedência estrutural × semântica, a ausência de normalização e a não obrigatoriedade das chaves; o teste do `AppModule` real ganhou 5 provas. Medições: focados 2 suítes · 265; API 34 suítes · 835; integração PostgreSQL 18 real 12 suítes · 424 passados · 2 skipped; typecheck, build e `lint:migrations` verdes. **(d) `R2.2-04` — NÃO ENCERRADO:** as sete condições foram verificadas uma a uma (§7.2). As **seis substantivas** estão satisfeitas; a sétima **não**: a inspeção final encontrou a cláusula final de `docs/09` §13.3 (`PBACK-AUD-02`, 05/09/2026) — *"enquanto `L-06` permanecer aberta, `R2.2-04` não pode ser declarado encerrado por esta via"* — e `L-06` segue **ABERTA / BLOQUEADA**. A expressão "por esta via" admite leitura restrita e ampla; a divergência foi **registrada e devolvida a Bruno** (`docs/09` §14.8), não resolvida silenciosamente (`TLF-BASE-V2` §15), e prevaleceu a leitura conservadora: **risco mantido ABERTO**, reclassificado para **POLÍTICA DE CONTEÚDO INTEGRALMENTE MITIGADA**, sem trabalho técnico pendente (§7.2.0). Estado anterior (REV. 8–REV. 41) preservado em §7.2.1, sem reescrita. **(e) NÃO alterados:** catálogo de ações (25), whitelist (3 positivas · 22 vazias), obrigatoriedade de chave, ownership (`D-AUD-08`), política de escalares (§7.3), schema, migrations, golden, protected-objects, OpenAPI, contratos HTTP, dependências, lockfile e CI; nenhum emissor de domínio criado. `D-AUD-01`..`D-AUD-08` e `PBACK-AUD-01`..`PBACK-AUD-09` **não** reabertas. **(f) Estados vivos:** `R2.2-04` **ABERTO / POLÍTICA DE CONTEÚDO INTEGRALMENTE MITIGADA** (não encerrado); `D-AUD-09` **HOMOLOGADA E MATERIALIZADA**; **`P-BACK-01` EM ANDAMENTO** (não encerrada); **`L-06` ABERTA / BLOQUEADA** (inalterada). **(g) Publicação NÃO AUTORIZADA nesta rodada:** sem commit, push, PR, merge, rebase, tag, release ou deploy; branch `agent/p-back-01-r2-2-04-semantica-contexto` deixada pronta para revisão independente. |
| **41** | **15/09/2026** | **REGISTRO PÓS-INTEGRAÇÃO DE `P-2.3D-08` / `D-2.3D-20` NA `main` (PR #47, MERGE `89fa492`) E ENCERRAMENTO FACTUAL DE `P-2.3D-04` MEDIANTE PROVA E2E REAL SAME-ORIGIN** (§6-S). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-20` vigentes; Base Imutável V2 intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependência adicionada em `package.json` ou lockfiles. **(a) Integração de `P-2.3D-08` na `main`:** integrada em 15/09/2026 por Bruno Menezes Noronha via PR [#47](https://github.com/BrunoMNoronha/techlab-fisio/pull/47) (commit `4f78c4ed2de5b2e9f358f55accfa021b027fc124`, merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`, CI verde run `34983136046`, 3m54s). Endpoint `GET /auth/sessao` em produção compilada; prova E2E real automatizada `apps/web/scripts/verify-web-api-e2e.mjs` versionada e integrada à CI. **(b) Encerramento factual de `P-2.3D-04`:** Encerrada em 15/09/2026 após a integração do PR #47 (merge commit `89fa492603bdb6e90693b03544c7857ee7d7fb3d`) que versionou `apps/web/scripts/verify-web-api-e2e.mjs` e o passo correspondente na CI, executando simultaneamente PostgreSQL real em container, NestJS real compilado, Next.js real compilado com Route Handler de proxy same-origin (`/api/*`), `ProtecaoCsrfGuard` real e cookie real de sessão, provando que a baseline de CSRF de `D-2.3D-07` (`SameSite=Strict`, cabeçalho obrigatório `X-TLF-Requisicao` nas mutações, Fetch Metadata, validação de `Origin`, ausência de CORS) protege a fronteira de ponta a ponta sem necessidade de synchronizer token adicional. **(c) Pendências vivas preservadas:** `P-BACK-01` EM ANDAMENTO; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `P-2.3D-05` e `P-2.3D-06` abertas/adiada; `L-06` ABERTA / BLOQUEADA; `R-BL-09` ABERTO/monitorado; `AUT-005` NÃO MATERIALIZADA; `P2.2-05` NÃO INICIADA. |
| **40** | **15/09/2026** | **MATERIALIZAÇÃO DA DECISÃO NORMATIVA `D-2.3D-20` (CONSULTA DA SESSÃO AUTENTICADA ATUAL VIA `GET /auth/sessao`), PROVA E2E REAL SAME-ORIGIN (`verify-web-api-e2e.mjs`) E RECONCILIAÇÃO FACTUAL DA INTEGRAÇÃO DE `P-2.3D-07` NA `main`** (§6-S). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-20` vigentes; Base Imutável intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependência adicionada em `package.json` ou lockfiles. **(a) Reconciliação factual de `P-2.3D-07`:** integrada na `main` em 14/09/2026 via PR [#46](https://github.com/BrunoMNoronha/techlab-fisio/pull/46), merge commit `b6427e1ab999aae40f41e7745fe374f32b719836`. **(b) Materialização de `D-2.3D-20` no backend (`apps/api`):** endpoint `GET /auth/sessao` sob `@UseGuards(SessaoAutenticadaGuard)`, contrato OpenAPI v3.1 tipado com `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@HttpCode(200)`, `@Header("Cache-Control", "no-store")`, respostas `200, 401, 500` e DTO `ConsultarSessaoRespostaDto` `{ usuarioId, sessaoId }`. Proteção CSRF (`ProtecaoCsrfGuard`) movida da classe `AuthController` para `@Post("login")` e `@Post("logout")`; `GET /auth/sessao` opera sob método seguro HTTP sem CSRF guard, sem `X-TLF-Requisicao`, sem corpo e sem synchronizer token. Proibição expressa de evento de auditoria respeitada (`usuario.sessao.consultada` proibida). `SessaoAutenticadaGuard` provido diretamente no `AuthModule`. `ErroAutenticacaoFilter` estendido para reconhecer `GET /auth/sessao`. **(c) Prova E2E real same-origin automatizada (`apps/web/scripts/verify-web-api-e2e.mjs`):** executa sobre PostgreSQL 18 real em contêiner descartável, NestJS compilado real (`main.js`), Next.js real (`next start`), Route Handler do proxy same-origin de Next.js, `ProtecaoCsrfGuard` real do backend e cookie de sessão real; integrada ao root `package.json` (`verify:web-api-e2e`) e à CI (`.github/workflows/ci.yml`). **(d) 4 mutation challenges executados e revertidos por hash (M1..M4):** M1 (proteção de sessão), M2 (Host no proxy Next.js), M3 (`X-TLF-Requisicao` no CSRF guard) e M4 (Origin estrita no CSRF guard). **(e) Critérios de aceite `A-01`..`A-22` integralmente provados.** `P-2.3D-04` permanece formalmente **ABERTA** nesta rodada de implementação, a ser encerrada exclusivamente em PR documental posterior após integração da prova na `main`. Branch de trabalho: `agent/p-2.3d-08-sessao-atual-e2e`. |
| **39** | **14/09/2026** | **IMPLEMENTAÇÃO E VALIDAÇÃO DA REVOGAÇÃO ADMINISTRATIVA DE SESSÃO DE TERCEIRO (`P-2.3D-07` / `AUT-002` / `sessoes.revogar_terceiro` / `D-2.3D-19`) EM BRANCH PRÓPRIA (`agent/p-2.3d-07-revogacao-sessao-terceiro`) — NÃO HOMOLOGADA / NÃO INTEGRADA NA `main`** (§6-R). Nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-19` intactas; Base Imutável intacta; zero drift de persistência (`packages/database`, 10 migrations, schema, golden e protected-objects intocados); zero dependência nova em `package.json` ou lockfile. **(a) Runtime implementado:** endpoint `DELETE /auth/sessoes/:sessaoId` sob `ProtecaoCsrfGuard` e `@RequerPermissao("sessoes.revogar_terceiro")`; OpenAPI v3.1 tipado com `@ApiCookieAuth(NOME_ESQUEMA_SESSAO)`, `@HttpCode(204)` e respostas de erro `400`, `401`, `403`, `413`, `500`; resposta anti-oráculo uniforme `204 No Content` sem corpo e sem cookies para todos os desfechos autorizados (revogação efetiva, inexistente, já revogada, já expirada e autorrevogação no-op). **(b) Ator estritamente derivado da sessão autenticada:** `ContextoAutenticado.usuarioId` sem parâmetros de ator no contrato HTTP. **(c) Inutilização imediata e atomicidade transacional:** `sessao_autenticacao.status` atualizado para `REVOGADA` e evento `usuario.sessao.revogacao` (`alvo_tipo = "sessao_autenticacao"`, `alvo_id = sessaoId`, `resultado = SUCESSO`, contexto `{}`) registrados na **mesma transação** de banco via `AuditWriter`. Se a auditoria falhar, rollback desfaz a mutação. Autorrevogação via rota administrativa é no-op seguro que mantém a sessão ativa e não emite auditoria falsa. **(d) 4 mutation challenges executados, detectados e revertidos por hash (M1..M4):** M1 (origem do ator), M2 (RBAC / permissão), M3 (inutilização imediata da sessão) e M4 (atomicidade da auditoria). **(e) Critérios de aceite `A-01`..`A-22` integralmente provados.** Validação integral verde: `typecheck` 0, `lint:migrations` 0, `build` 0, `verify:argon2-runtime` 16/16, `verify:openapi-runtime` 20/20, `test:api` 34 suítes / 728 testes, integração da API com PostgreSQL 18 real 11 suítes / 414 aprovados / 2 pulados POSIX. Branch de trabalho: `agent/p-2.3d-07-revogacao-sessao-terceiro`; sem commit, push, PR, merge, release ou deploy nesta execução. |
| **38** | **06/09/2026** | **REGISTRO PÓS-MEDIÇÃO DA INTEGRAÇÃO DAS PR [#26](https://github.com/BrunoMNoronha/techlab-fisio/pull/26) E [#27](https://github.com/BrunoMNoronha/techlab-fisio/pull/27) NA `main` — SINCRONIZAÇÃO DOCUMENTAL EXCLUSIVAMENTE FACTUAL.** Mesmo precedente `MEDIR → REGISTRAR` das REV. 10, 15, 20, 24, 29 e 36: **nenhuma decisão normativa criada, alterada, renumerada ou reaberta; `D-2.3D-01`..`D-2.3D-19` intactas; `docs/09` §12 não reaberto; Base Imutável intacta; nenhuma linha de runtime, persistência, teste ou dependência tocada.** **(a) PR #26 — `D-2.3D-19` publicada e integrada:** commit `0444fef`, merge commit `aba939c` sobre `867e9d2`, 06/09/2026 03:47:14Z, por Bruno Menezes Noronha; CI verde (run 34009649437, 3m46s); 2 arquivos (`docs/10`, `docs/12`; +155/−18). A afirmação da REV. 37 de que a rodada corria "sem commit, push, PR, merge ou deploy" era verdadeira quando escrita e é **preservada como registro histórico**; §10 item 14 recebeu a atualização pós-merge. **(b) PR #27 — Fatia 1 da integração `apps/web` ↔ `apps/api`:** commits `c891a71` e `c435b00`, merge commit `0d76474` sobre `aba939c`, 06/09/2026 23:01:48Z, por Bruno; CI verde (run 34065299227, 3m50s); **9 arquivos, +761/−16, todos sob `apps/web/`**. Proxy same-origin transparente (`app/api/[...caminho]/route.ts`) com preservação de `Host` público, `Origin`, `Sec-Fetch-*`, `X-TLF-Requisicao` e `Set-Cookie`, descarte de cabeçalhos hop-by-hop, guarda **fail-closed** contra SSRF (`400 CAMINHO_INVALIDO`) e `499` em aborto pós-repasse; client HTTP tipado (`lib/api-cliente.ts`) com `sanitizarCaminhoApi` recusando URL absoluta, protocol-relative, prefixo fora de `/api/` e path traversal, e injeção mandatória de `X-TLF-Requisicao` e `application/json` em mutações; tela de login em `app/login/`. **`apps/api` e `packages/` NÃO foram tocados** — zero drift, 10 migrations, golden e OpenAPI inalterados, `package-lock.json` intocado, **zero dependência nova** (o único delta de manifesto é o script `test` de `apps/web/package.json`). **(c) Medições locais sobre a `main` integrada** (`main` = `origin/main` = `0d76474`, host Windows, árvore limpa): `npm run typecheck` (raiz + 3 workspaces) **exit 0**; `npm run build` (`database → api → web`) **exit 0** com `/`, `/_not-found` e `/login` pré-renderizadas e `/api/[...caminho]` dinâmica; `npm run test --workspace @techlab-fisio/web` **exit 0 — 24/24** (8 de sanitização com código real, 5 de repasse HTTP simulado em `node:http` sem Route Handler, 3 de compatibilidade contra a função local de reprodução `avaliarGuard` sem o guard real do NestJS, e 8 de integridade estática no disco). As baterias de `apps/api` e de persistência **não foram reexecutadas** nesta rodada por ausência de delta: a PR não toca `apps/api` nem `packages/`, e a CI verde da PR já as cobre. **(d) `P-2.3D-04` NÃO encerrada** — permanece **ABERTA** em `docs/12` §11; a fatia **mede** a compatibilidade da topologia same-origin contra função local de reprodução e **preserva** a baseline de `D-2.3D-07` sem enfraquecimento, mas **não** executa o guard real do NestJS nem o Route Handler real em fluxo integrado ponta a ponta; homologar é ato de Bruno. **(e) Divergências registradas:** `apps/web/README.md` afirmava a pendência "resolvida ... aprovada por Bruno Menezes Noronha" **sem lastro documental**, citava "18 verificações" quando são 24 e mantinha as linhas da fundação "sem testes ... sem client HTTP" e `next.config.ts` "vazio" já superadas pelo conteúdo integrado — corrigidas editorialmente, por acréscimo datado e sem tocar código, no precedente de `F0H-01`; `verify-web-integration.mjs` **não** roda na CI nem no `npm test` da raiz, e `URL_API_INTERNA` **não** está em `.env.example` — registrados **sem** criar pendência nova e **sem** alterar workflow, script raiz ou `.env.example`. **(f) Pendências vivas preservadas:** `P-2.3D-07` ABERTA e revogação administrativa **NÃO IMPLEMENTADA**; `AUT-005` NÃO MATERIALIZADA; `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `R-BL-03` residual e ABERTO; `R-BL-09` ABERTO / monitorado; `P2.2-05` NÃO INICIADA. Nenhum deploy, tag ou release. Atualizados: cabeçalho, §9, §10, §11 e rodapé; `docs/08` REV. 26 e `docs/12` REV. 11 na mesma rodada. |
| **37** | **06/09/2026** | **REGISTRO DA DECISÃO NORMATIVA `D-2.3D-19` — REVOGAÇÃO ADMINISTRATIVA DE SESSÃO DE TERCEIRO (AUT-002 / `sessoes.revogar_terceiro`). RODADA EXCLUSIVAMENTE NORMATIVA E DOCUMENTAL — NENHUMA ALTERAÇÃO DE CÓDIGO, TESTE, SCHEMA, MIGRATION, OPENAPI, DEPENDÊNCIA OU CONFIGURAÇÃO.** Executada na branch documental `agent/aut-002-d19-decisao`, a partir de `main` = `origin/main` = `867e9d2` com árvore limpa; **sem commit, push, PR, merge ou deploy**. **(a) Antecedente factual.** A homologação técnica independente de 06/09/2026 mediu, e não presumiu: `git diff origin/main...HEAD` **vazio**; 40 branches remotos, 1 worktree e 0 stashes **sem** qualquer artefato da revogação administrativa; `git grep "D-2.3D-19"` com **0 ocorrências** — o registro terminava em `D-2.3D-18`. Veredito **`C — NÃO APTA`**, por **ausência do objeto da homologação**, não por defeito de implementação. Confirmou ainda que a lacuna era **normativa**: `D-2.3D-05` (logout) e `D-2.3D-18` (T-07) derivam a autoria da própria linha (`SET revogada_por_usuario_id = usuario_id`, §6-E/§6-P), e `D-2.3D-18` declara **expressamente** não determinar o valor para a operação administrativa — que, por isso, não era implementável sem decisão expressa de Bruno. **(b) Decisão registrada.** **`D-2.3D-19`**, homologada por Bruno Menezes Noronha (TLF-BASE-V1 §15, item 1) e registrada em **`docs/12` §5.19 (REV. 10)**: operação **individual** sobre sessão de terceiro identificada por `sessao_id` — identificador, **não** credencial, com o token do alvo nunca exigido nem exposto; **ator** exclusivamente derivado da sessão autenticada e validada pelo backend, persistido em `revogada_por_usuario_id` **e** em `evento_auditoria.ator_usuario_id`, **proibido** vir do corpo, query, path, cabeçalho ou DTO, **proibido** ser inferido do dono da sessão-alvo e **proibido** existir como campo de contrato ainda que fosse ignorado; **exceção delimitada** — a primeira e única — ao desenho de autoria derivada da própria linha, válida **somente** para `sessoes.revogar_terceiro` e **não generalizável**; **autorrevogação recusada** pela capacidade administrativa, com o logout mantido como via própria; **`204 No Content` uniforme** para revogação efetiva, sessão inexistente, já revogada, já expirada e alvo do próprio ator, sem corpo e sem cookie, com limite **declarado** de que a uniformidade é de envelope e **não** temporal, e sem absorver `400` estrutural nem falhas de autenticação, autorização, CSRF, conteúdo ou infraestrutura; **estado temporal preservado** — sessão já vencida por `D-2.3D-04` **não** é reclassificada como revogada pelo Administrador; **atomicidade e convergência** sob concorrência, com as invariantes de que o `204` efetivo nunca coexiste com sessão utilizável e de que não há ressurreição; **auditoria obrigatória** da transição efetiva por `usuario.sessao.revogacao`, `alvo_tipo = "sessao_autenticacao"`, `alvo_id = sessao.id`, `resultado = SUCESSO`, `justificativa = NULL` e `contexto` **vazio**, sem token, hash, cookie, senha, segredo, payload ou cabeçalho sensível; **atomicidade transacional** entre a mutação e a trilha, com **rollback** da revogação se o evento não puder ser persistido — expressamente **distinta** da Q2 *best-effort* de `PBACK-AUD-09`, porque ali não há mutação e aqui há, e exequível **sem infraestrutura nova**, já que o `AuditWriter` recusa cliente não transacional; **no-op não emite auditoria falsa de sucesso**, sem criar ação de "tentativa"; **zero drift** — nenhuma migration autorizada, com parada e devolução para aprovação caso a implementação encontre impossibilidade estrutural concreta. **(c) Nada foi ampliado.** `D-AUD-01`, `D-AUD-02`, `D-AUD-03`, `D-AUD-07` e **`PBACK-AUD-09`** permanecem **inalterados**: nenhuma ação, valor de `resultado` ou chave de `contexto` foi criada, e `sessoes.revogar_terceiro` **não** entra na lista fechada de `autorizacao.negada`, que continua com **um** item. `docs/09` e `docs/13` **não foram tocados**. **(d) Acrescentados em `docs/12`:** §10.4 com os critérios de aceite normativos **`A-01`..`A-22`** e as quatro provas de poder obrigatórias; alternativas rejeitadas **A-15**..**A-18**; riscos **`R-2.3D-09`** e **`R-2.3D-10`**, ambos aceitos e declarados; pendência **`P-2.3D-07`**. **(e) Estados preservados:** **AUT-002 continua NÃO IMPLEMENTADA** — esta revisão fecha **somente** a decisão; **`AUT-005` NÃO MATERIALIZADA**; `P-2.3D-04` ABERTA; `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA; `R2.2-04` ABERTO / MITIGADO PARCIALMENTE; `R-BL-03` residual e ABERTO; risco de volume **V0** de `PBACK-AUD-09` **permanece aceito** até a implementação. Atualizados: cabeçalho, §9 (linha nova de AUT-002 / `P-2.3D-07`), §10 (item 14) e §11. Nenhum registro histórico foi reescrito. |
...
**Fim — `docs/10-backend-implementacao.md` — REV. 40 — documento mutável; estado vivo das pendências da frente de backend.**
...
**Etapa 2.3D-B e fatias subsequentes, por fatia:**
...
| **P-2.3D-07** — Revogação administrativa de sessão de terceiro (`AUT-002` / `D-2.3D-19`) | **CONCLUÍDA / HOMOLOGADA / INTEGRADA NA `main` em 14/09/2026** | PR [#46](https://github.com/BrunoMNoronha/techlab-fisio/pull/46), merge commit `b6427e1ab999aae40f41e7745fe374f32b719836`; `D-2.3D-19`; §6-R; REV. 39 |
...
**Estado da `main` em 17/09/2026 (REV. 48):** `origin/main` = **`e7abe300e49b0fb1427fc19ef7843774b67212d5`**. **`CI-E2E0` integrada: PR #58 (merge `4156689`), PR #59 (merge `63bb058`) e PR #61 (merge `e7abe30`); `main` protegida com o job de CI como required check** (§6-U). Decisões de configuração da clínica registradas pela PR #60 (merge `261ec65`, `docs/14`). Revisão R3 do FRONT-E2E0 integrada pela PR #57 (merge `4f36462`). Correções de revisão do FRONT-E2E0 integradas pela PR #54 (merge `cb11f72`). **Reforço de provas de `P-2.3D-09` integrado pela PR #55 (merge `bce22e8`).** Registro pós-integração de `P-2.3D-09` pela PR #53 (merge `5ff4452`). **`P-2.3D-09` / AUT-005 / `D-2.3D-21` integrada pela PR #52 (merge `817da42`).** FRONT-E2E0 integrada pela PR #51 (merge `594e6f0`). Registro pós-integração de `D-AUD-09` pela PR #50 (merge `e6a5662`). `P-2.3D-07` integrada pela PR #46 (merge `b6427e1`). `P-2.3D-08` integrada pela PR #47 (merge `89fa492`); registro pós-integração pela PR #48 (merge `3d8cbfc`). **`D-AUD-09` / `D-AUD-09-A` integradas pela PR #49 (merge `c2d57b8`) — `R2.2-04` ENCERRADO** (§7.2; não é declaração de risco zero). **`P-2.3D-04` ENCERRADA.** `P-BACK-01` EM ANDAMENTO; `L-06` ABERTA / BLOQUEADA.

**Fontes normativas:** `TECHLAB_FISIO_BASE_IMUTAVEL_V1.md`, `TECHLAB_FISIO_BASE_IMUTAVEL_V2.md`, `docs/02`..`docs/07`, `docs/08` (baseline técnica), `docs/09` §12 (`D-AUD-01`..`D-AUD-08`), `docs/11` (`PROP-RN-2.3C-01`) e **`docs/12` (`D-2.3D-01`..`D-2.3D-21`)**.
