...
> **Revisão vigente:** REV. 34 (05/09/2026) — **ETAPA 2.3D-B / F7 (consolidação, revisão independente e rito de integração): EXECUTADA — ETAPA 2.3D ENCERRADA** (§6-Q). Revisão independente do conjunto integrado F1..F6 e segunda passagem adversarial sobre 17 hipóteses de ataque: **nenhum defeito de código novo**. **`L-F6-06` FECHADA:** reaberta do zero, com a semântica de `revogada_por_usuario_id` apurada no código integrado — três escritas, **nenhuma leitura** por código de aplicação, `NULL` em `ATIVA` e `EXPIRADA`, invariante `REVOGADA` ⟺ preenchido — e os atores da T-07 separados; o campo registra **quem praticou o ato do qual a revogação decorreu**, ator que `docs/09` §5 já fixa. Promovida a **`D-2.3D-18`** (`docs/12` §5.18, REV. 9) **sem alteração de código**, com prova regressiva já existente; `NULL`, Administrador iniciador e coluna nova registrados como alternativas rejeitadas. **Matriz normativa** `D-2.3D-01`..`D-2.3D-18` + AUT-004/D-04/RN-005/T-07: **todos COBERTA**, nenhum `PARCIAL`, `SEM PROVA` ou `INCONSISTENTE`. **Duas correções documentais:** baseline de §3 (`D-2.3D-01`..`D-2.3D-13`, desatualizada desde a F5) e `P-2.3D-03` (medida e verde desde a F3) — corrigida e **ENCERRADA**, sem alterar `D-2.3D-11`. **F0..F7 CONCLUÍDAS; `D-2.3D-01`..`D-2.3D-18` vigentes; nenhuma decisão local comportamental permanece aberta.** Permanecem fora da Etapa: `P-2.3D-01`, `P-2.3D-04`, `P-2.3D-05`, `P-2.3D-06` (adiada), **AUT-005 NÃO MATERIALIZADA** e a revogação administrativa de sessão de terceiro **NÃO IMPLEMENTADA**.
...
Última fatia da Etapa 2.3D (`docs/12` §9: "Consolidação, revisão independente e rito de integração"). Executada em 05/09/2026, a partir de `main` = `origin/main` = `50412d9` (F6 integrada e registrada). **Nenhuma expansão funcional.**

### 6-Q.1 Estado recebido de F1..F6

F1..F6 integradas na `main`; `D-2.3D-01`..`D-2.3D-17` vigentes; AUT-004 materializada. Uma única ambiguidade comportamental permanecia deliberadamente aberta — **`L-F6-06`** —, e a F7 a tratou como primeiro gate.

### 6-Q.2 `L-F6-06` — semântica apurada e decisão

A questão foi **reaberta do zero**, sem reutilizar a conclusão anterior. A tentativa de encerrá-la por analogia com o logout havia sido recusada, com razão, pela revisão da PR #21: `D-2.3D-05` governa o logout e não determina o valor na T-07.

**O que a apuração encontrou.** `sessao_autenticacao.revogada_por_usuario_id` é FK anulável para `usuario`, sem CHECK. Levantamento exaustivo no código integrado:
...
| Leituras por código de aplicação | **nenhuma** — o campo é trilha forense, não insumo de decisão de runtime |
| `NULL` ocorre em | toda sessão `ATIVA` e toda sessão `EXPIRADA` — a transição de expiração **não toca** a coluna |
...
| --- | --- | --- |
| Administrador iniciador | **Sim** | `segredo_recuperacao_senha.criado_por_usuario_id` e ator de `usuario.senha.recuperacao_iniciada` |
| Usuário alvo | **Sim** | `sessao_autenticacao.usuario_id` |
| Executor da conclusão | **Não como identidade** | a rota é sem sessão; o sistema prova **posse do segredo**, não identidade. `docs/09` §5 já fixa o **Usuário** como ator do evento de conclusão |
| Sistema | **Não** | não há usuário sintético nem flag; a única representação livre seria `NULL`, que já significa outra coisa |

**Fundamento adotado.** O campo registra **quem praticou o ato do qual a revogação decorreu** — e esse ator é exatamente o que `docs/09` §5 já fixa para o evento de auditoria correspondente: `Usuário` para `usuario.senha.recuperacao_concluida` ("Conclusão de D-04 (T-07)"). Não é analogia com o logout: é a mesma fonte normativa que já governa a autoria do ato.

Verificou-se ainda que `docs/09` §5 atribui ator **Administrador** a `usuario.sessao.revogacao` ("revogação administrativa … em massa via T-07"). Essa linha descreve a **revogação administrativa de terceiro** — cuja fonte é `docs/04` §8 e que **não existe** no produto — e não a revogação decorrente da recuperação, que a própria tabela audita pela linha de `usuario.senha.recuperacao_concluida`. A leitura **confirma** `L-F6-07`: emitir `usuario.sessao.revogacao` na conclusão atribuiria o ato a um Administrador ausente.

**Alternativas avaliadas:**

| Alternativa | Fidelidade semântica | Auditoria | Compatibilidade | Custo | Recomendação |
| --- | --- | --- | --- | --- | --- |
| **A — próprio usuário** (vigente) | Coerente com o ator de `docs/09` §5; derivada da própria linha, impossível forjar | Consistente com o evento que a causou | Preserva a invariante `REVOGADA ⟺ preenchido` | zero | **ADOTADA** |
| B — `NULL` | Colide com o significado vigente (`ATIVA`/`EXPIRADA`); afirmaria "sistema", que o modelo não representa | Perde o autor | **Quebra** a invariante | zero, mas perda semântica | rejeitada |
| C — Administrador iniciador | Falsa: ele iniciou; a revogação só ocorre se e quando alguém concluir, e pode nunca ocorrer | Confundiria com a futura revogação administrativa | Preserva a invariante | zero | rejeitada |
| D — coluna/enum de origem | Marginalmente mais expressiva | — | Exigiria migration e drift | migration + golden | rejeitada — o contrato vigente já representa o requisito |

**Classificação: comportamento vigente CORRETO** — promovido a **`D-2.3D-18`** (`docs/12` §5.18). **Nenhuma alteração de código.** A prova regressiva já existe e não precisou ser criada: `recuperacao-senha.integration.spec.ts:678` verifica `revogada_por_usuario_id = alvo.id` após a T-07, e `:701` verifica `NULL` na sessão fechada como `EXPIRADA`; `sessao.integration.spec.ts` cobre `NULL` em `ATIVA` (`:176`, `:424`), `NULL` em `EXPIRADA` (`:511`, `:525`) e o próprio usuário no logout (`:477`).
...
**Nenhum defeito de código novo foi encontrado.** Os defeitos reais da Etapa foram achados e corrigidos nas revisões das próprias fatias — `F-01`, `F-04`, `F-05`, `F-09`, `F-10`, `F3R-03` (F3); `M-01`, `M-02` (F6, pré-PR); **`M-03`** (F6, pós-review da PR #20). Dois achados **documentais** foram encontrados e corrigidos nesta fatia (§6-Q.5).

Pontos verificados que se confirmaram corretos, e que merecem registro por serem os de maior risco:

- **`#registrarAtividade` não é TOCTOU.** O `UPDATE` condicional exige `estado = 'ATIVA' AND expira_em > agora AND ultima_atividade_em > limiteOcioso`. `registrou = true` implica, **atomicamente**, sessão válida — a validação não depende da leitura anterior.
...
- **Guards fail-closed.** `PermissoesGuard` nega quando não há permissão declarada no handler e quando não há contexto autenticado — a ausência de metadado nunca libera.
- **Autoria não é parametrizável.** Tanto o logout quanto a T-07 derivam `revogada_por_usuario_id` da própria linha (`= usuario_id`), nunca de argumento: não há caminho para atribuir a revogação a terceiro.
- **`M-03` fecha o último TOCTOU conhecido do login**, e a guarda incondicional também cobre a inativação concorrente (`AND ativo = true`).

**Observação sem efeito (P3, não corrigida):** a T-07 calcula `agora` em `concluir` e a revogação em massa calcula o seu próprio em `revogarTodasDoUsuarioEm`. Em produção os instantes diferem por microssegundos e a ordem resultante (`encerrada_em` ≥ `consumido_em`) reflete a ordem real das operações; sob o relógio controlado dos testes são idênticos. Não há invariante violada e a correção não se justifica.

### 6-Q.4 Matriz normativa — decisão × implementação × prova

| Regra | Implementação | Prova | Estado |
| --- | --- | --- | --- |
| `D-2.3D-01` persistência de sessão | migration `20260827175151`; `token_hash`, `ultima_atividade_em`, CHECK | guardas 1–3; golden byte a byte | `COBERTA` |
...
| **T-07** | transação única | `:952`, `:997`, `:1209` | `COBERTA` |

Nenhum item `PARCIAL`, `SEM PROVA` ou `INCONSISTENTE`.

**Fora da matriz, por não pertencerem à Etapa 2.3D:** AUT-005 (ativação/inativação) e a revogação administrativa de sessão de terceiro seguem **NÃO MATERIALIZADAS**, sem fatia atribuída.

### 6-Q.5 Correções documentais desta fatia

| # | Achado | Correção |
| --- | --- | --- |
| 1 | `docs/10` §3 descrevia a baseline normativa como `D-2.3D-01`..`D-2.3D-13` — desatualizada desde a F5 | Atualizada para `D-2.3D-01`..`D-2.3D-18` |
| 2 | `P-2.3D-03` (compatibilidade ESM do `@nestjs/swagger`) constava **ABERTA**, embora medida e verde desde a F3, com encerramento formal reservado a decisão expressa | **ENCERRADA** em `docs/12` §11. A prova é permanente e roda a cada CI (`verify:openapi-runtime`, 19/19). `D-2.3D-11` **não** foi alterada |

### 6-Q.6 Pendências que permanecem abertas — e por que não são da Etapa 2.3D

| Pendência | Estado | Razão |
| --- | --- | --- |
| `P-2.3D-01` | **ABERTA** | Absorção editorial de colunas em `docs/07` — ação de governança documental separada |
...
| `P-2.3D-06` | **ADIADA** | Rate limit resiliente a restart/múltiplas instâncias — limitação aceita do MVP (`R-2.3D-02`) |
...
L-F6-06 = FECHADA (-> D-2.3D-18); nenhuma decisão local comportamental permanece aberta
P-2.3D-03 = ENCERRADA

AUT-001, AUT-002, AUT-003, AUT-004, AUT-006 = MATERIALIZADAS
AUT-005 = NÃO MATERIALIZADA / SEM FATIA ATRIBUÍDA
revogação administrativa de sessão de terceiro = NÃO IMPLEMENTADA
```
...
11. **`F7 — consolidação da Etapa 2.3D`** — **CONCLUÍDA em 05/09/2026** (§6-Q). Revisão independente do conjunto integrado F1..F6 e segunda passagem adversarial: **nenhum defeito de código novo**. `L-F6-06` — a última ambiguidade comportamental aberta — foi reaberta do zero, teve a semântica de `revogada_por_usuario_id` apurada no código integrado e foi promovida a **`D-2.3D-18`**, sem alteração de código. `P-2.3D-03` encerrada. Matriz normativa `D-2.3D-01`..`D-2.3D-18` + AUT-004/D-04/RN-005/T-07 **sem item PARCIAL, SEM PROVA ou INCONSISTENTE**. **Etapa 2.3D ENCERRADA.** Próximo marco: **AUT-005** (ativação e inativação de usuários) e a revogação administrativa de sessão de terceiro, ambas sem fatia atribuída — por autorização própria.
...
| REV. | Data | Conteúdo |
| --- | --- | --- |
| **34** | **05/09/2026** | **ETAPA 2.3D-B / F7 — CONSOLIDAÇÃO, REVISÃO INDEPENDENTE E ENCERRAMENTO DA ETAPA 2.3D** (§6-Q). **(a) `L-F6-06` FECHADA:** reaberta do zero, sem reutilizar a conclusão anterior. A apuração da semântica real de `revogada_por_usuario_id` no código integrado — três escritas, **nenhuma leitura** por código de aplicação, `NULL` em `ATIVA` e `EXPIRADA`, invariante `REVOGADA ⟺ preenchido` — e a separação dos atores da T-07 mostraram que o campo registra **quem praticou o ato do qual a revogação decorreu**, ator que `docs/09` §5 já fixa (`Usuário` para `usuario.senha.recuperacao_concluida`). Fundamento **não** é analogia com o logout. Promovida a **`D-2.3D-18`** (`docs/12` §5.18), **sem alteração de código**; prova regressiva já existente. Alternativas `NULL`, Administrador iniciador e coluna nova registradas como rejeitadas, com o motivo de cada uma. Confirmou-se também `L-F6-07`: a linha de `usuario.sessao.revogacao` em `docs/09` §5 descreve a revogação **administrativa** de terceiro (`docs/04` §8), que não existe no produto. **(b) Revisão independente de F1..F6 + passagem adversarial sobre 17 hipóteses: NENHUM defeito de código novo.** Registrados como corretos os pontos de maior risco — `#registrarAtividade` é atômico e não TOCTOU; usuário inativado é recusado por `resolverDoUsuario` mesmo com sessão `ATIVA`; guards fail-closed; autoria da revogação derivada da própria linha, não parametrizável; `M-03` fecha o último TOCTOU do login. **(c) Matriz normativa** `D-2.3D-01`..`D-2.3D-18` + AUT-004/D-04/RN-005/T-07 — **todos COBERTA**, nenhum `PARCIAL`, `SEM PROVA` ou `INCONSISTENTE`. **(d) Duas correções documentais:** a baseline em §3 dizia `D-2.3D-01`..`D-2.3D-13` (desatualizada desde a F5) e `P-2.3D-03` constava ABERTA embora medida e verde desde a F3 — corrigidas e encerrada, respectivamente; `D-2.3D-11` não alterada. **(e) `docs/12` REV. 9:** `D-2.3D-18` acrescentada, F7 → CONCLUÍDA em §9, `P-2.3D-03` → ENCERRADA em §11, AUT-002 passa a citar `D-2.3D-18`. **(f) ETAPA 2.3D ENCERRADA.** Permanecem, fora da Etapa: `P-2.3D-01`, `P-2.3D-04`, `P-2.3D-05`, `P-2.3D-06` (adiada), **AUT-005 NÃO MATERIALIZADA** e a revogação administrativa de sessão de terceiro **NÃO IMPLEMENTADA**. |
