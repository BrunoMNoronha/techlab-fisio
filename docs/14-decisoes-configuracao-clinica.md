...
| Aplicação de RN-014 e reavaliação da troca de fuso (`D-CFG-20`, `D-CFG-08`) | **CONTRATO DECIDIDO** (`D-CFG-60`..`D-CFG-63`: predicado de contenção, `422 FORA_DO_HORARIO_FUNCIONAMENTO`, estratégia flexível, troca de fuso permitida sem transição). **COMPONENTE IMPLEMENTADO (17/09/2026) — INTEGRADO NA BRANCH LOCAL `integration/local-fase4`, NÃO PUBLICADO NA `main`**: regra pura `apps/api/src/agenda/horario-funcionamento.regra.ts` e `VerificadorHorarioFuncionamento` (`verificador-horario-funcionamento.ts`), que lê fuso e grade na transação recebida, sem lock, e lança `ErroForaDoHorarioFuncionamento` (código `FORA_DO_HORARIO_FUNCIONAMENTO`, status `422` reservado) ou `CLINICA_NAO_CONFIGURADA`; provas: `test/agenda-horario-funcionamento.regra.spec.ts` (16, CH-AG-01..09, horário de verão, precisão de milissegundos) e `test/integration/agenda-horario-funcionamento.integration.spec.ts` (7, PostgreSQL real: grade gravada por CFG-002, fuso lido do banco, grade nova só para validações posteriores, ausência de escrita/auditoria e de bloqueio sob `FOR UPDATE` concorrente); mutações locais `FOR SHARE`, borda `<=`→`<` e remoção da checagem de meia-noite detectadas. **CONECTADO À AGENDA EM 17/09/2026** (`docs/15` §8, `docs/10` §6-AI): `VerificadorHorarioFuncionamento` passou a ser provider do `AgendaModule` e é chamado no passo 8 de `D-AGD-04`, na criação e na remarcação de agendamento; o `422 FORA_DO_HORARIO_FUNCIONAMENTO` é mapeado pela fronteira HTTP da agenda e `CH-AG-01`..`CH-AG-09` passaram a ser provados de ponta a ponta. **Nenhuma linha da regra ou do verificador foi alterada.** *(Correção factual: até a REV. 27 esta linha registrava "NÃO CONECTADO A NENHUMA OPERAÇÃO" e que o componente estava integrado em branch local não publicada — ambas verdadeiras na data daquele registro; o componente foi publicado na `main` pela PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89), merge `4b2e2c0`.)* |
| Itens remetidos por CFG-002/CFG-003/CFG-005 à **fatia de agenda** (`D-CFG-30`, `D-CFG-33`, `D-CFG-51`, `D-CFG-52`, `D-CFG-55`, `D-CFG-57`, `D-CFG-65`) | **DECIDIDOS E MATERIALIZADOS PELA AGENDA** em `docs/15`: leitura de catálogos pela agenda em `D-AGD-13` (`GET /agenda/opcoes`, só ativos, sem preço, sob `agenda.gerenciar`, sem ampliar as rotas administrativas); duração sobrescrevível por agendamento em `D-AGD-05`; motivo obrigatório em todo cancelamento, comportamento sem motivo ativo e leitura do motivo sob `FOR SHARE` em `D-AGD-07`; auditoria da agenda em `D-AGD-10`. **`D-CFG-51` foi SUPERADA**: `D-AGD-07` autorizou o CHECK de coerência do cancelamento, implementado na migration `20260917230000_agendamento_cancelamento_coerente` (`docs/07` §10.2, nota da REV. 2.6). Implementação em branch local `agent/agd-a-agenda-core`, **não publicada** (`docs/10` §6-AI) |
...
| REV. | Data | Descrição |
| --- | --- | --- |
| **28** | 17/09/2026 | **Atualização exclusivamente factual** após a implementação de AGD-A (`docs/15` §8, `docs/10` §6-AI). §5: o componente de RN-014 (parcela da clínica) **deixou de estar sem consumidor** — é chamado no passo 8 de `D-AGD-04` — e foi publicado na `main` pela PR [#89](https://github.com/BrunoMNoronha/techlab-fisio/pull/89); acrescentada linha registrando que os itens remetidos à fatia de agenda (`D-CFG-30`, `D-CFG-33`, `D-CFG-51`, `D-CFG-52`, `D-CFG-55`, `D-CFG-57`, `D-CFG-65`) foram decididos em `docs/15` e materializados, e que **`D-CFG-51` foi superada** por `D-AGD-07`. **Nenhuma decisão `D-CFG-*` criada, alterada ou reaberta; nenhuma linha de código de CFG alterada**; §§1–4 preservadas. |
